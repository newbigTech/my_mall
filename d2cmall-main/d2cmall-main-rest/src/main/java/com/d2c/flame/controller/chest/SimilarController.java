package com.d2c.flame.controller.chest;
...
public class SimilarController extends BaseController {
	...
	public ResponseResult productMixture(String topType, PageModel page, Integer interval) {
		...
		PageResult<SearcherProduct> pager = productSearcherQueryService
				.findTopRecomBy(similarManager.findTopIds(topType), page);
	...
	public ResponseResult productWorldList(PageModel page) {
		PageResult<SearcherProduct> pager = productSearcherQueryService.findTopRecomWorld(page);
	...
	public Response findPromotionTop(@PathVariable Integer limit, String topType) {
		...
		if (resList == null || resList.isEmpty()) {
			resList = productSearcherQueryService.findTopPromotion(similarManager.findTopIds(topType),
					limit * SimilarConst.SELECT_LIMIT_MULTIPLY);
	...
	public Response findTopCategory(@PathVariable Long categoryId, @PathVariable Integer limit) {
		...
		// 默认推荐数据
		if (resList == null || resList.isEmpty()) {
			resList = productSearcherQueryService.findTopByCategoryId(categoryId, limit);
		}
		return ResultHandler.success(getJsonList(resList));
	}
	...
	private List<?> findTopRecom(Integer limit, String topType) {
		...
		return productSearcherQueryService
				.findTopRecom(query, new PageModel(1, limit * SimilarConst.SELECT_LIMIT_MULTIPLY)).getList();
	}
