...
import com.d2c.cms.model.Theme.ThemeType;
import com.d2c.cms.search.model.SearcherTheme;
import com.d2c.cms.search.query.ThemeSearcherBean;
import com.d2c.cms.search.service.ThemeSearcherService;
...
import com.d2c.msg.model.Remind.RemindType;
import com.d2c.msg.service.MemberSearchKeyService;
import com.d2c.msg.service.RemindService;
...
public class ProductController extends BaseController {
	...
	public ResponseResult product(@PathVariable Long id) {
		...
		// 品牌推荐商品
		PageResult<SearcherProduct> pager = productSearcherQueryService.findRecommendProductByDesigner(designerId,
				new PageModel(1, 6));
		...
		if (StringUtil.isNotBlank(picUrl)) {
			...
			try {
				...
				if (productIds.getList() != null && productIds.getList().size() > 0) {
					Map<String, SearcherProduct> list = productSearcherQueryService
							.findOrderProductIds(productIds.getList(), 1);
					JSONArray array = new JSONArray();
					for (String id : productIds.getList()) {
						if (list.get(id) != null) {
							array.add(list.get(id).toJson());
						}
					}
