...
import com.d2c.order.third.payment.alipay.core.pcwap.AlipayPcNotify;
import com.d2c.order.third.payment.alipay.core.pcwap.AlipayUtil;
import com.d2c.order.third.payment.alipay.core.pcwap.AlipayWapNotify;
...
public class AlipayController extends BaseController {
			...
			if (valid) {
				...
				if (status.equalsIgnoreCase("success")) {
					...
					int value = paymentTxService.doPaySuccess(orderType, orderSn, new BigDecimal(totalFee), paySn,
							buyer_email, PaymentTypeEnum.ALIPAY, AlipayConfig.partnerId, null);
					if (value > 0) {
						result = true;
					}
	...
	public @ResponseBody String alipayWapNotify(Model model, HttpServletResponse response) {
		boolean result = false;
			...
			if (mySign.equals(request.getParameter("sign"))) {
				...
				if (status.equalsIgnoreCase("TRADE_FINISHED")) {
					...
					int value = paymentTxService.doPaySuccess(trade_order_type, orderNum, new BigDecimal(totalFee),
							tradeNo, buyer_email, PaymentTypeEnum.ALIPAY, AlipayConfig.partnerId, null);
					if (value > 0) {
						result = true;
					}
	...
	public @ResponseBody String alipayPcNotify(Model model, HttpServletResponse response) {
	...
	public String refundNotify(Model model, HttpServletResponse response) throws Exception {
		...
		try {
			...
			Date notifyTime = sdf
					.parse(new String(request.getParameter("notify_time").getBytes("ISO-8859-1"), "UTF-8"));
			// 退款批次号
				...
				if (num > 0 && StringUtils.isNotEmpty(resultDetails)) {
					...
					if (resultDetail.length >= 3) {
						...
						if ("SUCCESS".equalsIgnoreCase(resultCode)) {
							int success = refundTxService.doThirdSuccess(refundSn, notifyTime, payMoney, paySn);
							if (success == 1) {
								response.getWriter().println("success");
							}
						} else {
							// 如果失败则记录错误码
							refundService.updateErrorCode(refundSn, AlipayUtil.convertError(resultCode));
						}
					}
				}
			} else {
