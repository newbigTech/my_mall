...
import com.d2c.common.base.utils.security.AESUt;
				...
				if (responseSignNew.equals(responseSignOld)) { // 微信xml报文，签名验证
					...
					if ("JSAPI".equals(trade_type)) {
						result = paymentTxService.doPaySuccess(order_type, orderSn, tatolFee, tradeNo, payer,
								PaymentTypeEnum.WXPAY, mchId, appId);
					} else if ("NATIVE".equals(trade_type)) {
						result = paymentTxService.doPaySuccess(order_type, orderSn, tatolFee, tradeNo, payer,
								PaymentTypeEnum.WX_SCANPAY, mchId, appId);
					}
					if (1 == result) {
	...
	public void weixinRefundNotify(HttpServletResponse response) throws Exception {
		...
		if ("SUCCESS".equalsIgnoreCase(responseMap.get("result_code"))) {
			try {
				...
				String infoStr = Base64Ut.decode(req_info);
				String data = AESUt.decrypt(infoStr, MD5Ut.md5(WxAppConfig.PAY_KEY));
				HashMap<String, String> dataMap = StringUtil.xmlToMap(data);
				String refundSn = dataMap.get("out_refund_no");
				Date notifyTime = DateUtil.str2second(dataMap.get("success_time"));
				String refund_fee = dataMap.get("settlement_refund_fee");
				...
				int result = refundTxService.doThirdSuccess(refundSn, notifyTime, payMoney, paySn);
				if (1 == result) {
