...
import com.d2c.msg.service.MemberSearchKeyService;
...
public class ProductController extends BaseController {
	...
	public String product(@PathVariable Long id, ModelMap model) {
		...
		// 猜您喜欢
		PageResult<SearcherProduct> recommendPager = productSearcherQueryService
				.findRecommendProductByDesigner(designerId, new PageModel(1, 5));
		model.put("recommendProducts", recommendPager.getList());
		// 热卖商品
		PageResult<SearcherProduct> hotSalePager = productSearcherQueryService
				.findSaleProductByCategory(product.getProductCategoryId(), new PageModel(1, 7));
		model.put("hotSaleProducts", hotSalePager.getList());
		// 评价咨询
		CommentSearchBean searcher = new CommentSearchBean();
		searcher.setProductId(id);
		int countComment = commentSearcherService.count(searcher);
	...
	public @ResponseBody JSONObject adProduct(PageModel page, Long last) throws Exception {
		Date lastDate = null;
		if (last != null) {
	...
	public @ResponseBody JSONObject categories() {
		JSONObject root = new JSONObject();
	...
	public ResponseResult recommend() {
		...
		pager = productSearcherQueryService.findRecommendProductByDesigner(0L, new PageModel(1, 10));
	...
	public String getSkuStore(@PathVariable Long productId, ModelMap model) {
		...
		for (ProductSku sku : productSKUSet) {
			...
			map.put("colorId", sku.getColorId());
			map.put("size", sku.getSizeValue());
			map.put("sizeId", sku.getSizeId());
