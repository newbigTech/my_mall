...
(function($){
	...
	$.utilBaseModal={
		mask:function(option){
			...
			$('#'+type+'-modal-mask').css({'z-index':level,'height':$(document).height(),'filter:':'alpha(opacity='+options.opacity+')','opacity':(options.opacity/100),'-moz-opacity':(options.opacity/100),'background':options.background});
			$('#'+type+'-modal-outer').css({'z-index':level+1,'width':width+'px','padding':options.padding+'px'});
		...
		loading:function(type,status){
			if (status=='hide') {
				$('#'+type+'-modal-loading').fadeOut(function(){ $(this).remove();});
			}else{
	...
	$.utilAlertModal={
		...
		show: function(title, msg, value, mtype, callback) {
			...
			if (ie6){
				...
				height>=eheight && $(window).scroll(function(){$.utilBaseModal.positionUpdate(type);});
			}
			$(window).resize(function(){$.utilBaseModal.positionUpdate(type)});
		...
		if (options.iframe==true){
			...
			if (options.url) {
				if (options.datatype=='JSON') {
					$.getJSON(options.url+'&callback=?', function(data) {$.utilBaseModal.loading(type,'hide');$('#'+type+'-modal-outer').show();$('#'+type+'-modal-content').html(data);$.utilBaseModal.position(type);ispos=false;});
				}else{
					$.get(options.url, function(data) {$.utilBaseModal.loading(type,'hide');$('#'+type+'-modal-outer').show();$('#'+type+'-modal-content').html(data);$.utilBaseModal.position(type);ispos=false;});
				}
			}
			if (options.content){
	...
	$.fn.utilValidateForm=function(){
		...
		if (obj_form.find('.tip-validate').size()>0){
			$.each(obj_form.find('.tip-validate'),function(i,o){
				...
				if (validate_rule==undefined && validate_function==undefined){validate_rule='empty';}
