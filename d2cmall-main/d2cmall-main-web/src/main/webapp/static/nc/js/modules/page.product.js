...
$('.product-img .img-thumbs a').click(function(){
	...
	if ($(this).attr('data-video')!=undefined){
		...
		a.play();
		a.addEventListener('ended',function(){$('.video-play-layer').html('').hide();},false)
	}else{
	...
	if (type=='cart'){
		$.getJSON('/member/islogin',function(data){
			if (data.result.login==false){
				...
				if(product_type == 'CROSS'){
					...
					$('.qr-close').on('click', function(){$.popModalClose();});
				}else{
					$.ajax({
		...
		$.getJSON('/member/islogin',function(data){
			if (data.result.login==false){
				...
				$.popModal({'url':'/o2oSubscribe/edit?skuId='+skuid+'&quantity='+num,datatype:'','width':560})
			}
		});	
	}else{
		var data_form = obj.attr('data-form');
