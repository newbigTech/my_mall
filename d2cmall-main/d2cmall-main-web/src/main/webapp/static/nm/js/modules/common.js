...
$(function(){    
								...
								success	:	function(data){
									...
									if (data.result.status==1){
										if (success_tip!='false'){
											...
											success_tip!='' && $.flashTip({position:'center',type:'success',message:success_tip});
										}
										if (call_function){
					...
					$.ajax({
						...
						success	:	function(data){
							...
							if (data.result.status==1){
								...
								}else{$.flashTip({position:'center',type:'error',message:fail_tip});}
	...
	$(document).on('click','.validate-send:not(.disabled)',function(){
	    ...
	    var data = {mobile:account,source:source,type:type,terminal:'PC',nationCode:nation_code,appParams: appParams};		
		$.ajax({
	 ...
	 * 在元素上指定className get-coupon-simple，指定batchId（优惠券组）或id（单个优惠券）
	 */
	$('.get-coupon-simple').css('cursor','pointer');
	$('.get-coupon-simple').on('click',function(){
		var batchid=$(this).attr('data-batchid');
		var id=$(this).attr('data-id');
		if (batchid && id){
			jAlert('配置错误，只能设置一种形式的优惠券，请在前端调整');
			return false;
		}
		var url='';
		if (batchid){
		...
		$.ajax({
			...
			success:function(data){
				if (data.result.status==1){
					...
					if(mobile!=""){$('.use_mobile').val(mobile).attr("readonly","readonly");}
				}
			}		
		});
	}
...
function navigateTo(url){//解析规则
    ...
    if (/product\/[0-9]/.test(data.path)) {//商品详情
		...
		var collage_id = array[array.length-2];
