...
(function($){
	...
	$.basebox={
		mask:function(option){
			...
			$('#'+type+'-ebox-mask').css({'z-index':level,'height':$(document).height(),'filter:':'alpha(opacity='+options.opacity+')','opacity':(options.opacity/100),'-moz-opacity':(options.opacity/100),'background':options.background});
			$('#'+type+'-ebox-outer').css({'z-index':level+1,'width':width+'px','padding':options.padding+'px'});
		...
		loading:function(type,status){
			if (status=='hide') {
				$('#'+type+'-ebox-loading').fadeOut(function(){ $(this).remove();});
			}else{
	...
	$.alerts={
		...
		show: function(title, msg, value, mtype, callback) {
			...
			if (ie6){
				...
				height>=eheight && $(window).scroll(function(){$.basebox.positionNow(type);});
			}
			$(window).resize(function(){$.basebox.positionNow(type)});
			if( $.alerts.draggable ) {
	...
	$.viewbox=function(option){
		...
		$('#view-large').css({'left':($('.view-box-img').width()/2-40)+'px','top':($(window).height()/2-20)+'px'});
		...
		if (options.iframe==true){
			...
			if (options.url) {
				if (options.datatype=='JSON') {
					$.getJSON(options.url+'&callback=?', function(data) {$.basebox.loading(type,'hide');$('#'+type+'-ebox-outer').show();$('#'+type+'-ebox-content').html(data);$.basebox.position(type);ispos=false;});
				}else{
					$.get(options.url, function(data) {$.basebox.loading(type,'hide');$('#'+type+'-ebox-outer').show();$('#'+type+'-ebox-content').html(data);$.basebox.position(type);ispos=false;});
				}
			}
			if (options.content){
