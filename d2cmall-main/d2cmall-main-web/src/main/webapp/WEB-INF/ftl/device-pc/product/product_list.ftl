...
<div class="layout layout-response layout-product lazyload">
    <div class="breadcrumb">
        <#if pager.totalCount gt 0><span class="float-right">共找到 <strong>${(pager.totalCount)!}</strong> 件商品</span></#if>
        <i class="fa fa-caret-right"></i> <a href="/">D2C首页</a>
        <#if (RequestParameters.k!='')> &nbsp;>&nbsp; <span class="search-bar"><input type="text" name="k" value="${RequestParameters.k}" class="input input-s" /><i class="fa fa-search"></i>
        ...
        <#if cate?exists>
	        <#if cate.parent?exists> &nbsp;>&nbsp; <a href="?t=${(topCate.id)!}&c=${cate.parent.id}">${(cate.parent.name)!}</a></#if>
        ...
        <div class="select-condition">
            ...
            <dl class="category">
                <#if topCategorys?exists && topCategorys?size gt 0>
                    ...
                    <dt><i class="fa <#if (RequestParameters.t==top.value.id) && categorys?exists && categorys?size gt 0>fa-angle-down<#else>fa-angle-right</#if>"></i> <a href="?t=${(top.value.id)!}">${(top.value.name)!}</a></dt>
                    <#if (RequestParameters.t==top.value.id) && categorys?exists && categorys?size gt 0>
                       <#list categorys as item>
                    <dd class="category-level-${item.value.depth}<#if RequestParameters.c==item.value.id> on</#if><#if item.value.depth==2 && item.value.parentId!=cateParentId> display-none</#if>"><#if item.value.depth==1><span class="fa<#if item.value.id==cateParentId> fa-minus-square-o<#else> fa-plus-square-o</#if> toggle-category-child"></span> </#if><a href="?t=${(top.value.id)!}&c=${(item.value.id)!}">${(item.value.name)!}</a></dd>
                       </#list>
                    </#if>
                    </#list>
                </#if>
            ...
            <dl class="brand">
                ...
                <dd><div id="product-brand-scrollbar" class="scrollbar brand-section">
                        <div class="scrollbar"><div class="track"><div class="thumb"><div class="end"></div></div></div></div>
                        <div class="viewport">
                             <div class="overview">
                                <#list relationDesigners as relationDs><a href="?d=${(relationDs.value.id)!}${url}"<#if RequestParameters.d==relationDs.value.id> class="on"</#if>>${(relationDs.value.name)!}</a></#list>
                             </div>
                        </div>
                    </div>
                </dd>
                ...
                <dd>
                	<div><a style="padding:5px 10px 5px 20px;display:block;font-size:11px;margin-top:10px" href="javascript:;" id="mode">滚动查看更多</a></div>
                </dd>
                </#if>
            </dl>
            </#if>
    ...
    <div class="layout-right layout-product-right">
        ...
        <div class="list-filter scroll-suspend" data-scroll-end="product-list-bottom-page" data-offset="37">
            <div class="list-sort">
                ...
                <#else>
                <a href="?o=dd${param}" class="on" title="点击按上架最晚时间排序">上架时间 <i class="fa fa-caret-up"></i></a>
                </#if>
                <#else>
                ...
                <a href="?o=sa${param}" class="on" title="点击按销量从少到多排序">销量 <i class="fa fa-caret-down"></i></a>
                <#else>
                ...
                <a href="?o=pa${param}" class="on" title="点击按价格从低到高排序">价格 <i class="fa fa-caret-down"></i></a>
                <#else>
        ...
        <div class="list product-list clearfix">
            ...
            <div class="list-item">
                ...
                <#if product.store?exists && product.store lt 1><i class="sale" ></i><i class="satip">已售罄</i></#if><img src="${static_base}/blank/300-467.gif" data-image="${picture_base}${(product.productImageCover)!}!300" alt="${(product.name)!}" />  </a>
                <p class="title"><#if product.topical==1><strong style="color:#FD555D">[主推]</strong></#if><a href="/product/${product.id}" target="_blank" title="${(product.name)!}">
                ${(product.name)!}</p>
                <p class="price">
                    <span ><span class="symbol">&yen;</span><span class="foreigner-price">${(product.currentPrice)!}</span></span>
                    <#if (product.currentPrice?exists && product.currentPrice lt product.originalPrice)> &nbsp;<s><span class="symbol">&yen;</span><span class="foreigner-price">${(product.originalPrice)!}</span></s></#if>
                </p>
              </a>
            </div>
            </#list>
        ...
        <#else>
            <div style="line-height:560px;width:100%;" class="text-center"><#if searcher.keywords!=''>呜~，没有找到与“${searcher.keywords}”相关的商品哦，小D会加油完善的哟<#else>请稍后，新品陆续在上架中</#if></div>
        </#if>
    </div>
</div>
<script>
