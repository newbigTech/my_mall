...
<div class="layout layout-response layout-user">
    ...
    <div style="width:40px;display:table-cell;background-color:#ededed"></div><div class="layout-user-right">
        ...
        <div class="tab tab-user-menu clearfix">
            <ul>
                <li <#if RequestParameters.refundStatus?exists&&RequestParameters.refundStatus=='APPLY,WAITFORPAYMENT'>class="on" style="position:relative"</#if> ><#if (processCount gt 0)><span class="show-num">${processCount}</span></#if><a href="/member/refund/list?refundStatus=APPLY,WAITFORPAYMENT">处理中</a><i class="interval">|</i></li>
                 <li <#if !RequestParameters.refundStatus?exists>class="on" style="position:relative"</#if> ><a href="/member/refund/list">所有申请</a><i class="interval">|</i></li>
                <#----
        ...
        <div class="search-form order-search-form display-none clearfix">
            ...
            <div class="search-form-item" style="width:25%">
                ...
                <select id="" name="refundStatus" class="input input-s">
                    ...
                    <option value="MEMBERCLOSE" <#if searcher.refundStatus?exists&&searcher.refundStatus[0]=='MEMBERCLOSE'>selected</#if>>用户取消退款</option>
                    <option value="CLOSE" <#if searcher.refundStatus?exists&&searcher.refundStatus[0]=='CLOSE'>selected</#if>>商家拒绝退款</option>
                    <option value="CREATE" <#if searcher.refundStatus?exists&&searcher.refundStatus[0]=='CREATE'>selected</#if>>正在申请退款</option>    
                    <option value="APPLY" <#if searcher.refundStatus?exists&&searcher.refundStatus[0]=='APPLY'>selected</#if>>客服正在确认</option> 
                    <option value="WAITFORPAYMENT" <#if searcher.refundStatus?exists&&searcher.refundStatus[0]=='WAITFORPAYMENT'>selected</#if>>财务正在退款</option> 
                    <option value="SUCCESS" <#if searcher.refundStatus?exists&&searcher.refundStatus[0]=='SUCCESS'>selected</#if>>退款成功</option> 
                </select>
            </div>
        </div>
        </form>
        ...
        <table border="0" cellpadding="6" cellspacing="0" width="100%" class="table table-grey">
            ...
            <#list pager.list as refund>
                ...
                <tr class="item">
            		...
            		<td class="text-left">
            			<p style="float:left;"><img src="${picture_base}/${(refund.sp1?eval.img)!}!80" width="80" alt=""  /></p>
            			<div style="margin-left:90px;line-height:200%;">
        	    			<p style="margin-top:0;">订单编号：<span class="small"><a class="blue" href="/member/order/${refund.orderSn}" target="_blank">${refund.orderSn}</a></span><span style="float:right">【${(refund.orderPayTypeName)!}】</span></p>
            				<p><a href="/product/${(refund.productId)!}" target="_blank">${(refund.productName)!}</a></p>
            				<p class="grey">货号：${(refund.productSn)!}<br />颜色：${(refund.sp1?eval.value)!} &nbsp;&nbsp;&nbsp;尺码: ${(refund.sp2?eval.value)!}</p>
            			</div>
            		</td>
            		<td class="text-left"> 
                    ...
                    <td class="text-center">	
         				<p><a href="javascript:" template-url="/member/refund/${refund.id}" modal-type="pop" modal-width="600" class="ajax-request blue">查看</a></p>
                   		<#if refund.refundStatus==1>
         					<p><a href="javascript:" data-url="/member/refund/cancel/${(refund.id)!}.json" method-type="post" confirm="确定要取消退款申请中吗？" class="ajax-request blue">取消申请</a></p>
         				</#if>	
                    </td>
                </tr>
            </#list>
