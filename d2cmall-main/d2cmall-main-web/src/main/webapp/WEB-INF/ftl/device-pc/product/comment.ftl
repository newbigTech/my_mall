...
<div class="pcomment-item">
	 ...
	 <p><#if comment.content?length gt 0>${comment.content}<#else>未填写评论!</#if></p>
        ...
        <#list comment.pictures as path>
         <a class="hover-img" href="${picture_base}${path}" target="_blank"><img src="${picture_base}${path}!120" height="100%" alt="" /></a>                                                                
        </#list>    
        </div>
        </#if>
	 </div>
	 	...
	 	<div class="head-pic">
	 		<#if comment.headPic=="" || !comment.headPic>
		 		<a href="javascript:;" class="head">
		 			 <img src="//static.d2c.cn/img/home/160627/images/headpic.png" width="30" style="border-radius:50%;"/>
		 		</a>
		 	<#else>
	 ...
	 <div class="replay-item clearfix">
	 	<div class="reply" style="margin-left:16%;margin-top:20px;">
	 		...
	 		<p class="grey" style="margin-top:10px">${commentReply.createDate?string("yyyy.MM.dd")}</p>
	 	</div>
	 </div>
	 <#else>
