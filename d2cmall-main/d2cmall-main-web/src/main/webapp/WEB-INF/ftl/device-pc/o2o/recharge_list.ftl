        ...
        <form id="search-form" name="searcher" method="GET" action="/o2opay/recharge/list">
            <div class="search-form clearfix">
                ...
                <div class="search-form-item" style="width:26%">
                    <label>开始结束</label>
                    <input type="text" name="begin" value="${(RequestParameters.begin)!}" class="input input-s" style="width:45%" /> - <input type="text" name="end" value="${(RequestParameters.end)!}" class="input input-s" style="width:45%" />
                </div>
                <div class="search-form-item" style="width:20%">
                    <label>金额范围</label>
                    <input type="text" name="" value="" class="input input-s" style="width:40%" /> - <input type="text" name="" value="" class="input input-s" style="width:40%" />
                </div>
                <div class="search-form-item" style="width:12%">
                    ...
                    <select id="" name="payChannel" class="input input-s">
                        ...
                        <option value="UNIONPAY"<#if RequestParameters.payChannel=='UNIONPAY'> selected</#if>>银联支付</option>
            ...
            <div class="search-form-button">
                ...
                <button type="button" data-url="/o2opay/recharge/excel" class="button button-red excel-export">导出</button>
            </div>
        </form>
		<div class="pages float-right" style="margin:0">
            <@m.p page=pager.pageNumber totalpage=pager.pageCount num=pager.totalCount />
		</div>
		<table border="0" cellpadding="6" cellspacing="0" width="100%" class="table table-grey" style="margin-top:20px;">
			<tr>
				<th>会员信息</th>
				<th>交易信息</th>
				<th>实充金额</th>
			...
			<tr class="item">
			    ...
			    <td>
			    	...
			    	<p>创建时间：<a href="javascript:;" class="strong">${(recharge.createDate?string("yyyy-MM-dd"))!}</a></p>
			    	<#if recharge.status==-1>
			    		<p>关闭人：<a href="javascript:;" class="strong">${(recharge.closer)!}</a></p>
				    	<p>关闭时间：<a href="javascript:;" class="strong">${(recharge.closeDate?string("yyyy-MM-dd"))!}</a></p>
				    <#elseif recharge.status==1>	
				    	<p>审核人：<a href="javascript:;" class="strong">${(recharge.submitor)!}</a></p>
				    	<p>审核时间：<a href="javascript:;" class="strong">${(recharge.submitDate?string("yyyy-MM-dd"))!}</a></p>
			    	</#if>
			    </td>
			    ...
			    <#if recharge.status==0 >
			   	 	<button type="button" data-url="/o2opay/recharge/close/${recharge.id}" template-id="cancel-template" title="取消充值" class="button button-red ajax-request">取消充值</button>
			    </#if>
			    </td>
			</tr>
			</#list>
