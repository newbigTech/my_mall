...
<style>
	.modal-inner .modal-title{display:none;}
	.modal-inner .modal-content{margin:0;padding:0;color:#000;}
	.modal-outer{border:0;}
	.modal-inner{padding:0;}
	.modal-mask{opacity:.5!important;}
	.qr-box{width:640px;height:400px;text-align:center;}
	.box-title{background:#FD555D;color:#FFF;font-size:16px;font-weight:bold;height:68px;line-height:68px;}
	.box-cont{width:100%;height:332px;padding:75px 115px 0;box-sizing:border-box;display:flex;justify-content:space-between;}
	.box-cont-item .cont-item-tip{padding-top:10px;color:#7F7F7F;font-size:12px;}
	.qr-close{width:30px;height:30px;position:absolute;bottom:-100px;left:50%;transform:translateX(-50%);z-index:80;cursor:pointer;}
</style>
<div class="layout layout-response layout-product clearfix">
    <div class="product-base clearfix">
        <div class="product-img">
            ...
            <#list product.productImageList as image>
         		...
         		<#else>
         		<img src="${picture_base}/${image}!120" data-image="${picture_base}/${image}!media" ori-image="${picture_base}/${image}" /></a>
         		</#if>
             </#list>
             <#if product.video><a href="javascript:" class="video-play" data-video="${picture_base}${product.video}"><span></span><img src="${picture_base}/${image}!120" data-image="${picture_base}/${product.productImageList[0]}!media" ori-image="${picture_base}/${product.productImageList[0]}" /></a></#if>
            </div>
            <div class="img-media" style="display:table;height:790px;">
                <div class="middle-image" style="display:table-cell;vertical-align:middle;">
                	<img src="${static_base}/c/images/space.gif" id="product-image-middle" data-image="" data-id="0" data-picStyle="${product.picStyle}" data-productType="${product.productTradeType}" data-video="<#if product.video>true<#else>false</#if>" alt="" />
                 </div>
                <div class="video-play-layer"></div>
                <#if LOGINMEMBER.distributorId!=null><a id="download-pic" href="javascript:" style="position:absolute;display:block;bottom:0;right:0;background:#000;color:#FFF;padding:6px 10px;">下载图片</a></#if>
            </div>
        </div>
        <div class="product-info product-normal">
            <div class="product-navi">
            	<a id="type-c" style="display:none"  data-type="<#if product.salesproperty&& (product.salesproperty?eval.sp2)?size gt 0 >Hassistant<#else>Nassistant</#if>"> </a>
                ...
                <a target="_blank" href="/product/list?t=${(product.topCategory?eval.id)!}">${(product.topCategory?eval.name)!}</a> &nbsp;>&nbsp;
            ...
            <#if designer.headPic?exists><div class="product-brand"><a href="${base}/showroom/designer/${(designer.id)!}" target="_blank" title="点击打开设计师主页">${(designer.name)!}</a> &nbsp; <i class="fa fa-caret-right"></i></div></#if>
            <div><h1 class="product-title"> 
            	${product.name}</h1>
            	 <br />
	             <#if productPromotion?exists>
	            	<#if flashPromotion?exists && !productPromotion.flashPromotionOver>
	            	【<div class="product-promotion" style="display:inline-block;color:red"> <a href="/flashpromotion/product/${flashPromotion.id}">${flashPromotion.name}</a></div>】
	             	<#elseif !productPromotion.promotionOver>
	           	 	【<div class="product-promotion" style="display:inline-block;color:red"> <a href="/promotion/${productPromotion.promotionId}">${productPromotion.promotionName}</a></div>】
		         	</#if> 
		         </#if> 
	          	<#if (orderPromotions?exists && orderPromotions?size gt 0)>
	          	       满减：
		            <#list orderPromotions as orderPromotion>
		            	【<div class="product-promotion" style="display:inline-block;color:red"><a href="/promotion/${orderPromotion.id}">${orderPromotion.name}</a></div>】
		            </#list>
	            </#if>
            </div>
            ...
            <div class="product-prices clearfix">
                ...
                <span class="product-price-current">&yen; <strong class="current-price">${(productPromotion.currentPrice?string("currency")?substring(1))!}</strong></span>
                &nbsp;
                ...
                <span class="product-price-current">&yen; <strong class="current-price">${(product.minPrice?string("currency")?substring(1))!}</strong></span>
                    &nbsp;
                <span class="product-price-original">吊牌价  <s>&yen;${(product.originalPrice?string("currency")?substring(1))!}</s></span>
                <#elseif (product.minPrice?exists)>
                <span class="product-price-current">&yen; <strong class="current-price">${(product.minPrice?string("currency")?substring(1))!}</strong></span>
                <#else>
                 <span class="product-price-current">&yen; <strong class="current-price">${(product.originalPrice?string("currency")?substring(1))!}</strong></span>
                </#if>
            ...
            <#if productPromotion?exists>
            	...
            	<div class="crowd-count-down">
	                <i class="fa fa-clock-o"></i><span class="count-down"  data-endTime="${productPromotion.flashStartDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}" data-type="split-time" data-musttime="must">
	              距D2C快抢开始&nbsp;<span class="hour down-time">00</span>:<span class="minute down-time">00</span>:<span class="second down-time">00</span>
	                </span>
	            </div>
	             <#else>
		        	<div class="crowd-count-down">
	                <i class="fa fa-clock-o"></i> <span class="count-down" data-endTime="${productPromotion.flashEndDate?string("yyyy/MM/dd HH:mm:ss")}"  data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}" data-type="split-time" data-musttime="must">
	                距D2C快抢结束&nbsp;<span class="hour down-time">00</span>:<span class="minute down-time">00</span>:<span class="second down-time">00</span>
	                </span>
	            </div>
		        </#if>
            	<#elseif !productPromotion.promotionOver>
	            <div class="crowd-count-down">
	                <i class="fa fa-clock-o"></i> <span class="count-down" data-startTime="${productPromotion.startDate?string("yyyy/MM/dd HH:mm:ss")}" data-endTime="${productPromotion.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}"></span>
	            </div>
                </#if>
            </#if>
            <form name="form-buy" id="form-buy" action="/order/buynow" method="post">
                <div class="product-sku">
                    <div class="img clearfix">
                        ...
                        <#list product.salesproperty?eval.sp1 as p>
                        	<a href="javascript:" class="color-section c-${p.id}" data-type="color" data-id="${p.id}" rel="${p.value}" title="${p.value}">
                        	<#if p.img!=''>
                        	<span class="img">
                        		<#if p.img?index_of('http')!=-1>
                        		<img src="${p.img}" data-image="${p.img}" ori-image="${p.img}" alt="" />
                        		<#else>
                        		<img src="${picture_base}/${p.img}!50" data-image="${picture_base}/${p.img}!media" ori-image="${picture_base}/${p.img}" alt="" />
                        		</#if>
                        	</span>
                        	</#if>
                        	<i>已选择</i>
                    ...
                    <div class="txt clearfix sec-font">
                        ...
                        <#list product.salesproperty?eval.sp2 as p>
                            <a href="javascript:" class="size-section s-${p.id}" data-type="size" data-id="${p.id}" rel="${p.value}"><span>${p.value}</span><i>已选择</i></a>
                        </#list>
                        </div>
                    </div>
                     </#if>
                ...
                <div class="product-tip">
                    ...
                    &nbsp;&nbsp;&nbsp;<#if product.after==0><span class="red" style="font-weight:blod;">【本商品不支持七天无理由退换货】</span></#if>
                </div>                
                <#if product.status==3>
                <div class="product-alert">
                    <div>已售罄。留下你的手机号或邮箱，系统会在到货的时候通知您</div>
                    <input type="text" name="mobilemail" value="<#if LOGINMEMBER.id!=null><#if (LOGINMEMBER.mobile)!=null>${LOGINMEMBER.mobile!}<#elseif LOGINMEMBER.email?exists>${LOGINMEMBER.email!}</#if></#if>" placeholder="输入手机号或邮箱" class="input remind-param mobile" />
                ...
                <div class="clearfix" style="margin:3px 10%;position:relative;">
                    ...
                    <button type="button" class="button button-l disabled" style="width:90%;" disabled>已下架</button>
                    <#elseif store lte 0>
                     <button type="button" class="button button-l disabled" style="width:90%;" disabled>已售空</button>
                    <#else>
                    ...
                    <#else>
	                    ...
	                    <button type="button" class="product-operator-button first gobuynow" data-form="top" data-type="buy" style="width:${width};" data-productType="${product.productTradeType}">立即购买</button>
	                    <#if product.cart==1><button type="button" class="gobuynow product-operator-button second" data-form="top" data-type="cart" style="width:${width};" data-productType="${product.productTradeType}">加入购物车</button></#if>
	                    <#if designer.subscribe==1 && product.subscribe == 1>
	                    <button type="button" class="product-operator-button third gobuynow" data-form="top" data-type="fitting" style="width:36%;">免费门店试衣</button>
            ...
            <div class="product-attr">
                <div class="tab product-attr-tab clearfix">
                    <ul>
                        <#if designer.headPic?exists><li data-id="1">设计师|品牌</li></#if>
                    </ul>
                </div>
                <div class="tab-cont product-attr-cont">
                    <#if designer.headPic?exists>
                    <div data-id="1" class="display-none" style="line-height:150%;">
                        <a href="${base}/showroom/designer/${(designer.id)!}" target="_blank" title="点击打开设计师主页" class="designer-logo"><img src="${picture_base}${(designer.headPic)!}!120" alt="${(designer.name)!}" /></a>
                        <#assign designerIntro=designer.intro?replace("<.*?>","","r") />
                        <#if designerIntro?length gt 200>${(designerIntro?substring(0,200))!}...<a  href="${base}/showroom/designer/${(designer.id)!}" target="_blank" class="more-info">更多>></a><#else>${designerIntro}</#if>
                    </div>
                    </#if>
                </div>
                <#if designer.headPic?exists>
                	<div class="designer-detail"> 
                    	<a class="CustomerCare" style="margin-right:20px;cursor:pointer" onclick="ysf.open(),consult_track();"><span style="margin-right:5px"></span>联系客服</a><a class="shop" style="cursor:pointer" href="${base}/showroom/designer/${(designer.id)!}" target="_blank"><span style="background-position:-267px -589px"></span>进入店铺</a>
                    </div>
                </#if>
            </div>
            </#if>
            <div class="product-others">
                ...
                <div class="product-other-operator">
              		  <a href="javascript:" id="collection-${product.id}" data-url="/member/interest/collection/insert/${product.id}" success-tip="收藏成功！" call-back="collectsu" class="ajax-request" style="padding-left: 35px;padding-top: 5px;"> <i class="collection <#if product.collectioned==1>hascollect</#if>"></i>
            ...
            <span>猜你喜欢</span>
 			<#if recommendProducts?exists && recommendProducts?size gt 0>
            	...
            	<div class="relate-item">
	                <p class="relate-item-pic"><a href="${base}/product/${items.id}" target="_blank"><img src="<#if items.productImageCover?index_of('http') != -1>${items.productImageCover}<#else>${picture_base}/${items.productImageCover}!120</#if>" alt="${items.name}" /></a></p>
        ...
        <form action="" method="post">
            <table class="table table-lightgrey">
                <tr class="title text-center">
                    <td width="5"><input type="checkbox" name="checkall" class="check-all" value="" checked class="checkbox" style="margin-top:3px;" /> </td>
                    <td>相关搭配</td>
                    <td width="100">颜色</td>
                    <td width="100">尺码</td>
                    <td width="120">数量</td>
                ...
                <tr class="item">
                    <td class="text-center"><input type="checkbox" name="" value="" checked class="checkbox" /><input type="hidden" name="productId" value="${product.id}" />
                    <input type="hidden" name="skuId" value="" /></td>
                    <td>
                        <p class="img"><a href="${base}/product/${product.id}" target="_blank" title="${product.name}"><img src="${picture_base}/${product.productImageCover}!120" class="pair-img" width="50" alt="${product.name}" /></a><p>
                        ...
                        <p class="title"><a href="${base}/product/${product.id}" target="_blank">${product.name}</a></p>
                    ...
                    <td class="text-center">
                        ...
                        <#if product.salesproperty>
                        <#list product.salesproperty?eval.sp1 as p>
                        <option rel="<#if p.img!=''>${picture_base}/${p.img}!120</#if>" value="${p.id}">${p.value}</option>
                        </#list>
                        </#if>
                        </select>
                    </td>
                    ...
                    <td class="text-center"><input type="hidden"  class="store" value="0" />
                        <em class="data-crease decrease" unselectable="on" onselectstart="return false;">-</em>
                        <input type="text" name="quantity" value="1" size="1" maxlength="3" readonly class="data-num" />
                        <em class="data-crease increase" unselectable="on" onselectstart="return false;">+</em>
                    </td>
                     <td class="text-center sku-price">&yen; <span>${product.minPrice?string("currency")?substring(1)}</span></td>
                </tr>
            </#list>
            </table>
    ...
    <div class="product-cont-bar scroll-suspend" data-offset="50" data-scroll-holder="product-suspend-holder" data-scroll-end="product-hot" data-scroll-bg="product-cont-tab-bar">
        ...
        <div class="product-detail-right">
            <div class="product-info-suspend scroll-suspend" data-offset="70" data-scroll-end="product-hot">
                ...
                <#if productPromotion?exists>
	                ...
	                <div class="crowd-count-down">
	                    <i class="fa fa-clock-o"></i> <span class="count-down" data-type="toend" data-startTime="${productPromotion.flashStartDate?string("yyyy/MM/dd HH:mm:ss")}" data-endTime="${productPromotion.flashEndDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}"></span>
	                </div>
	            	<#elseif !productPromotion.promotionOver>
	                <div class="crowd-count-down">
	                    <i class="fa fa-clock-o"></i> <span class="count-down" data-type="toend" data-startTime="${productPromotion.startDate?string("yyyy/MM/dd HH:mm:ss")}" data-endTime="${productPromotion.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}"></span>
	                </div>
	                </#if>
                </#if>
                <form name="form-buy-small" id="form-buy-small" action="/order/buynow" method="post">
                    <div class="product-sku">
                        ...
                        <#if product.salesproperty>
                            ...
                            <#list product.salesproperty?eval.sp1 as p>
                            	...
                            	<span class="img">
	                        		<#if p.img?index_of('http')!=-1>
	                        		<img src="${p.img}" data-image="${p.img}" ori-image="${p.img}" style="width:50px;height:50px;" alt="" />
	                        		<#else>
	                        		<img src="${picture_base}/${p.img}!50" data-image="${picture_base}/${p.img}!media" ori-image="${picture_base}/${p.img}" style="width:50px;height:50px;" alt="" />
	                        		</#if>
	                        	</span>
                            	</#if>
                            	<i>已选择</i>
                    ...
                    <div class="clearfix" style="margin-top:10px;position:relative;">
                        ...
                        <#assign width='44%' />
	                        ...
	                        <button type="button" class="product-operator-button first" data-form="bottom" data-type="buy" style="width:${width};" data-productType="${product.productTradeType}">立即购买</button>
	                         <#if product.cart==1><button type="button" class="product-operator-button second" data-form="bottom" data-type="cart" style="width:${width};" data-productType="${product.productTradeType}">加入购物车</button></#if>
	                        <#if !(crowdItem?exists)><#if designer.subscribe==1 && product.subscribe == 1>
	                        <button type="button" class="product-operator-button third gobuynow" data-form="bottom" data-type="fitting" style="width:36%;">免费门店试衣</button>
	                        <div class="fit-tip">免费预约新款到D2C实体店，免费试穿体验</div>
                        </#if>
                        </#if>
                        </#if>
    ...
    <div class="product-hot clearfix" id="product-hot">
        ...
        <div>
            <p class="relate-item-pic"><a href="/product/${product.id}" target="_blank"><img src="<#if product.productImageCover?index_of('http')!=-1>${product.productImageCover}<#else>${picture_base}/${product.productImageCover}!280</#if>" alt="${product.name}" /></a></p>
...
<script id="cart-success-template" type="text/html">
    <div class="to-cart-success">
        <h3><a href="javascript:void(0)" onclick="$(this).parent().parent().parent().remove();"><i class="model-close"></i></a></h3>
        <div class="cart-modal-content">
            <p style="font-size:24px;line-height:45px;color:#fd555d;font-weight:900;"><span class="model-success"></span>成功加入购物车</p>
        ...
        <div class="cart-modal-button" style="margin-top:5px">
            <button type="button" class="button button-xl" onclick="continueShop()" style="margin-right:30px">继续购物</button>
