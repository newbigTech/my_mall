...
<div class="layout layout-response layout-showroom lazyload">
    <#if designer.headPic?exists><div class="designer-detail clearfix">
        <div class="info">      
            <h2><strong>设计师/品牌简介</strong></h2>
            <h1>
                <strong class="name">${designer.designers}</strong> &nbsp;&nbsp;
                <button data-url="/member/interest/attention/insert/${designer.id}.json" method-type="post" success-tip="关注成功" call-back="followIncrease(${designer.id})" id="designer-follow-${designer.id}" class="button button-s  ajax-request"><i class="fa <#if designer.attentioned==1>fa-heart<#else>fa-heart-o</#if>"></i><#if designer.attentioned==1>已关注<#else>关注</#if>(<strong>${designer.tfans}</strong>)</button> &nbsp;
                <#if designer.moreIntro && isPreview==null><a href="${base}/showroom/designerintro/${designer.id}" target="_blank" class="button button-s button-purple">更多详情 <i class="fa fa-angle-right"></i></a></#if>
                <#if isPreview><a href="${base}/showroom/designerintro/${designer.id}?isPreview=${isPreview}" target="_blank" class="button button-s button-purple">更多详情预览  <i class="fa fa-angle-right"></i></a></#if>
            </h1>
    ...
    <#if series?size gt 2>
    <div class="tabs-container scroll-suspend clearfix" data-anchor="true" data-offset="37" data-scroll-bg="tabs-holder-bg" data-scroll-holder="tabs-holder">
    ...
    <#list series as serie>
        ...
        <div class="series-product" data-url="/showroom/series/${serie.id}.json" style="min-height:565px;">
            <div style="text-align:center;padding-top:200px;"><i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i></div>
        </div>   
    </#list>
    </div>
    <#else>
    <div class="list designer-product clearfix">
    	<div class="series-product" data-url="/showroom/product/${designerId}.json" style="min-height:565px;" data-type="cola">
            <div style="text-align:center;padding-top:200px;"><i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i></div>
        </div>
     </div>   
    </#if>                      
...
<script id="item-template" type="text/html">
    ...
    {{else if value.productSellType=='CUSTOM'}}	
     ...
     {{/if}}  
        <a href="javascript:" data-url="/member/interest/collection/insert/{{value.id}}" method-type="post" success-tip="false" call-back="favIncrease({{value.id}})" class="favorite ajax-request" id="product-{{value.id}}"><i class="fa {{if value.collectioned==1}}fa-heart {{else}} fa-heart-o{{/if}} "></i> 我喜欢</a>
        <a href="/product/{{value.id}}" class="img" target="_blank">{{if value.store < 1}}<i class="sale" ></i><i class="satip">已售罄</i>{{/if}}<img src="${picture_base}{{value.productImageCover}}!300" alt="{{value.name}}"></a>
