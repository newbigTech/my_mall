	...
	<div class="main-wrap bg-white border-gray">
		...
		<div class="my-cont">
			...
			<div class="order-search-tip">
				我的订单：<a href="/seller/orderitem?index=2">待付款<span class="small">(${waitingForPay})</span></a>  <span class="light-grey">︱</span>
				<a href="/seller/orderitem?index=3">待发货<span class="small">(${waitingForDelivery})</span></a> <span class="light-grey">︱</span>
			...
			<div class="form-search">
				<form id="form-order" name="form_order" method="get" action="/seller/orderitem">
					...
					<div class="label" style="width:390px;">
						<p>宝贝名称 <input type="text" name="productName" value="<#if queryOrder.productName?exists>${queryOrder.productName}</#if>" size="46" class="input"></p>
						<p>成交时间 <input type="text" name="startDate" value="<#if queryOrder.startDate?exists>${queryOrder.startDate?string("yyyy-MM-dd")}<#else></#if>" size="18" class="input" /> 至 <input type="text" name="endDate" value="<#if queryOrder.endDate?exists>${queryOrder.endDate?string("yyyy-MM-dd")}<#else></#if>" size="18" class="input" /></p>					
					</div>
					...
					<div class="float-left text-center"><input type="submit" value="确定" class="button-l b-b" style="margin-top:20px;" /></div>
					<div class="clear"></div>					
				</form>
			</div>
			<#if (orders?size>0)!>
			...
			<table border="0" cellpadding="6" cellspacing="0" width="100%" class="table">
				...
				<tbody>
					...
					<tr class="text-center order-item align-top${orderClass}">
						...
						<td class="border-r-none"><img src="<#if orderItem?exists>${picture_base}/${(orderItem.sp1?eval.img)!}!80</#if>" alt="" width="70" height="70" /></td>
						<td class="text-left border-l-none"><a href="/product/${orderItem.productId}" style="line-height:18px;" target="_blank">${orderItem.productName}</a>
							<span class="display-block grey" style="margin-top:4px;line-height:18px;">${orderItem.sp1?eval.name}：${orderItem.sp1?eval.value}&nbsp;&nbsp;${orderItem.sp1?eval.name}：${orderItem.sp1?eval.value}<br />设计师：${orderItem.designerName}</span>
						</td>
				...
				<#else>
				<tbody>
					<tr><td colspan="7" style="line-height:150px;text-align:center;">暂时无记录</td></tr>
				</tbody>
				</#if>
			</table>
			<#if (orderItems?size > 0)>
