			...
			<div class="interest-list clearfix">
				<ul>
					...
					<li style="width:240px;height:460px;">
						...
						<div style="position:relative"><img src="${picture_base}${(collocation.pic)!}" width="240" />
								...
								<#list collocation.items as item>
									<div class="tag-img-item" id="tag-item-${item.id}" style="top:${item.y*100}%;left:${item.x*100}%">
										...
										<div class="tag-img-edit display-none" id="tag-edit-${item.id}">
											...
											<div class="main">
											<input type="hidden" name="id" class="form-input" value="0" /><input type="hidden" name="collocationId" class="form-input" value="${item.id}" /><input type="hidden" name="x" class="form-input" value="${item.x}" /><input type="hidden" name="y" class="form-input" value="${item.y}" />
											<p>单品标题</p>
...
$('button[name=close-order-btn]').die().live('click',function(){
	...
	jConfirm('确定要提交吗？','',function(r){
		if (r){
			$.ajax({
				'url':url,
				'data':{id:id,feedback:feedback,payAmount:payAmount,payStatus:payStatus,actualNumbers:actualNumbers,retailSn:retailSn},
				dataType: 'json',
				type: "post",
