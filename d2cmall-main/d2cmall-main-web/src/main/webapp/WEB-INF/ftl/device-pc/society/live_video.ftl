...
<@m.page_header title='直播'   keywords="预售,设计师品牌,全球设计师集成平台,SHOWROOM,D2C,Designer To Customer,原创设计,个性,原创设计概念店,设计师品牌集成店"   js='lib/swfobject|utils/jquery.zoom' description="消费者可以通过平台了解设计师的产品理念和创作灵感，树立设计师形象，提升设计师品牌影响力，扩大设计师产品市场" />
<@m.top_nav />
<style>
body{background:#fff}
</style>
...
<script type="text/javascript" language="JavaScript">
    ...
    var s = "";
    if (!fls.f){alert("您的浏览器没有安装flash插件或者flash已被禁用");}
</script>
<div style="width:1000px;margin:0 auto;height:80px;border:1px solid #e5e4e4;font-family: \5FAE\8F6F\96C5\9ED1,arial,sans-serif">
	<div style="float:left;">
			<#if live.headPic>
		      <img src="<#if live.headPic?index_of('http') lt 0>//img.d2c.cn</#if>${live.headPic}" style="width:80px;height:80px" />
	...
	<div style="float:left;max-width:800px">
		...
		<p style="font-size:14px;line-height:16px;padding:6px 0 0 10px;">
			<a href="/showroom/designer/${live.designerId}" style="color:#838c9a;" target="_blank">${live.designerName}>${live.brandName}</a>
		</p>
	...
	<div>
		<p style="margin-top:60px;float:right;font-size:14px;line-height:20px;bottom:5px;color:#2c3e50;padding-right:50px;position:relative;background:url(//static.d2c.cn/common/nc/css/img/view_in_mobile.png) center right no-repeat">
			...
			<img src="/picture/code?type=1&width=120&height=120&noLogo=true&&code=//t.cn/RLstL36" style="position:absolute;top:25px;left:-11px;z-index:2;display:none" class="img-code"/>
		</p>
	</div>
</div>
...
<script>
  ...
  var flashvars = {
            ...
            src: escape('<#if live.status==1>${live.hlsUrl}<#elseif live.status==2>${live.replayUrl}</#if>'),
