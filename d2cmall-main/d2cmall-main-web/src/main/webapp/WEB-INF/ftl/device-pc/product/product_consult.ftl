...
<div class="consult-top"><button class="button-top-consult">发表咨询</button></div>
<#list pager.list as consult>
       <#if consult.status==2>
		<div class="consult-item">
			<div class="d2cuser" style="color:#999;">
				<span>网&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;友：</span><span>${consult.nickName}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="time">${(consult.createDate?string("yyyy-MM-dd HH:mm:ss"))!}</span>
			</div>
			<div class="d2csonsult">
				<span class="consult-content">咨询内容：</span><#if consult.question?length gt 0><span class="consult-content" style="width:75%;">${consult.question}</span></#if>
			</div>
			<#if consult.status==2>
			<div class="d2cans">
				<div><span class="consult-reply">小D 回复：</span><span class="consult-reply"" style="width:75%;">${consult.reply}</span></div><span class="replydate" style="color:#999;">${consult.replyDate?string("yyyy-MM-dd HH:mm:ss")}</span>
			</div></div>
			</#if>
		</div>
		</#if>
</#list>
<div class="consult-publish">购买之前有疑问？请<button class="button-consult" id="button-consult">发表咨询</button></div><div class="pages cousult-pages"><@m.p page=pager.pageNumber totalpage=pager.pageCount /></div>
</div>
<#else>
<div style="padding-top:300px;padding-bottom:90px;text-align:center;overflow:hidden;">
<span>暂时还未有咨询</span>
<div class="consult-publish" style="margin:90px auto;width:250px;float:none;position:static;">购买之前有疑问？请<button class="button-consult" id="button-consult">发表咨询</button></div>
</div>
</#if>
<div class="w-consult" style="display:none;">
	<form id="consult-form" name="consult-form" class="validate-form" action="/consult/insert" method="post"  success-tip="咨询提交成功，请等待回复！">
