...
<div class="form-info n_form text-center">
	<div class="exchange step1_box" style="width:868px;height:70px;text-indent:-9999px;margin:auto;">申请换货</div>
    <form action="/member/exchange" class="validate-form" method="POST"  call-back="myorder">
        ...
        <div class="form-item form-item-vertical"  style="text-align:left;margin-left:18%">
            ...
            <select class="input" name="exchangeReason" id="exchange-reason" style="width:35%">
                ...
                <option value="repair"<#if exchange?exists && exchange.exchangeReason == 'repair'> selected</#if>>商品需要维修</option>
                <option value="damaged"<#if exchange?exists && exchange.exchangeReason == 'damaged'> selected</#if>>收到商品破损</option>
                <option value="wrong"<#if exchange?exists && exchange.exchangeReason == 'wrong'> selected</#if>>商品错发/漏发</option>
                <option value="not"<#if exchange?exists && exchange.exchangeReason == 'not'> selected</#if>>收到商品描述不符</option>
                <option value="quality"<#if exchange?exists && exchange.exchangeReason == 'quality'> selected</#if>>商品质量问题</option>
                <option value="noReason"<#if exchange?exists && exchange.exchangeReason == 'noReason'> selected</#if>>七天无理由退货</option>
            </select>
        ...
        <div class="form-item form-item-vertical"  style="text-align:left;margin-left:18%">
            ...
            <textarea class="input" style="height:80px;width:35%;" id="exchange-memo" name="memo" placeholder="请输入您要换的尺码和颜色"><#if exchange?exists>${(exchange.memo)!}</#if></textarea>
