		...
		<div class="fit-product-list">
		 	<#if o2oSubscribe.items?exists && o2oSubscribe.items?size &gt; 0>
				...
				<div class="o2oSubscribe">
					<a href="/product/${(item.productId)!}" title="${(item.productName)!}" target="_blank"><img src="${picture_base}/${(item.sp1?eval.img)!}!120" alt="" /></a>
...
<div class="subscribe-box">
	<form name="fitting-form" action="/o2oSubscribe/<#if o2oSubscribe.status !=1>create<#else>update</#if>" class="validate-form" method="post" target="<#if RequestParameters.target=='self'>_self<#else>_blank</#if>">
		<div class="subscribe-body">
			<div class="item" <#if o2oSubscribe.status==1>style="display: none;"</#if>>
				...
				<select class="input" id="store-id" name="storeId">
					...
					<option value="${store.id}" data-address="${store.address}" data-tel="${store.tel}" data-map="${store.bdxy}" data-startHour="${store.startHour}" data-startMinute="${store.startMinute}" data-endHour="${store.endHour}" data-endMinute="${store.endMinute}" data-storeService="${store.storeService}">${store.name}
					</option>
						</#list>
					</#if>
				</select>
			...
			<div class="item">
				...
				<label><input type="radio" name="contact" <#if o2oSubscribe.contact==""||o2oSubscribe.contact=="电话">checked</#if> value="电话"/> 电话</label>&nbsp;&nbsp;
				<label><input type="radio" name="contact" <#if o2oSubscribe.contact=="短信">checked</#if> value="短信" /> 短信</label>&nbsp;&nbsp;
				<label><input type="radio" name="contact" <#if o2oSubscribe.contact=="微信">checked</#if> value="微信" /> 微信（记得填写微信号码哦！）</label>&nbsp;&nbsp;
			</div>
			<div class="item">
			...
			<div class="item">
				...
				<input type="text" name="estimateDate"  id="estimateDate"  value="" class="input text-center require" size="10" data-msg="请选定一个日期"  data-time="${o2oSubscribe.estimateDate?string("yyyy-MM-dd")}" />
				<select class="input" name="startHour" id="startHour"  <#if o2oSubscribe.startHour!="">data-time="${o2oSubscribe.startHour}"</#if>>
				</select>
				...
				至
				<select class="input" name="endHour" id="endHour" <#if o2oSubscribe.endHour!="">data-time="${o2oSubscribe.endHour}"</#if>>
				</select>
			...
			<div class="item display-none" id="storeServices">
				<p>我们提供以下<strong style="color:blue">免费服务</strong>，请您选择：<p>
				<p><span></span></p>
			</div>
			<div class="item">
				<p>如果您需求还有其它个性化服务，D2C会尽最大可能地满足您：</p>
				<p><textarea style="width: 100%;" name="optionRequire" rows="4" class="input">${(o2oSubscribe.optionRequire)!}</textarea></p>
				<p style="color:#FF6600;">注:如您需要法式牛奶咖啡、提前预定电影票、预约订餐等！<!--如果您喜欢喝热咖啡，麻烦您填写一下种类，如:炭烧咖啡、法式牛奶咖啡、摩卡咖啡、提前购买电影票等。--></p>
			</div>
		</div>
		<div class="text-center">
			<input type="hidden" name="subscribeId" value="${(o2oSubscribe.id)!}">
			<button type="button" id="subscribe-button" class="button button-green button-l subscribe-button">提交预约</button>
		</div>
	</form>
</div>
