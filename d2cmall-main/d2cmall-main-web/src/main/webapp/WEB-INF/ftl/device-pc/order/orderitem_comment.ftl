...
<@m.top_nav suspend=false />
	<div class="layout layout-response">
		<p style="height:54px;line-height:54px;font-size:14px"><a href="/member/order">我的D2C</a>&nbsp;>&nbsp;<a href="/member/order">我的订单</a>&nbsp;>&nbsp;<a href="javascript:;">订单：${orderItem.orderSn}</a></p>
	</div>
	<div class="layout layout-response layout-order" style="margin-top:0">
        ...
        <p style="padding:5px 0;color:#999">订单号：<span style="color:#444">${orderItem.orderSn}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ${orderItem.createDate?string("yyyy.MM.dd HH:mm")!""}</p>
	...
	<div style="float:left;width:33%;text-align:center;">
		<div style="margin-top:60px;border-right:1px solid #CCC;">
			...
			<p class="title" style="width:210px;margin:0 auto;padding-top:10px;line-height:130%;color:#999;height:45px;overflow:hidden">${orderItem.productName}</p>
			<p style="width:210px;margin:0 auto;padding-top:8px;line-height:130%;color:#999">颜色:${orderItem.sp1Value} 尺码:${orderItem.sp2Value}</p>
			<p style="width:210px;margin:0 auto;padding-top:8px;"><strong>&yen;&nbsp;${orderItem.productPrice}</strong></p>
		</div>
	</div>
	<div class="form" style="float:left;width:67%;margin-top:5px;">
	    <form method="post" class="validate-form form-cinfo" action="${action}.json" style="margin-top:0;border:none" call-back="returnorder">
	        ...
	        <input type="hidden" name="productId" value="${comment.productId}"/>
	         ...
	         <div style="float:left;width:80%;text-align:left;padding-top:19px;">
		        <div class="form-item">
		            <textarea style="height:130px;width:90%" name="content" onKeyDown="checkLength()" onKeyUp="checkLength()" onPaste="checkLength()" class="input">${(comment.content)!}</textarea>
		            <div style="padding-top:20px;">
		                <a href="javascript:void(0)" style="display:inline-block;">
		                    <input type="file" class="upload-file upload-pic" name="file" data-upload-url="/picture/upload" accept="image/gif,image/png,image/jpeg" />
		                     <span style="display:inline-block;margin-left:10px;" class="text-upload">添加图片</span>
		                </a>
		                <div class="upload-preview<#if comment.pictures?exists> display-block<#else> display-none</#if>" style="margin-left:0">
		                    <#if comment.pictures?exists> 
		                    <#list comment.pictures as path>
		                        <div class="upload-img">
		                            <a href="${picture_base}/${path}" class="img" target="_blank"><img src="${picture_base}/${path}" height="100" alt="" /></a>
	 ...
	 <div class="form-button checkbox" style="margin-top:20px;">
	      <button type="button" class="button button-red button-l subbuton" style="font-size:18px">提交</button>
	        <label style="margin-left:20px;color:#999"><input type="checkbox" id="anonymous"  value="1"  checked /><label for="anonymous"> </label>匿名评价</label>
	  </div>
</div>
</div>
<script>
