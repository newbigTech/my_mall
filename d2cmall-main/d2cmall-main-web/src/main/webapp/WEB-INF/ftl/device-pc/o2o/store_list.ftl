...
<div class="layout layout-response layout-store lazyload">
    ...
    <#list list as store>
        ...
        <div class="list-item" data-name="${(store.name)!}" data-address="${(store.address)!}" data-tel="${(store.tel)!}" data-lant="${map_arr[0]}" data-long="${map_arr[1]}">
            <a href="javascript:" class="img"><img alt="" border="0" src="/static/blank/500-288.gif" data-image="${picture_base}${(store.pic)!}!600" /></a>
    ...
    <div class="store-map clearfix">
        <div class="store-map-list">
            <ul>
            <#list list as store>
                <li data-id="${store_index}" data-name="${(store.name)!}"><strong>${store.name}</strong><br />${store.address}<br /><i class="fa fa-phone"></i> ${store.tel}</li>
            </#list>
            </ul>
        </div>
...
{'id':'${store.id}','name':'${store.name}','address':'${store.address}','tel':'${store.tel}','lant':'${map_arr[0]}','long':'${map_arr[1]}','img':'${picture_base}${(store.pic)!}!280'}${period}
</#list>
];
