...
<div class="layout layout-response layout-user">
	...
	<div class="form-info n_form text-center">
		<div class="refund step1_box" style="width:667px;height:52px;text-indent:-9999px;margin:auto;">申请退款</div>
	    <form action="/member/refund" class="validate-form" method="POST" call-back="myorder">
	        ...
	        <div class="form-item  form-item-vertical" style="text-align:left;margin-left:18%">
	            ...
	            <select class="input" name="refundReason" id="refund-reason" style="width:35%">
	                ...
	                <option value="wrong"<#if refund?exists && refund.refundReason == 'wrong'> selected</#if>>拍错了</option>
	                <option value="no"<#if refund?exists && refund.refundReason == 'no'> selected</#if>>不想要了</option>
	                <option value="stock"<#if refund?exists && refund.refundReason == 'stock'> selected</#if>>商品缺货</option>
	                <option value="error"<#if refund?exists && refund.refundReason == 'error'> selected</#if>>订单信息错误</option>
	                <option value="consensus"<#if refund?exists && refund.refundReason == 'consensus'> selected</#if>>协商一致退款</option>
	            </select>
	            <div class="tip tip-validate" data-target="refund-reason"></div>
	        </div>
	        <#if refund.orderPayType==3>
	        <div style="padding:10px;color:#FD555D;display:none"><i class="fa fa-info-circle"></i>该笔交易退款无法原路返回，退款需要汇到您的支付宝账户</div>
	        <div class="form-item form-item-vertical" style="text-align:left;margin-left:18%">
	            <label><span class="red" style="font-size: 16px;vertical-align: middle;">*</span>支付宝账号</label>
	            <input class="input" name="backAccountSn" id="back-account" value="${(refund.backAccountSn)!}" placeholder="目前退款仅支持退到支付宝账号" size="40"  style="width:35%"/>
	            <div class="tip tip-validate" data-target="back-account"></div>
	        </div>
	        <div class="form-item form-item-vertical" style="text-align:left;margin-left:18%">
	            <label><span class="red" style="font-size: 16px;vertical-align: middle;">*</span>支付宝(实名)</label>
	            <input class="input" name="backAccountName" id="backaccount-name" value="${(refund.backAccountName)!}" placeholder="支付宝实名认证" size="40" style="width:35%" />
	            <div class="tip tip-validate" data-target="backaccount-name"></div>
	        </div>
	        <#else>
	        <div style="padding:10px;color:#FD555D;text-align:left;margin-left:18%"><i class="fa fa-info-circle"></i>该笔交易退款原路返回${(refund.orderPayTypeName)!}</div>
	        </#if>
	        
	        <div class="form-item form-item-vertical" style="text-align:left;margin-left:18%">
	            <label><span class="red" style="font-size: 16px;vertical-align: middle;">*</span>退款金额</label>
	            <input type="text" class="input" name="applyAmount"  id="total-amount" data-limit="${orderItem.actualAmount}" style="width:100px" value="${orderItem.actualAmount}" /> 元(可退 ${orderItem.actualAmount} 元)
	            <div class="tip tip-validate" data-target="total-amount" data-function="checkAmount"></div>
	        </div>
	       
	        <div class="form-item form-item-vertical" style="text-align:left;margin-left:18%">
	            <label><span></span>退款说明</label>
	            <textarea class="input" style="height:60px;width:35%" name="memo"><#if refund?exists>${refund.memo}</#if></textarea>
	        </div>
