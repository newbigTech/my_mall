<form name="award-form" class="validate-form" action="/award/create" call-back="createSuccess()" method="post">
    ...
    <div id="address-list" class="select-address address-list clearfix" name="address-list">
        ...
        <div class="address-item address-item-vertical<#if address.isdefault> on</#if>" data-id="${address.id}">
            <span class="fa fa-map-marker"></span> <strong>${(address.name)!} / ${address.mobile}</strong> ${(address.province.name)!}${(address.city.name)!}${(address.district.name)!}${address.street}
    ...
    <div id="address-add" class="form<#if addresses?size gt 0> display-none</#if>">
        ...
        <div class="form-item">
            <label>街道地址</label>
            <textarea name="street" cols="45" rows="2" id="street" class="input" minlength="5" maxlength="60"${input_disabled}></textarea>
            <div class="tip<#if addresses?size  == 0> tip-validate</#if>" data-target="street"></div>
        </div>
        <div class="form-item">
            <label>手机号码</label>
            <input type="text" name="mobile" id="mobile" maxlength="20" value="" size="30" class="input"${input_disabled}   onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
            <div class="tip<#if addresses?size  == 0> tip-validate</#if>" data-target="mobile" data-rule="mobile"></div>
        </div>
        <div class="form-item">
            <label>电话号码</label>
            <input type="text" name="phonePrefix" id="phonePrefix" maxlength="4" value="" style="width:10%;" class="input utilSetNumber" placeholder="区号"${input_disabled} /> - <input type="text" name="phoneMiddle" id="phoneMiddle" maxlength="8" value=""${input_disabled}  style="width:20%;" class="input utilSetNumber" placeholder="电话号码" /> - <input type="text" maxlength="5" name="phoneSuffix" value=""${input_disabled} style="width:15%;" class="input" placeholder="分机" />
        </div>
    </div>
