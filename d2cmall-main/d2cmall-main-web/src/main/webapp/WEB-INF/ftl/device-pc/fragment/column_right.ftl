			...
			<div class="shadow-10 column-cont">
				...
				<div class="tab-cont">
					<div class="tab-cont-data" data-id="notice">
						<div class="news-h1">
						<#if article?exists>
							<a href="/news/${(article.id)!}" target="_blank"><img src="${(article.picture)!}" width="300"  alt="${(article.title)!}" /></a>
							<p style="margin-top:10px;"><a href="/news/${article.id}" target="_blank">${(article.title)!}</a></p>
						</#if>
						</div>
						<div class="news-list">
