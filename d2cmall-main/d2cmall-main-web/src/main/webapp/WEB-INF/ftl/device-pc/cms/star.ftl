    ...
    <div class="star-slide-screen" id="top-slide">
        ...
        <#if homePage.blocks.block04?size gt 0>
            ...
            <li data-title="${(block.title)!}" <#if block.margin!=''> style="background:${(block.margin)!}"</#if>>
                <a href="${(block.bindUrl)!}" target="_blank"><img src="${picture_base}${(block.midPic)!}" alt="${(block.title)!}" /></a>
            </li>
            </#list>
        </#if>
        </ul>
...
<div class="layout-response" id="waterfall">
    ...
    <div class="item">
        <a href="${(block.bindUrl)!}" target="_blank">
            <img src="${picture_base}${(block.frontPic)!}!/fw/250/strip/true" class="img" alt="${(block.title)!}" width="247" />
...
<div id="load-more" class="clearfix w-1200 load-more" data-page="${pager.pageNumber}" data-total="${pager.pageCount}"><i></i></div>	
</#if>
</#if>
</#if>
<#if pageDefine.fieldDefs.block08.status==1>
    ...
    <#list homePage.blocks.block08 as block>
        <div class="float-left"<#if block.margin!=''> style="margin:${(block.margin)!}"</#if>>
            <a href="${(block.bindUrl)!}" target="_blank"><img width="${(block.width)!}" height="${(block.height)!}" alt="${(block.title)!}" src="/static/c/images/space.gif" data-image="${picture_base}${(block.frontPic)!}!/fw/250/strip/true" /></a>
        </div>
    </#list>
    </#if>
    </div>
...
<script id="list-template" type="text/html">
    ...
    <div class="item {{id}}">
        <a href="{{value.bindUrl}}" target="_blank">
            <img src="${picture_base}{{value.frontPic}}!/fw/250/strip/true" class="img" alt="{{value.title}}" width="247" />
            <span class="bottom text-center">
            {{value.title}}
            </span>
        </a>
