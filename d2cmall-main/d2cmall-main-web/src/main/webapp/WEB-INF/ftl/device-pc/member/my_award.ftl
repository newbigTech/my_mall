...
<div class="layout layout-response layout-user">
    ...
    <div style="width:40px;display:table-cell;background-color:#ededed"></div><div class="layout-user-right">
        ...
        <div class="tab tab-user-menu clearfix">
            <ul>
             	<li<#if status==0> class="on"</#if>><a href="/award/my/list?status=0">全部 </a><i class="interval">|</i></li>
                <li<#if status==1> class="on"</#if>><a href="/award/my/list?status=1">进行中 </a><i class="interval">|</i></li>
                <li<#if status==2> class="on"</#if>><a href="/award/my/list?status=2">已揭晓 </a><i class="interval">|</i></li>
            </ul>        
        </div>
			...
			<tr>
		 	...
		 	<#if pager.list?exists && pager.list?size &gt; 0>
				...
				<tr class="item text-center">
					...
					<td class="border-r-none">
						<a href="/award/${list.awardId}" style="line-height:18px;" target="_blank">${list.awardTitle}</a>						
					</td>
					<td class="border-r-none"><a href="/award/${list.awardId}"><img src="${picture_base}/${list.productImg}!80" alt="" height="70" /></a></td>										
					<#--  <td>&yen; ${list.price?string('0.00')}</td>-->
					<td style="line-height:150%;">${list.award.begin?string('yy/MM/dd HH:mm:ss')}<br />至<br />${list.award.end?string('yy/MM/dd HH:mm:ss')}</td>
					<td>
