...
<div class="account-body">
	...
	<div class="account-count">
	    ...
	    <div class="account-wrap">
	        <form name="form-register" id="form-register" action="/member/register" method="post">
	            <input type="hidden" name="" value="中国大陆" class="country-code" />
	            <input type="hidden" name="nationCode" value="86" class="mobile-code" />
	            <div class="account-title reg-title"></div>
	            <div class="account-bar text-right"><a href="/member/login" style="color:#FD555D">已有账号</a></div>
	            <div class="account-input">
	                  <span class="choose-country" id="choose-country"><strong id="mobile-code">+86</strong><em class="fa fa-caret-down"></em></span>
                <span class="input-tip" style="left:26%;">请输入手机号</span>
	                <input type="text" name="loginCode" id="logincode" data-rule="mobile" class="validate-account" value="" style="width:70%" />
	            </div>
	            ...
	            <div class="account-input" style="width:65%">
	                ...
	                <button type="button" data-source="" data-type="MEMBERMOBILE" class="button button-grey validate-send pn">获取验证码</button>
	            </div>
	            <div class="msg" id="errmsg"><#if account_error?exists>${account_error}</#if></div>
	            <div class="account-btn">
	                <input type="hidden" name="path" value="${path}" />
	                <input type="button" value="立即注册" id="reg-btn" class="input-sub" />
	            </div>
	            <div class="service-dtail checkbox" >
	            	<input type="checkbox" id="checkbox" checked/><label for="checkbox"></label><span style="font-size:14px;">立即注册代表同意<a href="/page/regagreement" target="_blank">《D2C网络服务使用协议》</a></span>
	            </div>			
	...
	<div class="copyright clearfix">
            	<div class="copyright-info">
			        ...
			        <p style="text-align:right;"><a target="_blank" href="//www.beian.gov.cn/portal/registerSystemInfo?recordcode=33010602004682" style="display:inline-block;text-decoration:none;padding-left:5px;color:#999;"><img src="//static.d2cmall.com/img/home/160627/images/police.png" style="float:left;"/>浙公网安备 33010602004682号</a></p>
            	</div>
        </div>
</div>
<#if nofloat?length == 0>
    ...
    if (obj.val()!=''){
         $.post('/member/checkLoginCode?'+new Date().getTime(),{'code':obj.val()},function(data){
            if (data.result.status==-1){
                $('.validate-send').attr('disabled',true);
                obj.parent().next('div').show().html(data.result.message);
                return false;
            }else{
                $('.validate-send').removeAttr('disabled');
...
$('#reg-btn').click(function(){
	if ($('#form-register input[name=loginCode]').val()==''){
		$('#errmsg').html('请输入手机号');
		$('#form-register input[name=loginCode]').focus();
		$('#errmsg').show();
		return false;
	}
	if ($('#form-register input[name=loginCode]').val().length>20){
		$('#errmsg').html('手机号码格式不正确');
		$('#form-register input[name=loginCode]').focus();
		$('#errmsg').show();
		return false;
	}
	if ($('#form-register input[name=code]').val()==''){
        $('#errmsg').html('请输入短信校验码');
        $('#form-register input[name=code]').focus();
        $('#errmsg').show();
        return false;
    }
	...
	if(!$('#checkbox').is(':checked')) {
   		$('#errmsg').html('请同意D2C网络服务协议');
   		$('#errmsg').show();
		return false;
	}
	var account=$('#form-register input[name=loginCode]');
	var button=$('#form-register input[type=submit]');
	...
	if (!account.utilValidateMobile()){
        ...
        $.post('/member/checkLoginCode?'+new Date().getTime(),{'code':account.val()},function(data){
            if (data.result.status==-1){
                $('.msg').html(data.result.message).show();
                account.focus();
                return false;
            }else{
                $.post($('#form-register').attr('action'),$('#form-register').serialize(),function(res){
                    if (res.result.status==-1){
                        $('#errmsg').html(res.result.message).show();
                        return false;
                    }else{
                        location.href=res.result.datas.redirect;
                    }
                },'JSON');
            }
        },'JSON');	
