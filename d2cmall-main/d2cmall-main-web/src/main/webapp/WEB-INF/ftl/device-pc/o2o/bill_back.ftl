<div class="form form-small">
    <form method="post" confirm="确定要退款吗？" class="validate-form" action="/o2opay/back">
        ...
        <div class="form-item">
            <label>零售单号</label>
            <input type="text" id="retail-sn" name="retailSn" value="${(oldPayment.transactionInfo)!}" placeholder="零售单号请务必填写正确" class="input" readonly />
            <div class="tip tip-validate" data-target="retail-sn"></div>
        </div>
        <div class="form-item">
            <label>退款金额</label>
            <input type="text" id="back-amount" name="backAmount" value="${(oldPayment.subTotal-oldPayment.backAmount)!}" class="input number" style="width:100px;" /> 可退款：<span style="font-size:14px;font-family:'Arial'">&yen; ${(oldPayment.subTotal-oldPayment.backAmount)!}</span>
