...
<div class="layout layout-response layout-help lazyload">
    ...
    <div class="password step2_box" style="width:667px;height:52px;text-indent:-9999px">更改密码</div>
        <form id="send-form" style="margin-top:100px" class="validate-form" action="/member/resetPassword" call-back="modifySave()" success-tip="false" method="post">
            <input type="hidden" name="loginCode" class="validate-account" value="${loginCode}" data-rule="mobile" />
            <div class="form-item">
                <label>输入新密码</label>
                <input type="password" name="password" id="password" placeholder="请输入8-20位密码" value="" class="input input-l" style="width:60%"/>
                <div class="tip tip-validate" data-target="password" data-rule="sixDigit"></div>
            </div>
            <div class="form-item">
                <label>确认新密码</label>
                <input type="password" name="repassword" id="confirm-password" placeholder="确认上面输入的密码" value="" class="input input-l" style="width:60%" />
                <div class="tip tip-validate" data-target="confirm-password" compare-with="password" data-rule="sixDigit"></div>
            </div>
            <div class="form-item">
                <label>输入验证码</label>
                <input type="text" name="code" id="validate-code" size="18" title="验证码" placeholder="输入收到的短信验证码" class="input input-l" style="width:43%;" value="" style="width:40%"/>
               	<input type="hidden" name="nationCode" class="mobile-code" value="86"/>
                <button type="button" data-source="" data-type="RETRIEVEPASSWORD" class="button button-l button-gray validate-send nb" >获取验证码</button>
