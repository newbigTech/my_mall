...
<div class="layout layout-response layout-user">
    ...
    <div style="width:40px;display:table-cell;background-color:#ededed"></div><div class="layout-user-right">
        ...
        <form name="" action="" method="get" style="margin-bottom:20px;">
            <input name="productSn" value="${(RequestParameters.productSn)!}" size="40" class="input" placeholder="输入款号进行搜索" /> <button type="submit" name="" class="button button-green">搜索</button>
        </form>
            ...
            <#assign rowspan=setting.product.restSalesproperty.sp1?size />
	            ...
	            <tr class="item text-center">
	                ...
	                <td rowspan="${rowspan}" style="text-align:left;line-height:200%;vertical-align:top">
	                    <a href="/product/${setting.product.id}" target="_blank" class="float-left" style="margin-right:10px;"><img src="${picture_base}${(setting.product.productImageCover)!}!180" width="120" height="187" alt="${setting.product.name}" /></a>
	                    <p style="height:140px;">${(setting.product.name)!}</p>
	                    <p>款号：${(setting.product.inernalSn)!}</p>
	                    <p>品牌：${(setting.product.designerName)!}</p>
	                </td>
	                <td rowspan="${rowspan}">&yen; ${(setting.product.originalPrice?string(',000.00'))!}</td>
...
<script id="cart-success-template" type="text/html">
    <div class="to-cart-success">
        <h3><a href="javascript:void(0)" onclick="$(this).parent().parent().parent().remove();"><i class="fa fa-close"></i></a>购物车提示</h3>
        <div class="cart-modal-content">
            <p>你选择的商品成功放入购物车</p>
        </div>
        <div class="cart-modal-button">
            <button type="button" class="button button-s button-green" onclick="$(this).parent().parent().parent().remove();">继续购物</button>
