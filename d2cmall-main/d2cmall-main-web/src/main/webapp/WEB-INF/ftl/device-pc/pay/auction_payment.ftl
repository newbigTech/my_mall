...
<div class="layout layout-response layout-order">
    <form name="pay-form" id="pay-form" action="/payment" method="post" style="padding:15px;">
        ...
        <div class="pay-order-detail clearfix" style="text-align:center">
        	...
        	<p class="grey" style="margin-top:8px;">应付金额：<span class="display-none" id="total-price">${(auction.billTotalFee)}</span><span class="red" style="font-size:24px;font-weight:bold;" id="pay-price">${(auction.billTotalFee)?string("currency")?substring(1)}</span></p>
                    ...
                    <ul class="alipay clearfix">
                        ...
                        <li class="pay-li"><input type="radio" id="WXSCANPAY" data-type="WX_SCANPAY" name="payment" method="creditPay" value="WX_SCANPAY" /><label  class="WXSCANPAY">微信扫码支付<i class="select"></i></label></li>		               
		                 <#if account?exists && account.status==1 && account.totalAmount &gt;0>
			                <#if !account.password>
			                <li class="pay-li">
			                    <a href="/member/account/info" target="_blank" class="button button-s button-red" style="position:absolute;left:340px;top:15px;">设置支付密码</a>
			                </li>
			                <#else>
				                <#if auction.billTotalFee gt 0>
				                <li class="pay-li" data-type="wallet">
				                    <input type="radio" id="WALLET" name="type" value="WALLET" />
				                    <label class="WALLET" data-type="WALLET"<#if account.availableTotalAmount lt (auction.billTotalFee)> disabled</#if>> D2C钱包<i class="select"></i></label>
				                </li>
				                <div class="wallet-password" style="display:none;float:left;" <#if account.availableTotalAmount lt (auction.billTotalFee)> data-money="available"</#if>>
	
					                    <ul style="float:left;">
					                        <li class="pay-title" style="margin:0;margin-top:5px;">输入支付密码</li>
					                        <li style="float:left;margin:0 15px 0 0;"><input type="password" id="before-password" class="input" size="10" /><input type="hidden" name="password" id="pay-password" /></li>
					                    </ul>
					                    <div style="margin-top:20px;float:left;">        
					                    	钱包可用余额：<strong class="arial" style="color:#FF3300;font-size:14px;">&yen; ${account.availableTotalAmount?string("currency")?substring(1)}</strong>
        ...
        <div class="text-center" style="margin-top:20px;">
            <button type="button" name="order-pay" id="order-pay" data-url="/auction/member/mymargin" class="button button-red button-xl finalpay" style="padding:0 40px"> 去付款 </button>
        </div>  
    </form>
</div>
