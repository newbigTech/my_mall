		 	...
		 	<#if pager.list?exists && pager.list?size &gt; 0>
				<#list pager.list as list>
				 <tr class="item" style="height:46px">	
					<td><a href="/auction/product/${list.auctionId}" style="color:#fd555d">${list.auctionTitle}</a></td>
					...
					<td><#if list.status==8 || list.status == 10><span class="display-block"><a href="//www.kuaidi100.com/chaxun?com=${list.deliveryCorpCode}&nu=${list.deliverySn}" class="strong blue" target="_blank">物流查询</a></span></#if></td>
				    <td><#if list.status==8><button type="button" data-url="/auction/received/${list.id}" class="button button-s button-red  ajax-request" method-type="post" confirm="是否确认收货？">确认收货</button></#if></td>
				 </tr>
		        </#list>
			</#if>
		</table>
