        ...
        <form id="search-form" name="searcher" method="GET" action="/o2opay/bill">
            <div class="search-form clearfix">
                ...
                <div class="search-form-item" style="width:26%">
                    <label>开始结束</label>
                    <input type="text" name="startDate" value="${(RequestParameters.startDate)!}" class="input input-s" style="width:45%" /> - <input type="text" name="endDate" value="${(RequestParameters.endDate)!}" class="input input-s" style="width:45%" />
                </div>
        ...
        <table border="0" cellpadding="6" cellspacing="0" width="100%" class="table table-grey">
            ...
            <tr class="item text-center">
                ...
                <#if bill.status==1  && bill.payType=='PAY' && bill.canBack==1>
                    <button type="button" template-url="/o2opay/back/${bill.id}" modal-type="pop" class="button button-red ajax-request">退款</button>
                <#elseif bill.status==0  && bill.payType=='PAY'>
                    <button type="button" data-url="/o2opay/receive/close/${bill.id}" template-id="cancel-template" data-param="{'text':'取消收款'}" class="button button-red ajax-request">取消收款</button>
                <#elseif bill.status==0  && bill.payType=='REFUND'>
                    <button type="button" data-url="/o2opay/receive/close/${bill.id}" template-id="cancel-template" data-param="{'text':'取消退款'}" class="button button-red ajax-request">取消退款</button>
                </#if>
                </td>
            </tr>
            </#list>
