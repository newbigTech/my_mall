...
<style>
body {background:#f9f9f9;width:350px;min-width:350px;padding:5px;}
h2 {font-size:14px;;margin:0;padding:10px 0;}
table {border-collapse: collapse;width:100%;border-spacing: 0;}
table td {padding:5px 8px;font-family:Arial;font-size:12px;}
table td .title {margin-bottom:7px;height:29px;overflow:hidden;line-height:130%;}
table td .grey {margin-bottom:7px;}
.tdbg td {background:#f6fdff}
.current-price {font-size:16px;color:#FF6600;}
</style>
<script>
...
<table>
    <tr>
        <td class="text-center"><a href="//www.d2cmall.com/product/${result.datas.product.id}" target="_blank"><img src="${picture_base}${result.datas.product.productImageList[0]}!300" width="250" /></a></td>
    </tr>
    <tr>
        <td>
            <p><a href="//www.d2cmall.com/product/${result.datas.product.id}" target="_blank">${result.datas.product.name}</a></p>
            <p>状态：
            ...
            <p>
        	    售价：<strong class="current-price">&yen; <#if result.datas.product.salePrice lte 0>????.??<#else>${(result.datas.product.salePrice?string("currency")?substring(1))!}</#if></strong>&nbsp;&nbsp;&nbsp; 
    ...
    <tr>
        <td colspan="2">
            <p>颜色：<#if result.datas.product.salesproperty><#list result.datas.product.salesproperty?eval.sp1 as p>${p.value}&nbsp;&nbsp;</#list></#if></p>
            <p>尺码：<#if result.datas.product.salesproperty><#list result.datas.product.salesproperty?eval.sp2 as p>${p.value}&nbsp;&nbsp;</#list></#if></p>
        </td>
    </tr>
