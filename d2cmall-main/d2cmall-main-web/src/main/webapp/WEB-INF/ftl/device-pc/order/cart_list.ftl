...
<div class="layout layout-response layout-cart clearfix">
    ...
    <form name="form-cart" id="form-cart" action="/order/confirm" method="POST"  style="padding:15px;">
        ...
        <table class="table table-lightgrey" style="margin-top:10px;">
            <thead>
                <tr>
                    <th style="text-align:left;border-right:none;" colspan="2" class="checkbox"><input type="checkbox" name="checkall" checked class="checkbox check-all" id="checkbox"/><label for="checkbox"> </label>全选&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;宝贝信息</th>
            ...
            <#list cart?if_exists.itemMapById?if_exists?values as cartItem>
                ...
                <tr class="item" order-id="${cartItem.id}">
                    <td class="text-center border-r-none checkbox">
                        <input type="checkbox" name="cartItemId" value="${cartItem.id}" id="${cartItem.id}" <#if (cartItem.productSku.availableStore gt 0) && (!cartItem.over)> checked<#else> disabled</#if> class="left" /><label for="${cartItem.id}"></label>
                    </td>
                    <td class="text-left border-l-none border-r-none">
                        <input type="hidden" name="skuId" value="${cartItem.productSkuId}" />
                        <p class="promotion-name"><span class="one"></span><span class="three"></span><span class="two"></span></p>						
                        <p class="img"><a href="/product/${cartItem.productId}" target="_blank"><img src="${picture_base}/${cartItem.sp1?eval.img}!80" alt="${cartItem.productName}" height="90" /></a></p>
                        <p><a href="/product/${cartItem.productId}" target="_blank"><#if cartItem.flashPromotion?exists && !cartItem.productPromotion.flashPromotionOver>
                <span style="color:#FD555D">【限时购】</span></#if>${cartItem.productName} </a>
                        ...
                        <p class="grey">${cartItem.sp1?eval.name}：${cartItem.sp1?eval.value} &nbsp;&nbsp;&nbsp; <#if cartItem.sp2>${cartItem.sp2?eval.name}：${cartItem.sp2?eval.value} &nbsp;&nbsp;&nbsp;</#if> 设计师：${cartItem.designerName} &nbsp;&nbsp; <#if cartItem.after==0><span class="red">本商品不支持七天无理由退换货</span></#if> </p>
                    </td>
                    <td class="text-left border-l-none border-r-none">
	                    <input type="hidden" name="goodPromotionId" value="${cartItem.goodPromotionId}" /></span><br />
	                    <input type="hidden" name="orderPromotionId" value="${cartItem.orderPromotionId}" /></span>
	                    <#if cartItem.choiceOrderPromotions?exists && cartItem.choiceOrderPromotions?size gt 1>
		                    <select name="selectOrderPromotionId" class="input-s" style="width:100%">
		                        ...
		                        <option value="${orderPromotion.id}" <#if cartItem.orderPromotionId==orderPromotion.id>selected="selected"</#if>>${orderPromotion.name}</option>
		                        </#list>
		                    </select>
	                    </#if>
                    </td>
                    <td class="text-center border-l-none border-r-none" > 
                        <p class="unit price original-l" data-price="${cartItem.originalPrice}">&yen;${cartItem.originalPrice?string("currency")?substring(1)}</p>
                        <p><strong class="unit price cut-price"></strong></p>
                    </td>
                    <td class="text-center border-l-none border-r-none">
                        <#if cartItem.over>
                        <span class="num-change">-</span>
                        <input type="text" name="quantity" value="0" size="2" class="input disabled text-center noime" />
                        <span class="num-change">+</span>
                        <div class="tip">该件商品已经下架</div>
                        <#elseif (cartItem.productSku.availableStore lte 0 && cartItem.productSku.freezeStore gt 0)>
                        <span class="num-change">-</span>
                        <input type="text" name="quantity" value="${cartItem.productSku.availableStore}" size="2" maxlength="3" class="input disabled text-center noime" readonly="readonly"/>
                        <span class="num-change">+</span>
                        ...
                        <a class="surplus" data-rule="${cartItem.productSku.availableStore}" style="display:none;visible:hidden" href="javascript:"></a>
                        <a href="javascript:" data-type="decrease" class="num-change">-</a>
                        <input type="text" name="quantity" value="${cartItem.quantity}" size="2" maxlength="3" class="input text-center noime" readonly="readonly"/>
                    ...
                    <td class="text-center border-l-none"><!-- <a href="/product/collection/${cartItem.productId}" data-id="${cartItem.productId}" class="add-action" rel="fav">加入收藏</a><br /><br /> -->
                        <p><a href="javascript:" data-url="/cart/delete/${cartItem.id}" method-type="get" confirm="确定要从购物车中移除该商品吗？" call-back="remove(${cartItem.id})" class="ajax-request">删除</a></p>
                    </td>
                </tr>
                </#if>
            </#list>
            ...
            <tfoot>
                ...
                <tr class="item">
                    <td class="text-left border-r-none relative" colspan="2" style="padding:15px 10px 13px 10px">
                        <button type="button" name="clear" class="button clear order" style="font-size:16px;"/>清空购物车</button>
                    </td>
                    <td class="text-right border-l-none  relative" colspan="5" style="padding:15px 170px 13px 10px">已选商品  <span class="important level-two select-count-number"></span> 件 
...
var b=location.href,c=d.referrer,f,s,g=d.cookie,h=g.match(/(^|;)\s*ipycookie=([^;]*)/),i=g.match(/(^|;)\s*ipysession=([^;]*)/);if (w.parent!=w){f=b;b=c;c=f;};u='//stats.ipinyou.com/cvt?a='+e('9U.tB.xSXrzPTx4cBWjYdyVMd1wX')+'&c='+e(h?h[2]:'')+'&s='+e(i?i[2].match(/jump\%3D(\d+)/)[1]:'')+'&u='+e(b)+'&r='+e(c)+'&rd='+(new Date()).getTime()+'&Money='+e(_money)+'&ProductList='+e(_productList)+'&e=';
