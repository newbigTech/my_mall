    ...
    <div style="width:40px;display:table-cell;background-color:#ededed"></div><div class="layout-user-right">
        ...
        <div class="tab tab-user-menu clearfix">
            <ul>
                ...
                <li<#if RequestParameters.status==2> class="on"</#if>><a href="?status=2">等待接收<#if WaitingForRec gt 0> <span>${WaitingForRec}</span></#if></a><i class="interval">|</i></li>
                <li<#if RequestParameters.status==3> class="on"</#if>><a href="?status=3">待准备确认<#if WaitingForServer gt 0> <span>${WaitingForServer}</span></#if></a><i class="interval">|</i></li>
                <li<#if RequestParameters.status==4> class="on"</#if>><a href="?status=4">待完成确认<#if FinishForServer gt 0> <span>${FinishForServer}</span></#if></a><i class="interval">|</i></li>
                <li style="float:right">
                <select id="o2oSubscribeStatus" name="status" class="input" style="font-size:14px;margin-top:-3px;margin-left:100px;" onchange="location.href='?status='+this.value;">
        ...
        <table border="0" cellpadding="6" cellspacing="0" width="100%" class="table table-grey" style="word-break:break-all">
         <#if pager.list?exists && pager.list?size &gt; 0>
	            ...
	            <tr class="title">
	                <td colspan="6">
	                                                    ...
	                                                    预约时间：<strong>${(list.estimateDate?string("yyyy-MM-dd"))!}&nbsp;&nbsp;${(list.startHour)!}点${(list.startMinute)!}分到${(list.endHour)!}点${(list.endMinute)!}分</strong> &nbsp;&nbsp;&nbsp;
	                    <span class="grey">|</span> &nbsp;&nbsp;
	                    <#if list.status==2>
	                    <button data-url="/o2oSubscribe/startPrepare" confirm="确定接收预约吗？" success-tip="接受预约成功！" method-type="post" data-param="{'id':${list.id},'memberId':${list.memberId}}" class="button button-green ajax-request">接收预约</button>
	                    <#elseif list.status==3>
	                    <button data-url="/o2oSubscribe/endPrepare" confirm="确定完成准备了吗？" success-tip="提交准备成功！" method-type="post" data-param="{'id':${list.id},'memberId':${list.memberId}}" class="button button-green ajax-request">完成准备</button>
	                    <#elseif list.status==4>
	                    <button data-url="/o2oSubscribe/success" method-type="post" data-param="{'id':${list.id}}" title="门店反馈" template-id="result-template" modal-type="pop" modal-width="600" class="button button-green ajax-request">完成服务</button>
	                    </#if>
	                </td>
	            </tr>
	            ...
	            <tr class="item text-left">
	                <td>试衣结果</td>
	                <td colspan="5"><#if list.payStatus==-1>用户已取消<#elseif list.payStatus==0>用户未购买<#elseif list.payStatus==1>用户已购买  &nbsp;&nbsp;&nbsp; 支付金额：&yen; ${(list.payAmount?string("currency")?substring(1))!}  &nbsp;&nbsp;&nbsp; 零售单号：${(list.retailSn)!}</#if>  &nbsp;&nbsp;&nbsp; 来店人数：${(list.actualNumbers)!}</td>
	            </tr>
	            </#if>
	            <#if list.status==6>
	            <tr class="item text-left">
	            ...
	            <tr class="item">
	                ...
	                <#list list.items as item>
	                    <div class="table-product-item">
	                        <p class="img"><a href="/product/${item.productId}" target="_blank"><img src="${picture_base}/${item.sp1?eval.img}!80" alt="" height="70" /></a></p>
	                        <p class="title"><a href="/product/${item.productId}" style="line-height:18px;" target="_blank">${(item.productName)!}</a></p>
	                        <p class="desc">颜色：${(item.sp1?eval.value)!}&nbsp;&nbsp;尺码：${(item.sp2?eval.value)!}&nbsp;&nbsp;数量：${(item.quantity)!}&nbsp;&nbsp;价格：${(item.price)!}元</span></p>
	                    </div>
	                </#list>
	                </td>
	             </tr>
    ...
    <div class="form">
        <form name="" class="validate-form" action="{{url}}" method="post">
            ...
            <div class="form-item">
                ...
                <input type="text" name="actualNumbers" data-limit="0" id="number" value="" style="width:100px;" class="input" /> 人
