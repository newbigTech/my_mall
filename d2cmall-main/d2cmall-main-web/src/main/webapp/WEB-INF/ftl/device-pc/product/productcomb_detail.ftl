...
<div class="layout layout-response layout-product clearfix">
    ...
    <div class="product-package clearfix">
        <div class="product-package-img" id="slide-img">
            ...
            <#list productComb.products as product>
                <li><a href="${base}/product/${product.id}" target="_blank" title="" alt=""><img id="large-img-${product.id}" src="${picture_base}/${(product.productImageCover)!}!media"><span>${product.name}</span></a></li>
            </#list>
            </ul>
        </div>
        <div class="product-package-info">
            ...
            <form id="product-package-form" action="/order/comb/buynow" method="post">
                ...
                <div class="item" data-id="${product.id}">
                    ...
                    <a href="${base}/product/${product.id}" target="_blank" class="item-img" target="_blank"><img width="80" height="125" src="${picture_base}/${(product.productImageCover)!}!120"></a>
                    <p class="title"><a href="${base}/product/${product.id}" target="_blank" >${product.name}</a><#if product.mark==0><span class="red">【已下架】</span></#if></p>
                    <div class="product-sku" style="height:100px">
                        <div class="img" style="width:88%">
                     	   <label>颜色 </label>
                            <div class="value">
                            <#list product.salesproperty?eval.sp1 as p>
                                <a href="javascript:" class="color-section c-${product.id}-${p.id}" data-type="color" data-id="${p.id}" data-self="${product.id}${p.id}" rel="${p.value}" title="${p.value}"><#if p.img><span class="img"><img src="${picture_base}/${p.img}!50" data-image="${picture_base}/${p.img}!media" ori-image="${picture_base}/${p.img}" alt="" /></span></#if><i>已选择</i></a>
                            </#list>
                            </div>
                        </div>
                        </br>
                        <div class="txt" >
                            ...
                            <#list product.salesproperty?eval.sp2 as p>
                                <a href="javascript:" class="size-section s-${product.id}-${p.id}" data-type="size" data-id="${p.id}" data-self="${product.id}${p.id}" rel="${p.value}"><span>${p.value}</span><i>已选择</i></a>
                            </#list> 
                            </div>
                        </div>
                    </div>
                    <div  style="clear:left"><span class="sku-select">请选择颜色尺码</span><span class="store-span"></span></div>
                </div>
                </#list>
                <div class="product-package-buy">
                    <span class="select-count" style="float:left;font-size:14px;margin-left:1%;margin-right:5px">购买</span>
                    ...
                    &nbsp;&nbsp; <span style="vertical-align:-webkit-baseline-middle">商品总价：&yen;</span> <strong class="totle-price"><#if productComb.price &lt; 0>????.??<#else>${productComb.price?string('0.00')}</#if></strong> &nbsp;&nbsp;
   ...
   <div class="product-cont-bar scroll-suspend" data-offset="50" data-scroll-holder="product-suspend-holder" data-scroll-bg="product-cont-tab-bar" data-scroll-end="product-hot">
        <div class="tab clearfix">
           <ul>
               <li class="on" data-id="desc">商品详情</li>
               <li data-id="comment" data-url="/productComb/comment?ids=<#list productComb.products?if_exists as product>${product.id}<#if product_index!=productComb.products?size-1 >,</#if></#list>">客户评价 <span></span></li>
           </ul>
        </div>
    ...
    <div class="product-cont-tab-cont" style="clear:right">
        <div class="product-detail-right">
             <div class="product-info-suspend scroll-suspend" data-offset="70"  data-scroll-end="product-hot">
                ...
                <input name="productCombId" type="hidden" value="${productComb.id}" />
	                <#list productComb.products as product>
	                  <div class="item" data-id="${product.id}" style="border-bottom:1px dotted #CCC;margin-bottom:10px">
	                   <input name="skuId" type="hidden" value=""/>      
                  	   <input type="hidden" name="store" value="" />
	                    ...
	                    <div class="product-sku" style="border-top:none">
	                        ...
	                        <div class="txt clearfix">
	                         ...
	                         <div  style="clear:left;color:#FF6600"><span class="sku-select">请选择颜色尺码</span><span class="store-span" style="color:#000"></span></div>
	                    </div>
                    </div>
                     </#list>
                    ...
                    <div class="clearfix" style="margin-top:10px;position:relative;">
                     <#if productComb.mark == 1>
	                     	 <button name="pay"  type="button" id="combo-buy" class="button button- button-red" data-form="bottom" data-type="buy" style="width:60%">立即购买</button>
	                      <#else>
