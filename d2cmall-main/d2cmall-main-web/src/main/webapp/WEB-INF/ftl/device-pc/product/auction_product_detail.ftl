...
<div class="layout layout-response layout-auction clearfix">
        ...
        <div class="product-img">
            <div class="img-thumbs">
             <#if result.datas.product.product.productImageList?size gt 0 &&result.datas.product.product.productImageList?exists>
                ...
                <a href="javascript:"><img src="${picture_base}/${image}!120" data-image="${picture_base}/${image}!media" ori-image="${picture_base}/${image}" /></a>
                </#list>
               </#if>
            </div>
            <div class="img-media">
                ...
                <div class="middle-image"><img src="${static_base}/c/images/space.gif" id="product-image-middle" data-image="" data-id="0" alt="" /></div>
        ...
        <div class="product-info product-normal">
            ...
            <div>
                ...
                <div class="auction-time">
                    <div class="auction-string"><img src="//static.d2c.cn/common/nc/img/auction_on.png" width="30"/> 拍卖中</div>                    
                    <div class="count-down" data-function="countdownOver" data-starttime="${result.datas.product.beginDate?string("yyyy/MM/dd HH:mm:ss")}" data-endtime="${result.datas.product.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-servertime="${.now?string("yyyy/MM/dd HH:mm:ss")}" data-defined="millisecond"></div>
                </div>
                <#elseif !result.datas.product.doing && (!myOffer?exists || myOffer?size lte 0 || myOffer[0].status==0)>
                 <div class="auction-time">
                    <div class="auction-string" style="color:gray"><img src="//static.d2c.cn/common/nc/img/auction.png"  width="30"/> ${result.datas.product.statusName}</div>   
                ...
                <#elseif !result.datas.product.doing && myOffer?exists && myOffer?size gt 0 && myOffer[0].status==1>
                 <div class="auction-time">
                    <div class="auction-string"><img src="//static.d2c.cn/common/nc/img/auction_on.png"  width="30"/> 竞拍成功</div>   
                    <div class="auction-end-time" >${result.datas.product.endDate?string("yyyy/MM/dd HH:mm:ss")}</div>                
                 </div>
                </#if>
                <div class="auction-price">
                    ...
                    <span class="price"><em>&yen;</em> <strong id="current-price" data-value="${result.datas.product.currentPrice}">${result.datas.product.currentPrice?string('currency')?substring(1)}</strong></span>
                ...
                <div class="auction-price">
                    ...
                    <span class='price-two' id="step-price" data-value="${result.datas.product.stepPrice}"><em>&yen;</em> ${result.datas.product.stepPrice}</span>
                ...
                <div style="margin-top:0px">
                    ...
                    <div class="clearfix" style="padding:15px 0 15px 0;float:left">
                        <em class="data-crease Auctiondecrease" unselectable="on" onselectstart="return false;">-</em>
                        <input type="text" name="offer" id="offer-price" value="${result.datas.product.currentPrice+result.datas.product.stepPrice}" maxlength="6" class="data-num"  readonly="readonly" <#if result.datas.product.doing> <#if !myMargin?exists || myMargin.status!=1>style="background-color:#F2F2F2" </#if></#if>/>
                        <em class="data-crease Auctionincrease" unselectable="on" onselectstart="return false;">+</em>                 
                    </div>
			        <#if LOGINMEMBER.id!=null>
						<#if result.datas.product.doing>
							<#if myMargin?exists && myMargin.status==1>
								<button type="button" class="auction-button" data-type="auction" data-id="${result.datas.product.id}">立即出价</button>    
							<#else>
								<button type="button" class="auction-button" data-type="margin" data-id="${result.datas.product.id}">支付保证金</button>    
							</#if>
						<#else>
							<#if myMargin?exists>
								<#if myMargin.status==2 || myMargin.status==6 ||  myMargin.status==8 || myMargin.status==-6> 
									<#if myMargin.status==2>
					        			...
					        			<button type="button" class="auction-button" data-type="payauction" data-id="${result.datas.product.id}">去支付</button>
									<#else>	
									</#if>
								<#elseif myMargin.status==-2 || myMargin.status==-8>
									<button type="button" class="auction-button auction-disable"  disabled="disabled">竞拍出局</button>
								<#else>
									<button type="button" class="auction-button auction-disable"  disabled="disabled">结果处理中</button>
								</#if>
							<#else>
								<button type="button" class="auction-button auction-disable"  disabled="disabled">${result.datas.product.statusName}</button>
							</#if>
						</#if>
					<#else>
						<#if result.datas.product.doing>
                    ...
                    <#if myOffer>             
	                    ...
	                    <span class="price"><em>&yen;</em> <strong data-value="${myOffer[0].offer}">${myOffer[0].offer?string('currency')?substring(1)}</strong></span>
              ...
              <#else>
		      <button type="button" class="button button-l disabled" style="width:90%;" disabled="disabled">已下架</button>
	          </#if>
            ...
            <div class="product-attr" style='margin-top:100px'>
            	...
            	<div class="designer-detail" style='margin-top:50px'> 
                	<a class="CustomerCare" style="margin-right:20px;cursor:pointer" onclick="NTKF.im_openInPageChat();"><span style="margin-right:5px"></span>联系客服</a>
                </div>
               
            </div>
            <div class="product-others">
                <div class="share-box" data-config="{'pic':'${picture_base}/${result.datas.product.product.productImageListFirst}'}">
                ...
                <div class="product-other-operator">
              		  <a href="javascript:" data-url="" success-tip="收藏成功！" call-back="false" class="ajax-request" style="padding-left: 35px;padding-top: 5px;"> <i class="collection"></i>
                    收藏</a>&nbsp;&nbsp;
                </div>
            </div>
        </div>
    ...
    <div class="product-cont-bar" data-offset="50" data-scroll-holder="product-suspend-holder" data-scroll-end="footer" data-scroll-bg="product-cont-tab-bar">
    <div class="tab clearfix">
        <ul>
            <li class="on" data-id="desc">商品详情</li>           
            <li class="offerList" data-id="offer" data-url="/auction/offer/${result.datas.product.id}">竞拍记录 <span>(${(result.datas.offers.list?size)!})</span></li>
        </ul>
    </div>
    </div>
...
<script id="auctionlist" type="text/html">
	...
	<div class="relate-item">
	        <p class="relate-item-pic"><a href="/auction/product/{{value.id}}" target="_blank"><img src="${picture_base}{{value.product.productImageCover}}!120" alt="{{value.title}}" /></a></p>
