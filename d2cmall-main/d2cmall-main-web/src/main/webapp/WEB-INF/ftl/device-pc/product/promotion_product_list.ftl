...
<div class="layout layout-response layout-product clearfix">
    ...
    <#list products.list as product>
        <div class="list-item">
            ...
            <a href="/product/${product.productId}" class="img" target="_blank"><#if product.store?exists && product.store lt 1><i class="sale" ></i><i class="satip">已售罄</i></#if><img src="${static_base}/blank/300-467.gif" data-image="${picture_base}${(product.productImageCover)!}!300" alt="${(product.name)!}" />  </a>
            <p class="title"><#if product.topical==1><strong style="color:#FD555D">[主推]</strong></#if><a href="/product/${product.productId}" target="_blank" title="${(product.name)!}">${(product.name)!}</p>
            <p class="price">
                <span>&yen; ${(product.currentPrice)!}</span>
                <#if (product.currentPrice?exists && product.currentPrice lt product.originalPrice)> &nbsp;<s>&yen; ${(product.originalPrice)!}</s></#if>
            </p>
          </a>
        </div>
    </#list>
