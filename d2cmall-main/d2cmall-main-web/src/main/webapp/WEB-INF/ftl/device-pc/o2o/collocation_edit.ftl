			...
			<div style="margin-top:15px;" class="clearfix grid-form">
				<div class="float-left" style="margin-right:20px;">
					<div style="width:300px;min-height:400px;padding:10px;border:1px solid #CCC;box-shadow:0 0 3px #CCC;">
						<div class="upload-button<#if (collocation.pic)!> display-none</#if>" style="margin:60% 0 0 36%;">
							<a href="javascript:void(0)" class="upload-a btn-my-upload"><span>上传照片</span><input type="file" class="upload-file" name="file" accept="image/gif,image/png,image/jpeg"></a>
						</div>
						<div class="upload-preview<#if (collocation.pic)!><#else> display-none</#if>">
							<div style="position:relative;"><#if collocation.pic><img src="${picture_base}${(collocation.pic)!}" class="tag-main" alt="" width="300"/></#if>
							<#if collocation.items && collocation.items?size gt 0>
								<#list collocation.items as item>
								<div class="tag-img-item" id="tag-item-${item.id}" style="top:${item.y*100}%;left:${item.x*100}%">
									...
									<div class="tag-img-edit display-none" id="tag-edit-${item.id}">
										...
										<div class="main">
										<input type="hidden" name="id" class="form-input" value="${item.id}" /><input type="hidden" name="collocationId" class="form-input" value="${(collocation.id)!0}" /><input type="hidden" name="x" class="form-input" value="${item.x}" /><input type="hidden" name="y" class="form-input" value="${item.y}" />
										<p>单品标题</p>
										<p><input type="text" name="title" size="30" class="form-input" value="${item.title}" /></p>
										<p>单品价格</p>
										<p><input type="text" name="price" size="30" class="form-input" value="${item.price}" /></p>
										<p>单品连接</p>
										<p><input type="text" name="url" size="30" class="form-input" value="${item.url}" /></p>
										<p class="text-center"><button type="button" rel="${item.id}" class="button-l b-b submit">提交</button> &nbsp;&nbsp; <button type="button" rel="${item.id}" class="button-l cancel">取消</button></p>
										</div>
									</div>
								</div>
								</#list>
				...
				<form id="submit-form" action="/collocation/${action}" method="post" class="float-left" style="width:500px;">
					<input type="hidden" name="type" value="CUSTOMER"/>
					<input type="hidden" id="collection-id" name="id" value="${(collocation.id)!0}"/>
					<input type="hidden" name="picture.path" id="picture" value="${(collocation.pic)!}" accept="image/*" class="path-input" />
					<p>搭配标题</p>
					<p style="margin:5px 0;"><input name="title" id="title" value="${(collocation.title)!}" class="input-l" style="width:90%;" /></p>
					<p class="tips-validate red display-none" id="title-tip" for="title"><i></i>标题不能为空</p>
					<p style="margin:5px 0;">搭配描述</p>
					<p><textarea name="description" id="description" class="input-l" style="width:90%;height:150px;">${(collocation.description)!}</textarea></p>
					<p class="text-center" style="margin:5px 0;"><button type="submit" name="button-save" class="button-l b-b">保存</button></p>
				</form>
		 	</div>
		</div>
	</div>
