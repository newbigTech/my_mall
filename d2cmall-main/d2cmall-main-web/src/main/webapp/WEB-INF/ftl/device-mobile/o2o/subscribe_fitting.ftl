...
<style>
input[type=checkbox]:before, input[type=checkbox]:checked:before, input[type=radio]:before, input[type=radio]:checked:before{width:1.2em;height:1.2em;margin-top:.15em;}
input[type=checkbox], input[type=radio]{margin-right:4px;}
.form .form-item.item-flex label{color:#999;}
.server-list label{width:30%;padding:.8em 0}
.form .form-item label[for]{font-size:.8em}
.item-flex .iic{width: 1.2em;height: 1.2em;vertical-align: middle;opacity: .6; position:absolute;right:1.5em;top:1.2em;}
    ...
    <#list o2oSubscribe.items as item>
        <div class="item form-item item-card  item-margin clearfix"  data-id="${(item.id)!}">
            <a href="/product/${(item.productId)!}">
                <span class="img"><img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}${(item.sp1?eval.img)!}!275" alt="" style="min-height:6em"/></span>
                <span class="title">${item.productName} &nbsp; ${(item.sp1?eval.value)!} ${(item.sp2?eval.value)!}</span>
             </a>
            <span class="property" style="position:absolute;bottom:.6em;left:23%;">${item.sp1?eval.name}：${item.sp1?eval.value} &nbsp;${item.sp2?eval.name}：${item.sp2?eval.value}</span>
            <a href="javascript:" data-url="/o2oSubscribe/deleteItem/${(item.id)!}" confirm="确定取消预约吗？" success-tip="取消预约成功！" call-back="removeItem(${(item.id)!})" class="icon icon-trash ajax-request reserve-trash" ></a>
        </div> 
    </#list>
    </div>
    <div style="height:3em"></div>
    <div class="suspend-bar">
        <button type="button" class="button button-l button-wblack" style="width:45.5%;margin-left:3%;" id="back">再挑几件</button><button type="button" class="button button-l button-black button-one" style="width:45.5%;margin-left:3%;margin-right:3%;" id="next">立即预约</button>
    </div>
</div>
<div class="section display-none" id="form">
    <div class="form">
        <form name="fitting-form" class="validate-form" action="/o2oSubscribe/create.json" method="post" target="_self" >
            ...
            <i class="icon icon-arrow-right iic" style="top:.8em"></i>
                ...
                <option value="${store.id}" data-id="${store_index}"<#if o2oSubscribe.storeId?exists && o2oSubscribe.storeId==store.id> selected</#if>>${store.name}</option>
                </#list>
                </#if>
                </select>
                <div class="form-item-tip display-none" id="store-info" style="padding-left:26%;font-size:10px;color:#999">
                </div>
            </div>
            <input type="hidden"  value="" id="store-id"/>
             <div class="form-item item-flex" style="margin-bottom:.45em">
                ...
                <div style="width:70%;display:inline-block;">
                    ...
                    <input id="StartTime" name="startTime" runat="server"  class="input" readonly="readonly" style="width:30%;"  />
                    <input id="EndTime"  name="endTime" runat="server" class="input" readonly="readonly" style="width:30%;" />
                    <!--
            ...
            <div class="form-item item-flex" style="margin-top:.45em">
                <label>预约人</label>
                <input type="text" name="name" value="${(o2oSubscribe.name)!}" title="您的称呼" placeholder="填写您的称谓" class="input validate" />
            </div>
            <div class="form-item item-flex">
                <label>性 &nbsp;&nbsp;&nbsp;&nbsp; 别</label>
                <input type="radio" id="girl" name="sex" value="女" <#if member?exists><#if member.memberInfo.sex=='女'> checked</#if><#else> checked</#if> /><label for="girl">女士</label>
                <input type="radio" id="boy" name="sex" value="男" <#if member?exists><#if member.memberInfo.sex=='男'> checked</#if></#if> /><label for="boy">先生</label>
            </div>
            <div class="form-item item-flex">
                <label>预约电话</label>
                <input type="tel" name="tel" value="${(o2oSubscribe.tel)!}" title="您的联系方式" placeholder="填写您的手机号" class="input validate" data-rule="num"/>
            </div>
            <div class="form-item item-flex">
                <label>微 &nbsp;信 &nbsp;号</label>
                <input type="text" name="wechat" value="${(o2oSubscribe.wechat)!}" placeholder="填写您的微信号" class="input" />   
            </div>
            <!--
            <div class="form-item item-flex">
                <label>首选方式</label>
            ...
            <div class="form-button">
                ...
                <button type="button" id="subscribe-button" name="send-button" class="button button-l button-red">确定</button>
            </div>
        </form>    
    </div>
</div>
...
<script id="store-info-template" type="text/html">
    <div style="line-height:150%;">
        <p>{{address}}</p>
        <p>电话：{{tel}}</p>
       	<p>营业时间：{{startHour}}:{{startMinute == 0 ? '00' : startMinute}} - {{endHour}}:{{endMinute == 0 ? '00' : endMinute}}</p>
    </div>
</script>

<script id="store-service-template" type="text/html">
    {{each services as value i}}
    <label for="service-{{i}}" style="white-space:nowrap;line-height:100%;"><input type="checkbox" name="storeService" value="{{value}}" id="service-{{i}}" />{{value}}</label>
    {{/each}}
</script>
<script>
<#if stores?exists&&stores?size gt 0>
var store_data=[<#list stores as store>{'id':'${store.id}','name':'${store.name}','map':'${store.xy}','address':'${store.address}','tel':'${store.tel}','startHour':'${store.startHour}','startMinute':'${store.startMinute}','endHour':'${store.endHour}','endMinute':'${store.endMinute}','service':'${store.storeService}'}<#if store_index lt (stores?size-1)>,</#if></#list>]; 
</#if>
