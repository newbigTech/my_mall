...
<div class="section lazyload">
    <#if !m.FROMAPP>
    <h2 class="block-title"  style="background:#fff;line-height:2.8em;height:2.8em;font-size:.8em;text-align:left;padding:0 0 0 1.2em">
        ...
        <#if (RequestParameters.c!='')>
        <#if cate.parent?exists>&nbsp; <span class="split"> 〉</span> <a href="?t=${(topCate.id)!}&c=${cate.parent.id}">${(cate.parent.name)!}</a></#if>
        &nbsp; <span class="split"> 〉</span> <a href="?t=${(topCate.id)!}&c=${cate.id}">${(cate.name)!}</a>
        </#if>
        <#else>所有商品 </#if>
    </h2>
    <#if (RequestParameters.d!='')><h3 class="block-subtitle">品牌：<a href="/showroom/designer/${RequestParameters.d}">${(designer.name)!}</a></h3></#if>
    </#if>
	...
	<a href="/product/${list.id}" class="item item-flex item-gap">
		<span class="img">
		<#if list.store?exists && list.store lt 1><i class="n-product"></i><span class="outp">已售罄</span></#if>		
		<img src="${static_base}/m/img/blank100x157.png" data-image="<#if list.productImageCover?index_of('http') != -1>${list.productImageCover}<#else>${picture_base}${list.productImageCover}</#if>" style="vertical-align:middle;" alt="" />
		</span>
	...
	<#if RequestParameters.o!=''>
	   <#assign param=param+'&o='+RequestParameters.o>
	</#if>
	<div class="load-more scroll-load-more" data-url="/product/pager?${param}" data-target="load-more-product" template-id="list-template" data-page="${pager.pageNumber}" data-total="${pager.pageCount}">点击加载更多</div>  
	</#if>
	<#else>
	<hr />
...
var gap = _width*0.02;
var _height = (_width-gap*0.64)*1.55 + 'px'; 
$('.img').css({'height':_height,'line-height':_height});
