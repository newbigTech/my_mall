...
<div class="section lazyload">
    ...
    <#list pager.list as exchange>
        <div class="form-item item-card item-margin clearfix">
            <div class="top">换货编号：${exchange.exchangeSn} <span class="float-right">${(exchange.exchangeStatusName)!}</span>
            </div>
            <a href="/member/exchange/${(exchange.id)!}" class="clearfix">
                <span class="img"><img src="${static_base}/m/img/blank100x157.png" data-image="<#if exchange.oldSp1?exists>${picture_base}/${(exchange.oldSp1?eval.img)!}!120</#if>" /></span>
            ...
            <div class="bar text-left">
           	 <a href="/member/exchange/${exchange.id}" class="button button-red" style="margin-top:.3em">查看详情</a>
                <#if exchange.exchangeStatus==1>
                <a href="/member/exchange/logistic/${exchange.id}" class="button button-red" style="margin-top:.3em;float:right">填写快递单号</a>
                </#if> 
                <#if exchange.exchangeStatus &lt;3 &&exchange.exchangeStatus &gt; -1>
                <a href="javascript:" data-url="/member/exchange/cancel/${exchange.id}.json" confirm="确定要取消申请中吗？" class="ajax-request button button-outline" method-type='post'  style="margin-top:.3em">取消申请</a>
                </#if>
                <#if exchange.exchangeStatus==4>
                <a href="javascript:" data-url="/member/exchange/receive/${exchange.id}.json" confirm="确定收到你想要的商品了吗？" class="ajax-request button button-outline" method-type='post'  style="margin-top:.3em">确认收货</a>
                </#if>
