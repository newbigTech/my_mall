	...
	<div class="margin-item">
		<div class="clearfix" style="padding-bottom:5px;border-bottom:1px solid rgba(0,0,0,.04);">
			<div class="margin-item-img">
				<a href="/auction/product/${marginItem.auctionId}"><img src="${picture_base}${marginItem.product.productImageList[0]}" style="width:70px;height:105px;" alt="" /></a>
			</div>
			<div class="margin-item-cont">
				<div style="line-height:1.125rem;">
					<#if marginItem.auctionProduct.status == "2"><span class="auction-icon">${marginItem.auctionProduct.statusName}</span><#elseif marginItem.auctionProduct.status == "0"><span class="auction-icon">${marginItem.auctionProduct.statusName}</span><#else><span class="auction-icon">${marginItem.auctionProduct.statusName}</span></#if>
				...
				<a href="/auction/product/${marginItem.auctionId}" class="margin-title">${marginItem.auctionTitle}</a>
				<div class="margin-bond"><a href="/auction/product/${marginItem.auctionId}">保证金：&yen;${marginItem.margin?string("currency")?substring(1)}</a></div>
				<div class="margin-offer cleearfix">
					...
					<#if marginItem.status == 2>
						<form name="form-auction" id="form-auction" action="/auction/margin/confirm/${marginItem.marginSn}" method="POST"></form>
						<span  class="margin-status-item pay-item" data-marginSn="${marginItem.marginSn}">支付尾款</span>
					</#if>
					<#if marginItem.status == 8><span  style="margin-left:10px;" class="margin-status-item confirm-item" data-id="${marginItem.auctionProduct.auctionMarginId}">确认收货</span><span class="margin-status-item"><a href="//m.kuaidi100.com/index_all.html?type=${marginItem.deliveryCorpName}&postid=${marginItem.deliverySn}">查看物流</a></span></#if>
					<#if marginItem.status == 10><span  class="margin-status-item"><a href="//m.kuaidi100.com/index_all.html?type=${marginItem.deliveryCorpName}&postid=${marginItem.deliverySn}">查看物流</a></span></#if>
				</div>
			</div>
		</div>
