...
<div class="page level-page">
	<div class="my-level">
		<div class="lev">
			我的等级  <#if level.memberDetail.level gt 0><i class="level-${level.memberDetail.level} level-icon"></i><#else> &nbsp;普通会员</#if>
		</div>
		<div class="lev-line">
			<div class="user-head">
			...
			<div class="line-grey">
				<#if level.memberDetail.level gt 0><div class="vd vd-1 flex" data-level="${level.memberDetail.level-1}"><p>v${level.memberDetail.level-1}</p><p class="pp">0</p></div></#if>
				<#if level.memberDetail.level lt 4><div class="vd vd-2 flex" data-level="${level.memberDetail.level+1}"><p>v${level.memberDetail.level+1}</p><p class="pp">0</p></div></#if>
				<#if level.memberDetail.level lt 3><div class="vd vd-3 flex" data-level="${level.memberDetail.level+2}"><p>v${level.memberDetail.level+2}</p><p class="pp">0</p></div></#if>
			</div>
			<div class="line-yel">
			</div>
		</div>
...
<script id="member-level" type="text/html">
	{{each list as value index}}
		<div class="prero-list">
			{{if index==4|| index==6|| index==7}}<div class="wait">敬请期待</div>{{/if}}
			<a href="{{if value.status==0}}javascript:;{{else}}{{value.href}}{{/if}}"><img src="{{value.src}}{{value.status}}.png" ></a>
			<p>{{value.title}}</p>
		</div>
	{{/each}}
</script>
