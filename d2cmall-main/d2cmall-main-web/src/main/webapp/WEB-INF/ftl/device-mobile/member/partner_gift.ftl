...
<script>
	...
	$('.box-button').on(click_type,function(){
		...
		if(!inputVal){
			...
			.then(function(res2){
				console.log(res2)
				//return ajaxRequest('/partner/update',{id:${partnerId},weixin:inputVal})	
			})
			.then(function(res3){
				console.log(res3)
			})
						...
						if(res1.result.status === 1){
							$.post('/partner/update',{id:${partnerId},weixin:inputVal},function(res2){
								if(res2.result.status === 1){
									$.flashTip({position:'center',type:'success',message:'快去我的优惠券查看礼包吧~',time:1000});
									setTimeout(function(){
										location.href="/coupon/myCoupon?status=UNCLAIMED&status=CLAIMED";
									},2000)
								}else{
									$.flashTip({position:'center',type:'error',message:res2.result.message,time:1000});
								}
							},'json');
						}else{
