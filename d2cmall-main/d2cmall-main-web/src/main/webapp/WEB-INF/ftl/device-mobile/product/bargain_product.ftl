...
<div class="bargin-section">
	<div class="bargin-top">
	 ...
	 <div class="bargin-productinfo" style="position:relative;">
		<a href="/page/1212miaowujie" class="pro-go" style="display:none"><img src="//static.d2c.cn/img/promo/bargin/icon_enter.png" style="width:100%;"></a>
	    <div class="barpro-title">${bargainPromotion.name}</div>
		<#if bargainPromotion.status==1 >
		<div class="bar-times on count-down" data-type="split-time"  data-endtime="${bargainPromotion.beginDate?string("yyyy/MM/dd HH:mm:ss")}" data-musttime="must"><span>距离开始时间</span><span class="n-time  hour down-time">00</span><span class="k-time">时</span><span class="n-time  minute down-time">00</span><span class="k-time">分</span><span class="n-time second down-time">00</span><span class="k-time">秒</span> </div>
		...
		<#else>
		<div class="bar-times on count-down" data-type="split-time"  data-endtime="${bargainPromotion.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-musttime="must"><span>距离结束时间</span><span class="n-time  hour down-time">00</span><span class="k-time">时</span><span class="n-time  minute down-time">00</span><span class="k-time">分</span><span class="n-time second down-time">00</span><span class="k-time">秒</span> </div>
		</#if>
		<#else>
	...
	<div class="bargin-detail">
		<div class="t-title" data-url="/product/detail/${bargainPromotion.product.productId}?type=ajax"><span>图文详情</span></div>
	</div>
    ...
    <#if bargainPromotion.status==1>
		<div class="bargin-button flex">
			...
			<div class="cur-price" style="margin-left:10px;">原价<span class="fh">¥</span><span class="c-strong">${bargainPromotion.product.minPrice}</span></div>
			<div class="bargin-sue flex"  style="flex:4;justify-content:flex-end">
				<button class="remind" data-id="${bargainPromotion.product.id}" style="color:rgba(242,51,101,1);">开抢提醒</button>
			</div>
	    </div>
    <#elseif bargainPromotion.status==2>
	    ...
	    <div class="bargin-button flex">
	    	 <i class="icon icon-bargin"></i>
			<div class="bargin-sue flex"  style="flex:4;justify-content:flex-end">
				<button class="overpromotion" onclick="gobuy()" style="color:rgba(242,51,101,1);">&yen;${bargainPromotion.product.minPrice}&nbsp;购买</button>
				<button class="overpromotion gobargin" style="background:rgba(242,51,101,1);color:#FFF">发起砍价</button>
	        </div>
	    </div>
	    <#else>
	    <div class="bargin-button flex">
	        <button class="button button-al" onclick="location.href='/bargain/promotion/list?status=1'">看看其他砍价商品</button>
	    </div>
	    </#if>
    <#else>
	 	<div class="bargin-button flex">
	        <button class="button button-al" onclick="javascript:location.href='/bargain/promotion/list?status=1'">看看其他砍价商品</button>
	    </div>
	</#if>
</div>
<a href="/page/918chudong"><img src="//static.d2c.cn/img/topic/180915/fresh/images/fresh.png" width="100%" id="nav" /></a>
<share data-title="${bargainPromotion.shareTitle}" data-pic="${picture_base}${bargainPromotion.sharePic}" data-desc="${bargainPromotion.shareDes?replace('#price',bargainPromotion.minPrice)}"></share>
<script>
	var page_elem={
		init:function(){
...
function gobuy(){
	...
	<#else>
		location.href='/product/${bargainPromotion.product.id}';TDAPP.onEvent('砍价商品购买');
	</#if>
}
</script>
