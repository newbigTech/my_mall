...
<div class="comment-item">
    <div class="comment-item-content">
        ...
        <#list comment.pictures as path>
         <a class="hover-img" href="${picture_base}/${path}"><img src="${picture_base}/${path}!120" alt="" /></a>                                                                
        </#list>    
        </div>
        </#if>
        <!--
    ...
    <div class="comment-reply-item">
        <#if commentReply.type='SYSTEM'>解释：<#else>追评：</#if>${commentReply.content} <i>${commentReply.createDate?string("yyyy.MM.dd")}</i>
    </div>
    </#list>
    </div>
    </#if>
