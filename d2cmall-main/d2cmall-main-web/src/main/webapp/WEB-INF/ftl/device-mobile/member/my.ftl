...
<#if m.LOGINMEMBER.id!=null>
    ...
    <div class="form clearfix">
        <a <#if !m.LOGINMEMBER.d2c>href="/member/bind"<#else>href="/member/order"</#if> class="form-item"><i class="space"></i>所有订单<i class="icon icon-arrow-right"></i></a>
        <a <#if !m.LOGINMEMBER.d2c>href="/member/bind"<#else>href="/member/order?orderStatus=WaitingForPay"</#if>  class="form-item item-mini-hori" style="width:20%;"><i class="iconfont icon-daifukuan"></i>待付款<#if WaitingForPay gt 0><em>${WaitingForPay}</em></#if></a>
        <a <#if !m.LOGINMEMBER.d2c>href="/member/bind"<#else>href="/member/order?orderStatus=WaitingForDelivery"</#if> class="form-item item-mini-hori" style="width:20%;"><i class="iconfont icon-daifahuo"></i>待发货<#if WaitingForDelivery gt 0><em>${WaitingForDelivery}</em></#if></a>
        <a <#if !m.LOGINMEMBER.d2c>href="/member/bind"<#else>href="/member/order?orderStatus=Delivered"</#if> class="form-item item-mini-hori" style="width:20%;"><i class="iconfont icon-daishouhuo"></i>待收货<#if Delivered gt 0><em>${Delivered}</em></#if></a>
        <a <#if !m.LOGINMEMBER.d2c>href="/member/bind"<#else>href="/member/order?orderStatus=Success"</#if> class="form-item item-mini-hori" style="width:20%;"><i class="iconfont icon-yishouhuo"></i>已收货<#if Success gt 0><em>${Success}</em></#if></a>
        <a <#if !m.LOGINMEMBER.d2c>href="/member/bind"<#else>href="/member/refund/list"</#if> class="form-item item-mini-hori" style="width:20%;border-left:1px solid #EEE;margin-left:-1px;"><i class="iconfont icon-tuikuantuihuo"></i>售后</a>
...
function judgeEnv() {
	if(isWeChat) {
		...
		$('.qr-close').on(click_type,function(){popupModalClose()});
	}else{
