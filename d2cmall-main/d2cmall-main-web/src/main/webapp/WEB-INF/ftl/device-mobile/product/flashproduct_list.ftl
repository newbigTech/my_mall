...
<div class="swiper-container swiper-container-horizontal limit-time" id="swiper-container2">
	<div class="swiper-wrapper">
		...
		<#list list as lists>
		<#if !lists.end>
		<div class="swiper-slide limit-time-item <#if list?size ==1>on<#else><#if currentId==lists.id>on</#if></#if>" data-id="${lists.id}">
...
<div class="load-more flash-load-more" data-target="load-more-product" template-id="limit-list" style="display:none;">点击加载更多</div>
<div style="height:3.125rem;"></div>  
<div class="fixed-taber flex">
    <a href="/flashpromotion/product/session" target="_self"><img src="//static.d2cmall.com/img/promo/limit/bar_timeshop_chosen.png" ></a>
    <a href="/flashpromotion/brand/session" target="_self"><img src="//static.d2cmall.com/img/promo/limit/bar_super.png" ></a>
</div>
...
<script id="limit-title" type="text/html">
	<div class="limit-title flex"  style="padding:0 15px;">
		<p>{{flashPromotion.sessionRemark}}</p>
		{{if flashPromotion.statusName=="即将开始"}}
			<div class="limit-countdown count-down" data-type="split-time"  data-endtime="{{flashPromotion.startDate}}" data-musttime="must">
				距开始&nbsp;<span class="hour down-time">00</span>:<span class="minute down-time">00</span>:<span class="second down-time">00</span>
			</div>
		{{else if flashPromotion.statusName=="抢购中" || (flashPromotion.statusName=="已开抢"&& !flashPromotion.end)}}
			<div class="limit-countdown count-down" data-type="split-time"  data-endtime="{{flashPromotion.endDate}}" data-musttime="must">
