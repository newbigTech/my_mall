...
<div class="bargin-content" style="padding:0 15px 15px 15px;background-color:#FFF;">
	<div class="bargin-navs flex">
		...
		<#if m.LOGINMEMBER.id!=null && m.LOGINMEMBER.d2c><div class="nav-itemss" data-url="/bargain/mine/list" data-status="2">我的砍价</div></#if>
		
	</div> 
	<div class="bargin-list" id="load-more-product" style="min-height:5em;"></div>
</div>
</div>
<div class="load-more bargain-load-more"  data-target="load-more-product" template-id="product-barginlist" style="display:none;">点击加载更多</div>
<share data-title="快跟我一起来砍价，明星同款最低砍到0元" data-pic="https://static.d2c.cn/img/promo/bargin/share_icon.jpg" data-desc="限时限量，最低可以砍到0元"></share>
...
{{if list.length>0}}
    ...
    <div class="bargin-item">
        <div class="bargin-product" >
			...
			{{if (value.statusName=='砍价成功' && value.bargain.over) || value.statusName=='砍价结束'}}
            <div class="my-bargin-mask" onclick="location.href='/bargain/personal/{{value.id}}'">
                <div class="mask-tips">
                    <div>距离最后支付时间仅剩</div>
                    <div style="color:#F23365;font-size:18px;line-height:26px;" class="count-down alowtime" data-type="split-time" data-endTime="{{value.bargain.endDate}}" data-musttime="must"><span>
                    <span class="hour down-time">00</span> <span class="time-split">:</span> <span class="minute down-time">00</span> <span class="time-split">:</span> <span class="second down-time">00</span></span></div>
        ...
        <div style="padding:0 10px">
            <div class="my-bragin-press">
            	<div class="prsss-price flex">
                    <div>原价:¥{{value.originalPrice}}</div><div>底价:¥{{value.bargain.minPrice}}</div>
            	</div>
            ...
            <div class="my-bragin-price flex" style="justify-content:space-between;">
				...
				{{if value.statusName=='支付商品超时'}}
				    底价<span style="padding-left:5px;font-size:12px;font-weight:bold">¥</span><span style="font-size:20px;font-weight:bold">{{value.bargain.minPrice}}</span>
				{{else}}
				    砍后价<span style="padding-left:5px;font-size:12px;font-weight:bold">¥</span><span style="font-size:20px;font-weight:bold">{{value.price}}</span>
				{{/if}}
				...
				{{else if value.statusName=='支付商品超时'}}	
                    <div class="my-bragin-button" onclick="location.href='/bargain/promotion/list?status=1'">看看其他</div>
			    {{else}}	
...
<script id="product-barginlist" type="text/html">
{{if list.length>0}}
    {{each list as value i}}
    <div class="bargin-item">
        <div class="bargin-product" >
            <a href="/bargain/detail/{{value.id}}">
                <img src="//img.d2c.cn/{{value.coverPic}}" style="width:100%">
            </a>
            <div class="bargin-title flex">
            {{if statued==0 && value.status==1}}
                <p class="count-down" data-type="split-time"  data-endtime="{{value.beginDate}}" data-musttime="must">距离开始仅剩:<span class="white">
                <span class="hour down-time">00</span> <span class="time-split">:</span>
                <span class="minute down-time">00</span><span class="time-split">:</span>
                <span class="second down-time">00</span></span></p>
            {{else if statued==1 && value.status==2}}
                <p class="count-down" data-type="split-time"  data-endtime="{{value.endDate}}" data-musttime="must">距离结束还剩:<span class="white">
                <span class="hour down-time">00</span> <span class="time-split">:</span>
                <span class="minute down-time">00</span><span class="time-split">:</span>
                <span class="second down-time">00</span></span></p>
            {{else if (statued==1 && value.status==0) || statued==0 && value.status==0}}
                <p>活动已砍完</p>
            {{/if}}
            {{if statued==1 && value.status!=1}}
                <p><span class="white">{{value.virtualMan+value.actualMan}}</span>人参与</p>
            {{/if}}
            </div>
        </div>
        <div class="bargin-info">
            <a href="/bargain/detail/{{value.id}}">{{value.name}}</a>
        </div>
        <div class="bargin-price">
            <p class="float:left;">
            	<span class="fz-14" style="color:rgba(0,0,0,.54)">底价</span><span style="font-weight:bold;padding-left:5px;">¥&nbsp;</span><span class="fz-20" style="font-weight:bold;color:rgba(0,0,0,.87)">{{value.minPrice}}</span>
            	<span class="origin" style="text-decoration:line-through;padding-left:15px;">&nbsp;原价&nbsp;¥&nbsp;{{value.product.minPrice}}</span>
            </p>
            {{if statued==1 && value.status==0}}
                <button class="bargin-but but-grey" disable onclick="javascript:alert('该商品砍完啦，请砍价其他商品哟~')">已砍完</button>
            {{else if statued==0&&  value.status==1}}
                <button class="bargin-but remind" data-id="{{value.id}}">提醒我</button>
            {{else if statued==0 &&  value.status==0}}
                <button class="bargin-but but-grey" disable  onclick="javascript:alert('该商品砍完啦，请砍价其他商品哟~')">已砍完</button>
            {{else if statued==0&&  value.status==-1}}
                <button class="bargin-but but-grey" disable >已下架</button>
            {{else if statued==1 && value.status==2}}
                <a class="bargin-but"  href="/bargain/detail/{{value.id}}" >去砍价</a>
            {{/if}}
        </div>
    </div>
    {{/each}}
...
var page_elem={
	init:function(){
        $.get('/ad/MAIN/BARGAIN.json',function(data){
            if (data.adResource){
                if (data.adResource.url!=''){
                    var html='<a href="'+data.adResource.url+'"><img src="//img.d2c.cn'+data.adResource.pic +'" style="width:100%;"/></a>';
                }else{
                    var html='<a href="javascript:" data-url="'+data.adResource.url+'" class="ajax-request" data-type="get"><img src="//img.d2c.cn'+data.adResource.pic +'" style="width:100%;"/></a>';                
                }
                $('.slide-class').html(html);
            }
        });
        this.lisen();
        this.nav();
		...
		$(document).on(click_type,'.nav-itemss',function(){
			...
			$.getJSON(url,function(res){
				...
				if(status == 1){
					...
					$('.bargain-load-more').show().utilScrollLoad();
				}else{//我的砍价详情
...
function gopay(){
	...
	<#else>
		location.href='/member/order';TDAPP.onEvent('砍价商品购买');
	</#if>
}
