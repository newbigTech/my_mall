...
<div class="section">
    ...
    <div class="form">
        <form action="/member/refund" class="validate-form" redirect-url="/member/order/${refund.orderSn}" method="POST">
            ...
            <div class="form-item item-flex">
                <label>支付宝账号</label>
                <input type="text" name="backAccountSn" value="${refund.backAccountSn}" class="input validate" placeholder="退款仅支持退到支付宝账号" title="支付宝账号" />
            </div>
            <div class="form-item item-flex">
                <label>支付宝名称</label>
                <input type="text" name="backAccountName" value="${refund.backAccountName}" class="input validate" placeholder="支付宝实名认证" title="真实姓名" />
            </div>
            <#else>
            <div style="padding:1em 0.8em 0.8em;color:#FD555D;font-size:0.8em;">该笔退款原路返回您的${(refund.orderPayTypeName)!}</div>
            </#if>
            <div class="form-item item-flex">
                <label>退款金额</label>
                <input type="text" class="input money validate" title="退款金额" name="applyAmount" id="total-amount" data-limit="${orderItem.actualAmount}" data-function="checkAmount" style="width:3em;" value="${(refund.applyAmount)!}" placeholder="" /> 元(可退 ${orderItem.actualAmount} 元)
            </div>
