...
<section class="share-type">
	...
	<i class="pop-close" onclick="$(this).parent().parent().hide()">
	  ...
	  <div class="friend"><i class="icon-wx"></i><p>发给好友</p></div>
	  <div class="billcreat"><i class="icon-hb"></i><p>生成海报</p></div>
	</div>
</section>
<div class="bargin-section">
	...
	<div class="bargin-productinfo" style="position:relative;">
		<a href="/page/1212miaowujie" class="pro-go" style="display:none"><img src="//static.d2cmall.com/img/promo/bargin/icon_enter.png" style="width:100%;"></a>
        <div class="barpro-title">${bargainPriceDO.bargain.name}</div>
			<#if bargainPriceDO.statusName=='砍价成功' >
			<div style="margin-top:8px;margin-bottom:20px;padding-left:15px;font-size:14px;rgba(35,36,39,.54)">该商品价格已经砍到<span style="font-size:16px;color:#F23365;">最低</span></div>
			<#elseif bargainPriceDO.statusName=='砍价中' >
			<div class="bar-times on count-down" data-type="split-time"  data-endtime="${bargainPriceDO.bargain.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-musttime="must"><span>距离结束时间</span><span class="n-time  hour down-time">00</span><span class="k-time">时</span><span class="n-time  minute down-time">00</span><span class="k-time">分</span><span class="n-time second down-time">00</span><span class="k-time">秒</span> </div>
	    ...
	    <#elseif bargainPriceDO.statusName=='砍价成功'>
	     ...
	     <#elseif bargainPriceDO.statusName=='支付商品超时'>
	     	 <#if m.LOGINMEMBER.id==bargainPriceDO.memberId>
	     	  <div class="bargin-dobutton">
		...
		<div class="more-bargain">
			<div class="bargin-list-title">
				...
				<p ><a href="/bargain/promotion/list?status=1" onclick="TDAPP.onEvent('前往砍价商城')" style="color:#FD555D;font-size:12px;padding-right:15px;">砍价商城>>>></a></p>
			</div>
		...
		<div class="app-download-bar">
            <div style="background:#FFF;border-bottom:1px solid #EEE;">
                <div class="bar-button">
                    <a href="//m.d2cmall.com/page/follownumber" class="open-app" style="background:#FFF;border:1px solid #000;color:#000;margin-top:-5px;box-shadow:none;" onclick="TDAPP.onEvent('立即关注');">立即关注</a>
                </div>  
                ...
                <div class="bar-cont open-app" href="javascript:void(0)" mlink-handling="true">
                    <div>
                        <div><p><strong>D2C全球好设计</strong></p><p>关注公众号，领取700元新人礼包</p></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mybargin-list">
            ...
            <div class="mybargin-item">
                ...
                <div class="flex usrinfo">
                   ...
                   <#if list.headPic>
				      <img src="<#if list.headPic?index_of('http') lt 0>//img.d2c.cn</#if>${list.headPic}" class="head-pic" />
...
<div class="product-detail" style="display:none">
    ...
    <div class="close_icon_v" onclick="javascript:$(this).parent().parent().parent().hide();"><image src="//static.d2c.cn/img/promo/bargin/icon_a_close.png" class="close_icon"></div>
        <form name="form-buy" id="form-buy" action="/order/bargain/buynow" method="post">
            ...
            <div class="product-sku clearfix num-crease">
                ...
                <div class="num-crease-wrap"><span class="num-input">1</span><input type="hidden" class="product-count" id="num" name="num" readonly value="1"></div>
            ...
            <div style="overflow:hidden;padding:3px;text-align:center">
                <#if product.mark==0>
                <button type="button" class="button disabled" disabled style="width:85%;">已下架</button>
                <#elseif product.store lte 0>
                <button type="button" class="button disabled" disabled style="width:85%;">已售完</button>
                <#else>
               <button type="button" data-type="buy" class="operator-button button button-red" style="height:40px;line-height:40px;width:85%;text-align: center;background: #000;color: #FFF;border:0;">立即购买</button>
                 </#if>
            </div>
			</#if>
			</div>
...
<a href="/page/918chudong"><img src="//static.d2c.cn/img/topic/180915/fresh/images/fresh.png" width="100%" id="nav" /></a>
<share data-title="${bargainPriceDO.bargain.shareTitle}" data-pic="${picture_base}${bargainPriceDO.bargain.sharePic}" data-desc="${bargainPriceDO.bargain.shareDes?replace('#price',(bargainPriceDO.price))}"></share>
<script>
	var page_elem={
		init:function(){
		...
		getbargainList:function(){
		  $.getJSON('/bargain/promotion/list.json?status=1',function(res){
				...
				$.each(list,function(i,d){
					str=str+'<li><a href="/bargain/detail/'+d.id+'"><img src="${picture_base}'+d.product.productImageCover+'!/both/1000x1400"><p class="title">'+d.product.name+'</p><p class="a-price">&yen;'+d.minPrice+'<span style="text-decoration:line-through;font-size:10px;color:#ccc;padding-left:10px;">&yen;'+d.product.minPrice+'</span></p></a></li>'
				})
		...
		size:function(){
			$.getJSON('/product/skuStore/${product.id}',function(res){
				...
				$.each(res.result, function(i,d){
					str=str+'<tr><td>'+d.color+'</td><td>'+d.size+'</td><td>'+d.store+'</td></tr>'
				});	
				$('.c-title').append(str)
			});	
		},
		...
		lisen:function(){
			$('.show-rule').on(click_type,function(){
                ...
                2、同一用户ID 24小时内只能帮砍3次 <br /><br />\
                3、同一用户ID一款砍价商品只能购买一次！同时只能成功支付3次砍价商品 <br /><br />\
                4、如遇到库存为0，还能发起砍价，这种情况是其它用户未付款，您可以继续分享，遇到有库存尽快购买！<br /><br />\
                5、砍价成功后请在5分钟内付款，过期订单将被关闭。<br /><br />\
                ...
                6、在活动中，如果用户出现违规行为（包括不限于作弊、扰乱系统、实施网络攻击等），D2C将取消违规用户的优惠购买资格，并有权终止其参与本活动。<br /><br />\
				</div>';
				popupModal({
					content:html
				});
					        	...
					        	var ratio=res.ratio;
			       				 if(res.result==0){
							 		...
							 		<div id="success-info" class="bargain-help-content">\
										...
										<div class="award-a"><p style="font-size:14px;padding-top:5px;color:#FFF;font-weight:700">520抽奖</p><p style="font-size:10px;margin-top:18px;color:#FFF;">每日一次抽奖机会</p></div>\
