...
<div class="header gradient fixed">
    ...
    <div class="header-btn">
    	<button type="button" onclick="location.href='/cart/list'" class="button-icon button-cart"><em class="bounceIn animated my-cart-num">0</em><i class="icon icon-cart-bar"></i></button>
...
<div class="section">
    <div class="swiper-container" id="product-slide">
        <div class="swiper-wrapper" style="align-items:center;">
            ...
            <div class="swiper-slide">
            	<a href="#"><img src="<#if image?index_of('http')!=-1>${image}<#else>${picture_base}${image}!600</#if>" /></a>
            </div>
            </#list>
        </div>
    ...
    <div class="product-base">
        <h1 class="title"><#if flashPromotion?exists && !productPromotion.flashPromotionOver><span style="color:#fd555d;font-size:.75rem;">【限时购】</span></#if> ${product.name}</h1>
        <#if productPromotion?exists>
          	...
          	<a href="/flashpromotion/product/session" class="red">${flashPromotion.name}</a>】</div>
         	<#elseif !productPromotion.promotionOver>
       	 	<div class="product-promotion" style="display:inline-block;color:#fd555d;font-size:.75rem;">【
          	<a href="/promotion/${productPromotion.promotionId}" class="red">${productPromotion.promotionName}</a>】</div>
         	</#if> 
     	</#if> 
        ...
        <div class="product-label">
            <label>购买价</label><span class="data-price">&yen; <strong class="current-price">${(productPromotion.currentPrice?string("currency")?substring(1))!}</strong></span>
        ...
        <#elseif (product.minPrice?exists && product.originalPrice?exists&& product.minPrice!=product.originalPrice)>
         <div class="product-label">
            <label>购买价</label><span class="data-price">&yen; <strong class="current-price">${(product.minPrice?string("currency")?substring(1))!}</strong></span>
            &nbsp;&nbsp;&nbsp;吊牌价 <s>&yen; ${product.originalPrice}</s>
        </div>
        <#if product.productTradeType == 'CROSS'>
        	<div style="margin-top:10px;font-size:12px;color:#7F7F7F;">
        	<#if product.taxation  == 1>
    ...
    <div class="share-recommender">
        <div class="display-flex space-between">
            <div class="share-income">分享最多可赚 <#if productPromotion?exists && (!productPromotion.promotionOver || !productPromotion.flashPromotionOver)>
            ${(productPromotion.currentPrice)*(product.secondRatio)*(product.grossRatio)}
            <#else>${(product.minPrice)*(product.secondRatio)*(product.grossRatio)}</#if>元</div>
            <div class="share-tool display-flex flex-end">
                <div><a href="/product/detail/${product.id}?type=summary"><i class="icon icon-package"></i><span>素材</span></a></div>
   ...
   <#if flashPromotion?exists && !productPromotion.flashPromotionOver>
    <div class="form">
   		...
   		<#if flashPromotion.statusName=='即将开始'>
	        <div class="limit-countdown count-down  float-right" style="font-size:.875rem"  data-type="split-time"  data-endtime="${flashPromotion.startDate?string("yyyy/MM/dd HH:mm:ss")}" data-musttime="must">
	            距开始&nbsp;<span class="hour down-time" style="float:none;">00</span>:<span class="minute down-time" style="float:none;">00</span>:<span class="second down-time" style="float:none;">00</span>
	        </div>
        <#else>
        	 <div class="limit-countdown count-down float-right" data-type="split-time" style="font-size:.875rem"  data-endtime="${flashPromotion.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-musttime="must">
	            距结束&nbsp;<span class="hour down-time" style="float:none;">00</span>:<span class="minute down-time" style="float:none;">00</span>:<span class="second down-time" style="float:none;">00</span>
	        </div>
        </#if>
   		</a>
    </div>
    ...
    <div class="form">
   		<a href="/showroom/designer/${(designer.id)!}" class="form-item">${(designer.name)!}<i class="icon icon-arrow-right right"></i><span class="grey float-right" style="font-size:12px;">进入设计师店铺</span></a>
    </div>
        ...
        <form name="form-buy" id="form-buy" action="/order/buynow" method="post">
            ...
            <div class="product-sku img clearfix" style="position:relative;">
                ...
                <#list product.salesproperty?eval.sp1 as p>
                	<a href="javascript:" id="c-${p.id}" class="color-section" data-type="color" data-id="${p.id}" rel="${p.value}" data-image="<#if p.img!=''>${picture_base}/${p.img}</#if>">${p.value}</a>
                </#list>
                </#if>
                </span>
            </div>
            ...
            <div class="product-sku txt clearfix">
                ...
                <#list product.salesproperty?eval.sp2 as p>
                	<a href="javascript:" id="s-${p.id}" class="size-section" data-type="size" data-id="${p.id}" rel="${p.value}">${p.value}</a>
                </#list>
                </span>
           	</div>
            </#if>
            ...
            <div class="product-sku clearfix num-crease">
                ...
                <div class="num-crease-wrap"><span class="num-decrease num-operation" id="decrease" data-type="decrease">－</span><span class="num-input">1</span><span class="num-increase num-operation" id="increase" data-type="increase">＋</span><input type="hidden" class="product-count" id="num" name="num" readonly value="1"></div>
            ...
            <#if productPromotion?exists>
            	...
            	<div class="product-times">
                    <div class="count-down red" data-type="toend" data-startTime="${productPromotion.flashStartDate?string("yyyy/MM/dd HH:mm:ss")}" data-endTime="${productPromotion.flashEndDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}"></div>              
                </div>
           		<#elseif !productPromotion.promotionOver>
                <div class="product-times">
                    <div class="count-down red" data-type="toend" data-startTime="${productPromotion.startDate?string("yyyy/MM/dd HH:mm:ss")}" data-endTime="${productPromotion.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}"></div>              
                </div>
                </#if>
       		 </#if>
			<#if product.status==3>
			<div class="product-alert">
                ...
                <input type="text" name="mobilemail" class="input remind-param mobile" value="<#if m.LOGINMEMBER.id!=null><#if (m.LOGINMEMBER.mobile)!=null>${m.LOGINMEMBER.mobile!}<#elseif m.LOGINMEMBER.email?exists>${m.LOGINMEMBER.email!}</#if></#if>" placeholder="输入手机号或邮箱" />
            ...
            <div class="suspend-bar" style="overflow:hidden;padding:3px">
            	<button type="button" class="button-icon" style="width:12%;float:left;" onclick="ysf.open()"><i class="icon icon-assistant"></i>客服</button>
                <#if product.mark==0>
                <button type="button" class="button disabled" disabled style="width:85%;float:right;">已下架</button>
                <#elseif store lte 0>
                <button type="button" class="button disabled" disabled style="width:85%;float:right;">已售完</button>
                <#else>
                ...
                <#if designer.subscribe==1 && product.subscribe == 1><button type="button" data-type="fitting" data-id="${product.id}" class="operator-button button-icon" style="width:14%;float:left;"><i class="icon icon-gift"></i>试衣</button></#if>
                <button type="button" data-url="/member/interest/collection/insert/${product.id}" id="collect-${product.id}" check-url="/member/interest/collection/check/${product.id}" class="button-icon ajax-request" call-back="interestSuccess(${product.id},'collect')" style="width:14%;float:left;"><i  class="icon <#if product.collectioned==1>icon-star<#else>icon-star-o</#if>"></i><#if product.collectioned==1>已收藏<#else>收藏</#if></button>
                <button type="button" data-type="buy" class="operator-button button button-red" style="width:${width}%;margin-left:2%;float:right;margin-right:2%;">立即购买</button>			
                <#if product.cart==1><button type="button" data-type="cart"class="operator-button button button-white" style="width:${width}%;float:right">加入购物车</button></#if>
                </#if>
            </div>
			</#if>
			</div>
...
<div class="section">
    ...
    <div class="list lazyload clearfix">
        ...
        <a href="${mobileBase}/product/${product.id}" class="item item-flex item-gap">
            <span class="img">
              <img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}${product.productImageCover}!280" alt="" />
             </span>
...
<share data-title="${product.name}" data-url="${mobileBase}/product/${product.id}<#if m.LOGINMEMBER.partnerId>?parent_id=${m.LOGINMEMBER.partnerId}</#if>" data-pic="${picture_base}${product.productImageList[0]}!wx.title"></share>
<script>
