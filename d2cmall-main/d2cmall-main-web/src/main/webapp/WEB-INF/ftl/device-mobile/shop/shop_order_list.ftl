...
<a href="/seller/order?orderStatus=WaitingForDelivery,WaitingForComfirm">待发货<span class="small">(${WaitingForDelivery})</span></a>&nbsp;&nbsp;
...
<div class="wrap-main">
	...
	<div class="order-item wrap-div" style="margin-bottom:0.6em;">
		<div class="item-main clearfix">
			...
			<p class="img-big">
				<img src="<#if orderItem?exists>${picture_base}/${orderItem.productImg}!80</#if>" style="min-height:5em;" alt="${orderItem.productName}" />
			...
			<p class="op-bar" style="font-size:0.8em;"><span class="float-left">共${orderItem.productQuantity}件</span> <strong class="price float-right">订单金额：&yen; ${(orderItem.totalPrice-orderItem.promotionAmount)?string("currency")?substring(1)}</strong></p>
		</div>	
	</div>	
	</#list>
	</#if>
	<#if pager.pageCount gt 1>
	<div id="load-more" class="load-more" data-page="${pager.pageNumber}" data-total="${pager.pageCount}">点击加载更多</div>		
	</#if>
