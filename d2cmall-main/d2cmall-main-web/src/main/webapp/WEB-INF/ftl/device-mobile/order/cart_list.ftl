	...
	<div class="form">
        <form name="form-cart" id="form-cart" action="/order/confirm" method="POST" style="width:100%">
            ...
            <div class="form-item item-card item-checkbox item-margin clearfix cart-disable">
            	...
            	<p class="promotion-name"><span class="one red"></span><span class="three red"></span><span class="two red"></span></p>	 
                <p class="img"><a href="/product/${cartItem.productId}"><img src="${static_base}/m/img/blank100x157.png" data-image="<#if cartItem.sp1?eval.img?index_of('http')!=-1>${(cartItem.sp1?eval.img)}<#else>${picture_base}/${(cartItem.sp1?eval.img)}</#if>" alt="${cartItem.productName}" /></a></p>
                <p class="title"><a href="/product/${cartItem.productId}">${cartItem.productName}</a></p>
                ...
                <p class="property">${cartItem.sp1?eval.name}：${cartItem.sp1?eval.value} <#if cartItem.sp2 != null>&nbsp;${cartItem.sp2?eval.name}：${cartItem.sp2?eval.value}</#if></p>
                <p class="amount"><span>吊牌价：<strong class="price original-l" data-price="${cartItem.originalPrice}">&yen;${cartItem.originalPrice}</strong></span>&nbsp;&nbsp;<span><strong class="price cut-price"></strong></span></p>  
                <#if cartItem.after==0><p class="amount"><span class="red">本件不支持七天无理由退换货</span></p></#if>
            </div>
            <#else>
            <div class="form-item item-card item-checkbox item-margin clearfix <#if (cartItem.productSku.availableStore gt 0) && (!cartItem.over)><#else>cart-disable</#if>" data-id="${cartItem.id}" data-cartType="available">
                <input type="hidden" name="cartItemId" value="${cartItem.id}" class="id" <#if (cartItem.productSku.availableStore gt 0) && (!cartItem.over)> <#else> disabled</#if> />
                ...
                <input type="checkbox" class="cartItem cartchecked"<#if (cartItem.productSku.availableStore gt 0) > checked<#else> disabled</#if>/>
               	<p class="promotion-name"><span class="one red"></span><span class="three red"></span><span class="two red"></span></p>	 
                <p class="img"><a href="/product/${cartItem.productId}"><img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}/${(cartItem.sp1?eval.img)!}!120" alt="${cartItem.productName}" /></a><#if (cartItem.productSku.availableStore gt 0) && (!cartItem.over)><#else><span style="position:absolute;width:100%;height:100%;background-color:#fff;filter:alpha(opacity=50);-moz-opacity:0.5;opacity:0.5;top:0;left:0;"></span></#if></p>
                <p class="title"><a href="/product/${cartItem.productId}"><#if cartItem.flashPromotion?exists && !cartItem.productPromotion.flashPromotionOver>
                <span style="color:#FD555D">【限时购】</span></#if>${cartItem.productName}</a></p>
                <#if cartItem.choiceOrderPromotions?exists && cartItem.choiceOrderPromotions?size gt 1>
                <div class="cart-select custom-select">
                   <select name="selectOrderPromotionId" class="input-s" style="width:50%">
                    <#list cartItem.choiceOrderPromotions as orderPromotion>
                ...
                <div class="bar clearfix">
                    <div class="float-right promotion-td" style="line-height:1em;font-size:1em;margin-top:.5em">共计：<strong class="price" style="font-size:1.2em"><em class="total-item red"></em></strong></div>
                    <div class="float-left" style="width:30%;"><a href="javascript:" data-url="/cart/delete/${cartItem.id}" confirm="确定要从购物车中移出该商品吗？" success-tip="商品从购物车中成功移出！" call-back="removeItem(${cartItem.id})" class="icon icon-trash ajax-request"></a></div>
                    <div class="float-left">
                        <#if cartItem.over>
                        <input type="hidden" name="quantity" value="0" size="2" class="quantity" disabled /><p style="margin-top:0.4em;font-size:1em;color:#FD555D!important">该件商品已经下架</p>
                        <#elseif (cartItem.productSku.availableStore lte 0 && cartItem.productSku.freezeStore gt 0)>
                        <input type="hidden" name="quantity" value="${cartItem.productSku.availableStore}" size="2" maxlength="3" class="quantity" disabled /><p style="margin-top:0.5em;font-size:0.8em;color:#FD555D!important">还有机会抢到</p>
                        <#elseif (cartItem.productSku.availableStore lte 0)>
                        <input type="hidden" name="quantity" value="0" size="2" class="quantity" disabled /><p style="margin-top:0.4em;font-size:1em;color:#FD555D!important">该件商品已经卖完了</p>
                        <#elseif (!cartItem.isMeetInventory(cartItem.productSku.availableStore))>
                        <div class="num-crease-wrap"><span class="num-decrease">－</span><span class="num-input">${cartItem.productSku.availableStore}</span><span class="num-increase">＋</span><input type="hidden" name="quantity" value="${cartItem.productSku.availableStore}" size="2" readonly maxlength="3"></div>
                        <div class="check-tip" style="margin-left:-1.3em;">目前库存只剩${cartItem.productSku.availableStore}件</div>
                        <#else>
                        <div class="num-crease-wrap"><span class="num-decrease">－</span><span class="num-input">${cartItem.quantity}</span><span class="num-increase">＋</span><input type="hidden" name="quantity" value="${cartItem.quantity}" size="2" readonly maxlength="3"></div>
                        </#if>
                    </div>
                </div>
            </div>
            ...
            <div class="suspend-bar" style="border-bottom:1px solid #F0F0F0">
                ...
                <div class="desc">总计：<strong class="price">&yen;<span class="totle-price">${(cart.totalPrice)!}</span>元</strong></div>
                <div class="buttons"><button type="button" id="submit-button" class="button button-red">结算(<span id="total-amount"></span>)</button></div>
            </div>
        </form>
    </div>
    </#if>
...
<script id="recomment-template" type="text/html">
    ...
    <a href="/product/{{value.id}}" class="item item-flex item-gap">
      <span class="img"><img src="${picture_base}{{value.img}}!300" alt="{{value.name}}" />  {{if value.store < 1}}<i class="n-product"></i><span class="outp">已售罄</span>{{/if}}</span>
