...
<@m.page_header title='买家秀评论审核'  keywords="设计师现货区,全球好设计,D2C,Designer To Customer,原创设计,个性,原创设计概念店,设计师品牌集成店" description="消费者可以通过平台了解设计师的产品理念和创作灵感，树立设计师形象，提升设计师品牌影响力，扩大设计师产品市场"/>
 <div class="category-search" style="padding:.5em 0.8em;height: 2.6em;">
        <form name="" action="/membershare/comment/list" style="width:100%;margin-left:0;">
            ...
            <input type="search" name="content" placeholder="输入评论内容关键字" class="input" value="" style="padding-left:2em;height: 2.5em;line-height: 1.6em;"/>
        </form>
    </div>
...
<div class="form clearfix">
	<div class="form-item" style="font-size:14px;line-height:180%;">会员ID：${comment.memberId}
		<div class="switch" style="float:right;margin-top:-.2em"> 
			<div class="btn_fath clearfix <#if comment.blockShareComment==0>off<#elseif  comment.blockShareComment==1>on</#if>" onclick="toogle(this)"> 
				<div class="move" data-state="<#if comment.blockShareComment==0>off<#elseif  comment.blockShareComment==1>on</#if>" data-id="${comment.id}" data-url="/membershare/blockShareComment/${comment.memberId}?status="></div> 
	...
	<div class="form-item" style="color:#666;font-size:12px;line-height:180%;">${comment.createDate}
			<div class="switch" style="float:right;margin-top:-.4em"> 
				<div class="btn_fath clearfix <#if comment.verified==false>off<#elseif  comment.verified==true>on</#if>" onclick="toogle(this)"> 
					<div class="move" data-state="<#if comment.verified==false>off<#elseif comment.verified==true>on</#if>" data-id="${comment.id}" data-url="/membershare/comment/verify/${comment.id}?verified="  ></div> 
					<div class="btnSwitch btn1"></div> 
					<div class="btnSwitch btn2"></div> 
				</div> 
			</div> 
	...
	<div class="form-item" style="font-size:14px;line-height:150%;word-wrap: break-word;">${comment.content}</div>
