...
<div class="section">
    ...
    <div class="form">
        <form action="/member/exchange" class="validate-form" redirect-url="/member/order/${exchange.orderSn}" method="POST">
            ...
            <div class="form-item item-flex">
                ...
                <select class="input validate" name="exchangeReason" id="exchange-reason" title="换货原因">
                    ...
                    <option value="noReason"<#if exchange?exists && exchange.exchangeReason == 'noReason'> selected</#if>>七天无理由换货</option>
                </select>
            </div>
            <div class="form-item item-flex">
                <label>换货说明</label>
                <input type="text"title="换货说明" class="input validate" placeholder="备注及说明" name="memo" value="${(exchange.memo)!}" />
            </div>
