...
<share data-title="${partnerStore.name}" data-url="${base}/partner/store?partnerId=${partnerStore.partnerId}&parent_id=${partnerStore.partnerId}" data-pic="${partnerStore.pic}"></share>
<div class="shop-head flex space-around">
  	<div class="flex">
    	<div class="shop-logo"><img src="<#if partnerStore.pic><#if partnerStore.pic?index_of('//img.d2c.cn') == -1 && partnerStore.pic?index_of('https') == -1 &&partnerStore.pic?index_of('//static.d2c.cn') == -1>//img.d2c.cn<#else>${partnerStore.pic}</#if><#else>//static.d2c.cn/img/home/160627/images/headpic.png</#if>" style="width:5em;height:5em;"></div>
		<div class="shop-text">
...
<div class="lazyload">
    ...
    <div class="load-more" data-url="" data-target="load-more-product" template-id="list-template" data-page="" data-total="">点击加载更多</div>  
</div>
<script id="list-template" type="text/html">
    {{each list as value i}}
        <div class="item item-horizontal">
            <div class="img">
                <a href="/product/{{value.id}}?parent_id=${partnerStore.partnerId}"><img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}{{value.productImageCover}}!300" alt="" /></a>
            </div>
            <div class="info">
                ...
                <#else>
                <p><span class="price">&yen; {{value.currentPrice}}</span>&nbsp;&nbsp;{{if (value.currentPrice < value.originalPrice)}}<s>&yen; {{value.originalPrice}}</s>{{/if}}</p>
                </#if>
            </div>
        </div>
    {{/each}}
