...
<script id="limit-list" type="text/html">
	<div class="limit-title flex"  style="padding:0 15px;">
		...
		{{if brands[0].statusName=="即将开始"}}
			<div class="limit-countdown count-down" data-type="split-time"  data-endtime="{{brands[0].startDate}}" data-musttime="must">
				距开始&nbsp;<span class="hour down-time">00</span>:<span class="minute down-time">00</span>:<span class="second down-time">00</span>
			</div>
		{{else if brands[0].statusName=="抢购中" || (brands[0].statusName=="已开抢"&& !brands[0].end)}}
			<div class="limit-countdown count-down" data-type="split-time"  data-endtime="{{brands[0].endDate}}" data-musttime="must">
  ...
  {{each brands as value i}}
	<div class="limit-brand-item">
        <div class="limit-brand-img" style="position: relative">
           <a href="/flashpromotion/products/list?id={{value.id}}">
            <img src="${picture_base}{{value.brandPic}}" style="width:100%">
           </a>
        </div>
        <div class="swiper-container swiper-container-horizontal swiper-container3" >
            <div class="swiper-wrapper" style="margin-top:.3rem;">
                 {{each value.products as product i}}
                    <div class="limit-brand-product swiper-slide">
                        <a href="/product/{{product.id}}">
                            <img src="${picture_base}{{product.productImageCover}}!300" style="width:100%;">
                            <p class="text-center">&yen;{{product.flashPrice}}&nbsp;&nbsp;<span class="grey small" style="text-decoration:line-through;font-weight:normal">&yen;{{product.minPrice}}</span></p>
                        </a>
                     </div>
                {{/each}}  
            </div>
