...
<share data-title="${designer.name}" data-url="${base}/showroom/designer/${designer.id}?parent_id=${m.LOGINMEMBER.partnerId}"></share>
<div class="designer-info">
    <div class="logo">
        <a href="javascript:void(0);" data-url="/member/interest/attention/insert/${designer.id}" check-url="/member/interest/attention/check/${designer.id}" id="follow-${designer.id}" call-back="interestSuccess(${designer.id},'follow')" class="ajax-request fav"><span class="icon icon-heart-o"></span> <strong>${designer.tfans}</strong></a>
        <img src="${picture_base}${(designer.headPic)!}" id="designer-logo" style="max-width:80%;max-height:5em;" alt="${designer.name}" />
    </div>
    <div class="text">
        ${designer.intro}
    </div>
...
<div class="section lazyload showroom">
    ...
    <div class="list clearfix">
        ...
        <a href="/product/${product.id}" class="item item-flex item-gap">
            <span class="img">
              <#if list.store?exists && list.store lt 1><i class="n-product"></i><span class="outp">已售罄</span></#if><img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}${product.productImageCover}!280" alt="${product.name}" /></span>
            <span class="title">${product.name}</span>
            <span class="price">&yen; ${(product.promotionPrice)!}  <#if product.promotionPrice < product.originalPrice> &nbsp;<s>&yen; ${(product.originalPrice)!}</s></#if></span>
        </a>            
        </#list>
    </div>
    </#if>
...
<script id="item-template" type="text/html">
	<div class="lazyload">
	    ...
	    <a href="/product/{{value.id}}<#if m.LOGINMEMBER.partnerId>?parent_id=${RequestParameters.parent_id}</#if>" class="item item-flex item-gap">
	       <span class="img">{{if value.store<=0}}<i class="n-product"></i><span class="outp">已售罄</span>{{/if}}<img src="${picture_base}{{value.productImageCover}}!300" alt="{{value.name}}" /></span>
