...
<div class="section">
    <div class="padding" style="margin-bottom:-0.6em"><button type="button" name="add-new-address" class="button button-l button-red add-new-address" style="width:100%;">新增地址</button></div>
    ...
    <#list addresses as address>
        <div class="form-item item-address item-margin" style="padding-left:1em;" data-id="${address.id}">
            ...
            <div class="text-right" style="margin-top:0.6em;">
                ...
                <#else>
                <button type="button" data-url="/address/default/${address.id}" confirm="确定要设置为默认地址吗？" method-type="post" class="button button-outline ajax-request">设为默认</button>
                </#if>
                <button type="button" data-url="/address/update" request-url="/address/${address.id}" template-id="address-edit-template" class="button button-outline ajax-request">修改</button>
                <button type="button" data-url="/address/delete/${address.id}" confirm="确定要删除该地址吗？" method-type="post" call-back="removeItem(${address.id})" class="button button-outline ajax-request">删除</button>
            </div>
        </div>
    </#list>
    <#else>
...
<script id="address-new-template" type="text/html">
    ...
    <div class="form" id="address-form">
        <form class="validate-form" action="/address/insert" success-tip="地址添加成功！" method="post" id="add_address_form">
            <div class="form-item">
                <input type="text" name="name" placeholder="收货人姓名" data-rule="byte"  class="input validate"  max-length="10" min-length="2" title="收货人姓名" />
            </div>
            <div class="form-item">
            ...
            <div class="form-item">
                <input type="text" name="mobile" placeholder="收货人手机号" min-length="5" max-length="20" class="input validate" title="收货人手机号"  onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
            </div>
            <div class="form-item">
            ...
            <div class="form-button">
                ...
                <button type="button" name="submit-button" id="add-submit-button" class="button button-l button-red">确定</button>
            </div>
        </form>
    </div>
    <script>
...
<script id="address-edit-template" type="text/html">
    ...
    <div class="form" id="address-form">
        <form class="validate-form" action="/address/update" success-tip="地址编辑成功！" method="POST" id="update_address_form">
            <input type="hidden" name="id" value="{{address.id}}" />
            <div class="form-item">
                <input type="text" name="name" placeholder="收货人姓名" value="{{address.name}}" data-rule="byte" class="input validate"  max-length="10" min-length="2" title="收货人姓名" />
            </div>
            <div class="form-item">
                <select name="regionPrefix" class="input province validate" rel="{{address.province.code}}" title="省份" style="width:32%;"> 
                <option value="">选择省份</option>
                </select> 
                <select name="regionMiddle" class="input city validate" rel="{{address.city.code}}" title="城市" style="width:32%;"> 
                <option value="">选择城市</option> 
                </select>
                <select name="regionSuffix" class="input district validate" rel="{{address.district.code}}" title="区县" style="width:32%;"> 
                <option value="">选择区县</option> 
                </select>
            </div>
            <div class="form-item">
                <input type="text" name="street" placeholder="路名或街道地址，门牌号" value="{{address.street}}" class="input validate" title="详细地址" min-length="4" />
            </div>
            <div class="form-item">
                <input type="text" name="mobile" placeholder="收货人手机号" min-length="5" max-length="20" value="{{address.mobile}}" class="input validate" title="收货人手机号" onkeyup="(this.v=function(){this.value=this.value.replace(/[^0-9-]+/,'');}).call(this)" onblur="this.v();" />
            </div>
            <div class="form-item">
                &nbsp;<input type="checkbox" name="isdefault" value="true" id="isdefault"{{if address.isdefault}} checked{{/if}} /><label for="isdefault">设为默认地址</label>
            </div>
            <div class="form-button">
            	<input type="hidden" name="longitude" value="" />
            	<input type="hidden" name="latitude" value="" />
                <button type="button" name="submit-button" id="update-submit-button" class="button button-l button-red">确定</button>
            </div>  
        </form>
    </div>
    <script>
