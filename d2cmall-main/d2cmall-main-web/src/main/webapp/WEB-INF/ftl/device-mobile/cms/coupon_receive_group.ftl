...
<div class="top-downloadl" >
	<div style="position:fixed;z-index:22;width:100%;" class="img-downl">
		<a href="//t.cn/RLstL36" style="display:block;"><img src="//static.d2c.cn/img/other/download.png" style="width:100%;" /></a>
	</div>
</div>
 <#if result.datas.coupon.wapCode?exists && result.datas.coupon.wapCode!="">${result.datas.coupon.wapCode}<#else>//static.d2c.cn/img/topic/160826/testest.png</#if> 
</div>
<div class="section">
    <div class="text-center">
     <#if (result.datas.coupon.over)>
      ...
      <#else>
       	...
       	<form id="coupon-form" class="validate-form" action="${result.datas.receiveUrl}" method="post" call-back="receviesuccess" success-tip="false" data-go="${result.datas.coupon.redirectUrl}">
     	  <div class="coupon-main clearfix" style="height:80px">
            <div class="coupon-right" style="text-align:center;width:100%;height:100%;background-image:url(//static.d2c.cn/common/nm/img/coupon-batch.png);background-size:cover;background-color:#000">
                <p style="font-size:15px;color:#000;padding-left:35%;width:65%;height:20px">${result.datas.coupon.name}</p>
                <p style="font-size:10px;color:#999;padding-left:35%;width:65%;">有效期：${(result.datas.coupon.enabledate?string("yy/MM/dd"))!} - ${(result.datas.coupon.expiredate?string("yy/MM/dd"))!}</p>
            </div>
	    ...
	    <#else>
           <form id="coupon-form" class="validate-form" action="${result.datas.receiveUrl}" method="post" call-back="receviesuccess" success-tip="false" data-go="${result.datas.coupon.redirectUrl}">
            <div class="form-item">
    	        ...
    	        <input type="text" name="mobile" id="mobile" value="" class="input validate validate-account" title="手机号码" placeholder="请输入手机号码" style="height:40px;display:block;width:100%;border-radius:3px;text-align:center;font-size:15px;margin-bottom:10px;padding:12px 0;line-height:120%"/>
	       ...
	       <div class="coupon-button" style="margin-top:10px;">
	            <button class="button button-red" type="submit" style="width: 100%;height:40px;line-height:40px;font-size: 15px;border-radius:3px">立即领取</button>
	        </div>
        </#if>
       </form>
    </div>
