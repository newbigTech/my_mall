...
<div class="section">
	...
	<div style="font-size:0.8em;padding:2em 0.6em 1em 1em;text-align:center;">
	   <button type="button" class="button button-l button-white" onclick="$('#convert-coupon').toggle();">如果您有优惠券密码，点此兑换</button>
	</div>
	...
	<div class="form display-none" id="convert-coupon" style="margin-top:-0.6em;">
		<form method="post" action="/coupon/convertCoupon" class="validate-form form-item">
			<input type="text" name="password" class="input validate" title="优惠券密码" style="width:70%;" placeholder="请输入优惠券的密码" /><button type="submit" name="convert" class="button button-red float-right">兑换</button>
		</form>
	</div>
	...
	<div class="coupon-item">
        <div class="coupon-main clearfix">
            ...
            <div class="coupon-right">
                <p class="coupon-title">${coupon.name} <#if (RequestParameters.status=='UNCLAIMED' || RequestParameters.status=='CLAIMED') && coupon.transfer><span class="transfer" data-id="${coupon.code}">转赠</span></#if></p>
                <p>有效期：${(coupon.enableDate?string("yy/MM/dd"))!} - ${(coupon.expireDate?string("yy/MM/dd"))!}</p>
            </div>
        </div>
        <div class="coupon-other">
            <#if coupon.consumesTime?exists><div>消费时间 ${coupon.consumesTime?string("yy/MM/dd HH:mm")}</div></#if>
        ...
        <div class="coupon-tip <#if coupon.status=='INVALID'>is-expired<#elseif (coupon.status=='LOCKED' || coupon.status=='USED')>is-used<#elseif (coupon.status=="UNCLAIMED")>is-unclaimed</#if>">
        	<#if coupon.redirectUrl?if_exists && coupon.redirectUrl!='' && coupon.status=="CLAIMED"><a href="${coupon.redirectUrl}" class="button button-blue button-l" style="padding:0 1.1em;margin-top:10px;">使用</a></#if>
        </div>
        </#if>
        ...
        <div class="coupon-button">
            ...
            <a href="javascript:" data-url="/coupon/transfer" data-param="{'code':'${coupon.code}','amount':'${coupon.amount}'}" data-type="post"template-id="trans-coupon-template" class="button button-purple ajax-request trans-coupon">转让</a>
            </#if>
        </div>
        </#if>
    </div>
    ...
    <div class="section">
        <div class="form">
            <form name="" confirm="确定要转让优惠券码？" success-tip="恭喜您，优惠券转让成功！" class="validate-form" action="{{url}}" method="post">
                ...
                <div class="form-item">
                    <input type="text" name="loginCode" id="coupon-code" value="" placeholder="请输入对方的登录账号" class="input validate" >
                </div>
                <div id="user-info">
                </div>
...
<div style="font-size:0.7em;padding:1em;line-height:150%;">
    <p>{{nickname}} {{mobile}}</p>
    <p>核对以上信息准确无误后再进行转让！</p>
</div>
</script>
<script>
