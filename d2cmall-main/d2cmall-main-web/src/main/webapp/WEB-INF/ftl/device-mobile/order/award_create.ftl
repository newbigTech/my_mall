    ...
    <header>
        <div class="header">
            ...
            <div class="header-btn">
                <button type="button" onclick="modalAddressNew()" class="button button-s button-blue" id="add-new-address-button">新增</button>
            </div>
        </div>
    </header>
    	...
    	<form name="award-form" class="validate-form" action="/award/create" call-back="createSuccess()" method="post">
    		...
    		<div class="form" id="address-form">
                ...
                <div class="form-item item-address select-address" data-id="${address.id}">
                    <label style="width:100%;"><input type="radio" name="addressId" value="${address.id}"<#if address.isdefault> checked</#if> />
                    <p>${address.name} &nbsp;&nbsp;&nbsp; ${address.mobile}</p>
                    <p>${(address.province.name)!}${(address.city.name)!}${(address.district.name)!}${(address.street)!}</p></label>
                </div>
                </#list>
                </#if>
