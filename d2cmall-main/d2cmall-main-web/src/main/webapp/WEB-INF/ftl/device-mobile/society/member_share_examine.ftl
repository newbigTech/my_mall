...
<div class="tab-holder swiper-container">
    <div class="tableshow swiper-wrapper">
        <a style="text-decoration:underline" class="swiper-slide  <#if RequestParameters.status=='0'>on</#if>" href="/membershare/mana/list?status=0">未审核</a>
        <a style="text-decoration:underline" class="swiper-slide <#if RequestParameters.status=='1'>on</#if>" href="/membershare/mana/list?status=1">已审核</a>
    </div>
</div>
</div>
...
<#if pager.list?size &gt; 0>
	...
	<div class="myshow-item" style="border:none;margin-bottom:.5em">
		...
		<div class="switch" style="float:right;"> 
			...
			<div class="btn_fath clearfix <#if minepage.status==0>off<#elseif  minepage.status==1>on</#if>" onclick="toogle(this)"> 
				<div class="move" data-state="<#if minepage.status==0>off<#elseif  minepage.status==1>on</#if>" data-id="${minepage.id}"></div> 
			...
			<form action="" id="form_${minepage.id}" method="get" <#if minepage.status==0> hidden = "hidden" </#if>> 
				<div style="color:#696969;font-size:12px;font-weight:700">标签</div>
				<input class="tag" shareId="${minepage.id}" name="bind-${minepage.id}" type="radio" value="1" <#if minepage.tags??><#list minepage.tags?split(",") as tag><#if tag=='1'>checked</#if></#list></#if> /><span style="color:#696969;font-size:5px">热门推荐</span>
				<input class="tag" shareId="${minepage.id}" name="bind-${minepage.id}" type="radio" value="6" <#if minepage.tags??><#list minepage.tags?split(",") as tag><#if tag=='6'>checked</#if></#list></#if> /><span style="color:#696969;font-size:5px">达人说</span>
			</form>
		</div>
	</div>
 </#list>
