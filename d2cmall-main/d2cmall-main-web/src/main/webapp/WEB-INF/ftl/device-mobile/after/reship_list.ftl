...
<div class="section lazyload">
   ...
   <#list pager.list as reship>
        <div class="form-item item-card item-margin clearfix">
            <div class="top">退款编号：${reship.reshipSn} <span class="float-right">${(reship.reshipStatusName)!}</span>
            </div>
            <a href="/member/reship/${(reship.id)!}" class="clearfix">
                <span class="img"><img src="${static_base}/m/img/blank100x157.png" data-image="<#if reship.sp1?exists>${picture_base}/${(reship.sp1?eval.img)!}!120</#if>" /></span>
            ...
            <div class="bar text-right">
                ...
                <a href="javascript:" data-url="/member/reship/cancel/${reship.id}.json" confirm="确定要取消退款退货申请中吗？" class="ajax-request button button-outline" method-type='post' style="margin-top:.3em">取消申请</a>
                </#if>
                 <#if reship.reshipStatus==2>
                <a href="/member/reship/logistic/${reship.id}" class="button button-red" style="float:right;margin-top:.3em">填写快递单号</a>
                </#if>                  
            </div>
        </div>    
    </#list>
