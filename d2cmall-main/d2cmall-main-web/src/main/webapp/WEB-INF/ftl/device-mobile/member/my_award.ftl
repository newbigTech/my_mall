	...
	<#list pager.list as list>
        <div class="form-item item-card item-margin clearfix">
            ...
            <a href="/award/${list.awardId}" class="clearfix">
                <span class="img"><img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}/${list.productImg}!120" alt="${list.awardTitle}" /></span>
                <span class="title">${list.award.name} <#if list.status gte 2><em class="green">中奖号码【${list.award.winNum}】</em></#if></span>
                <span class="property">金额：&yen; ${list.price?string('0.00')}</span>
                <span class="amount" style="word-wrap: break-word;word-break: normal;">幸运号码: <#if list.status == 2>${list.luckyNum?replace(list.winNum,'<strong class="red">'+list.winNum+'</strong>')}<#else>${list.luckyNum}</#if></span>
            </a>
            <#if list.status== 0>
