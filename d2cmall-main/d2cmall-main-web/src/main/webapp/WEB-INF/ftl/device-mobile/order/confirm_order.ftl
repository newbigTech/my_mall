...
<div class="section lazyload">
    <form id="confirm-form" class="validate-form" target="_self"  action="/order/create" method="post" >
        ...
        <div class="form">
           ...
           <#list orderItems?if_exists as orderItem>
            ...
            <div class="form-item item-card clearfix">
               <input type="hidden" name="skuId" value="${orderItem.productSkuId}" />
                ...
                <p class="img"><a href="/product/${orderItem.productId}"><img src="${static_base}/m/img/blank100x157.png" data-image="${picture_base}/${(orderItem.sp1?eval.img)!}!120" alt="${orderItem.productName}" /></a></p>
                <p class="title"><#if orderItem.cod==0>【<span style="color:red;">不支持货到付款</span>】</#if><a href="/product/${orderItem.productId}">${orderItem.productName}</a> <#if orderItem.promotionName gt 0><span>【${(orderItem.promotionName)!}】</span></#if><#if orderItem.orderPromotionName gt 0><span>【${(orderItem.orderPromotionName)!}】</span></#if></p>
                <p class="property">${orderItem.sp1?eval.name}：${orderItem.sp1?eval.value}&nbsp;&nbsp;&nbsp;<#if orderItem.sp2 && (orderItem.sp2?eval)?size gt 0 >${orderItem.sp2?eval.name}：${orderItem.sp2?eval.value}</#if><span>&nbsp;&nbsp;&nbsp;数量：
                	<#if orderItem.quantity gt orderItem.productSku.availableStore>
                     ...
                     <span class="check-tip" style="left:20%;margin:2em;">&nbsp;&nbsp;&nbsp;目前库存只剩${orderItem.productSKU.availableStore}件</span>
                    <#else>
                	...
                	<p class="property price">单价：<span class="red">&yen;${orderItem.productPrice-orderItem.promotionPrice}</span>
                	&nbsp;&nbsp;&nbsp; 原价：<span  class="red" style="text-decoration:line-through;">&yen;${orderItem.productPrice}</span>
                	&nbsp;&nbsp;&nbsp; 小计：<span class="red">&yen;${orderItem.totalPrice-orderItem.promotionAmount}</span>
                	 </p>
                <#else>
                	<p class="property price">单价：<span class="red">&yen;${orderItem.productPrice}</span>
                	 <#if orderItem.productPrice lt orderItem.originalPrice>
                	    &nbsp;&nbsp;&nbsp;<span  class="red" style="text-decoration:line-through;">&yen;${orderItem.originalPrice}</span>
                	 </#if>
                	&nbsp;&nbsp;&nbsp;小计：<span class="red">&yen;${orderItem.totalPrice-orderItem.promotionAmount}</span></p>
                </#if>
                <#if orderItem.after==0><p class="property"><span class="red">本商品不支持七天无理由退换货</span></p></#if>
            	<#if orderItem.promotionAmount gt 0><p class="amount"><span class="green">优惠${(orderItem.promotionAmount)!}元</span></p> </#if>
            </div>
            <#else>
           <#assign exceptions=exceptions+1 />
            ...
            <div class="form-item item-card item-excption clearfix">
                <p class="img"><img src="${picture_base}/${(orderItem.sp1?eval.img)!}!60" alt="${orderItem.productName}" /></p>
                <p class="title">${orderItem.productName}</p>
                <p class="property">${orderItem.sp1?eval.name}：${orderItem.sp1?eval.value}&nbsp;&nbsp;&nbsp;<#if  orderItem.sp2 && (orderItem.sp2?eval)?size gt 0 >${orderItem.sp2?eval.name}：${orderItem.sp2?eval.value}</#if></p>
                <p class="amount"><strong class="price"><#if !orderItem.productSku?exists || orderItem.productSku.status lte 0>该商品已下架
        ...
        <div class="form">
            <a href="javascript:" <#if ableCoupons?size gt 0 || disableCoupons?size gt 0>id="coupon-item"</#if> class="form-item">
            	    使用优惠券<#if ableCoupons><em class="red" style="font-size:12px;">${ableCoupons?size}张可用</em></#if> <i class="icon icon-arrow-right right"></i><#if ableCoupons><span class="red" style="font-size:12px;" id="coupon-selected">未选择</span><#else><span class="grey" style="font-size:12px;">暂无可用优惠券</span></#if>
            </a>
            <input type="hidden" id="coupons" name="coupons"  <#if ableCoupons><#list ableCoupons as coupon> <#if coupon_index lt 1> value="${coupon.code}" <#if coupon.type=="DISCOUNT">  rel="${coupon.amount/100}" data-type="DISCOUNT"<#else>rel="${coupon.amount}" data-type="" </#if> </#if></#list><#else> value="0" rel="0" </#if>   />
        </div>
        ...
        <div class="form">
            <div class="form-item item-flex"><label>商品金额</label><label class="red" id="product-price" rel="${(order.totalAmount+order.promotionAmount)}">&yen; ${(order.totalAmount+order.promotionAmount)?string("currency")?substring(1)} 元</label></div>
            <div class="form-item item-flex"><label>运费</label><label class="red">&yen; <em id="ship-fee" rel="${order.shippingRates}">${order.shippingRates?string("currency")?substring(1)}</em> 元</label></div>
            <div class="form-item item-flex"><label>满减优惠</label><label class="red">&yen; <em id="promotion-amount" rel="${order.promotionAmount}">${(order.promotionAmount)?string("currency")?substring(1)}</em> 元</label></div>   
            <div class="form-item item-flex"><label>优惠券</label><label class="red">&yen; <em id="coupon-amount"> 0 </em> 元</label></div>                 
        </div>
       <#assign cod=1/>
       <#list orderItems?if_exists as orderItem>
	       <#if orderItem.cod==0>
        ...
        <#if cod==0>
         ...
         <div class="form">
            <a href="javascript:;" class="form-item" id="invoices-change" style="font-size:.8em">
            	   开具发票 <i class="icon icon-arrow-right right"></i><span class="grey float-right invoices-text" style="font-size:12px;max-width:60%;">暂不需要</span>
            </a>
            <input type="hidden" name="drawee" value="" />
        </div>
        <div class="suspend-bar">
            <div class="desc">应付：
            	<span class="price">&yen; <strong id="now-pay-price" class="price" rel="${(order.totalAmount+order.shippingRates)}">${(order.totalAmount+order.shippingRates)}</strong> 元</span>
            </div>
            <input type="hidden" name="cod" value="${cod}" />
            <input type="hidden" name="longitude" value="" />
            <input type="hidden" name="latitude" value="" />
            <div class="buttons"><button type="button" id="submit-button" class="button button-red">去付款</button></div>
        </div>
    </form>
</div>
    ...
    <header class="modal-header">
        <div class="header">
            ...
            <div class="header-btn">
                <button type="button" onclick="location.href='/coupon/myCoupon?status=UNCLAIMED&status=CLAIMED'" class="button button-s button-red" style="font-size:0.8em;">兑换</button>
            </div>
        </div>
    </header>
        ...
        <div>
            <div class="form" id="coupons-enable" style="display:block;">
                ...
                <div class="form-item item-address" data-id="0">
                    <label style="width:100%;"><input type="radio" name="check-id" value="0" data-type="" rel="0"{{if selected=='0'}} checked{{/if}} />
                    <p>有钱任性，不使用优惠券</p>
                    <p>请注意优惠券的有效期哦</p>
                    </label>
                </div>
                ...
                <div class="form-item item-address" data-id="${coupon.code}">
                    ...
                    <label style="width:100%;"><input type="radio" name="check-id" value="${coupon.code}" data-type="DISCOUNT" rel="${coupon.amount/100}"{{if selected=='${coupon.code}'}} checked{{/if}} />
                    <p>${coupon.name}</p>
                    <p>${coupon.amount/10}折优惠(有效期至：${coupon.expireDate?string("yyyy年MM月dd日")})</p></label>
                    <#else>
                    <label style="width:100%;"><input type="radio" name="check-id" value="${coupon.code}" data-type="" rel="${coupon.amount}"{{if selected=='${coupon.code}'}} checked{{/if}} />
                    <p>${coupon.name}</p>
                    <p>现金优惠 ${coupon.amount}元(满${coupon.needAmount}减${coupon.amount})(有效期至：${coupon.expireDate?string("yyyy年MM月dd日")})</p></label>
                    </#if>
                </div> 
                </#list>
                </#if>
...
<script id="invoices-template" type="text/html">
    ...
    <section class="modal-body">
       	<div class="form">
             ...
             <div class="form-item item-flex">
                  <label>发票抬头</label><br><input type="text" name="text" placeholder="发票抬头" class="input validate mydrawee" style="background-color:#f2f2f2;padding-left:5px;" title="发票抬头" />
             </div>
        ...
        <button type="button" class="button button-l button-red" style="bottom:15px;width:90%;left:5%;position:absolute;" id="invoices-button">
        	确定
        </button>
    ...
    <div id="modal-d2c-card-list">
        <header class="modal-header">
            <div class="header">
                ...
                <div class="header-btn">
                    <button type="button" onclick="modalCardNew()" class="button button-s button-blue" style="font-size:0.7em;">新增</button>
                </div>
            </div>
        </header>
        <section class="modal-body">
            ...
            <div class="form" id="d2c-card-list">
                <div class="form-item item-address" data-id="0">
                    <label style="width:100%;"><input type="radio" name="check-id" value="0" rel="0"{{if selected=='0'}} checked{{/if}} />
                    <p>此次不使用D2C卡</p>
                    <p>请注意D2C卡的有效期哦</p>
                    </label>
                </div>
                ...
                <div class="form-item item-address" data-id="{{card.id}}">
                    <label style="width:100%;"><input type="radio" name="check-id" value="{{card.id}}" rel="{{card.available}}"{{if selected==card.id}} checked{{/if}} />
        ...
        <section class="modal-body">
            <div class="form">
                <form class="validate-form" action="/cashcard/convert" call-back="modalCardPostSuccess()" method="post">
                    <div class="form-item item-flex">
                        <label>卡号</label><input type="text" name="sn" placeholder="D2C卡编号" max-length="20" maxlength="20" class="input validate" title="D2C卡编号" />
                    </div>
                    <div class="form-item item-flex">
                        <label>密码</label>
                        <input type="text" name="password" placeholder="D2C卡密码" max-length="20" maxlength="20" class="input validate" title="D2C卡密码" />
                    </div>
...
<script id="address-place-template" type="text/html"> 
    ...
    <div class="form-item item-flex">
        <input type="text" name="mobile" placeholder="收货人手机号" class="input validate" min-length="4" maxlength="20" title="收货人手机号" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
    </div>
    <div class="form-item">
...
<script id="address-list-template" type="text/html">
    <div id="modal-address-list">
        <header class="modal-header">
            <div class="header">
                ...
                <div class="header-btn">
                    <button type="button" onclick="modalAddressNew()" class="button button-s button-blue" style="font-size:0.8em;">新增</button>
                </div>
            </div>
        </header>
        <section class="modal-body">
            ...
            {{each list as address i}}
                <div class="form-item item-address select-address" data-id="{{address.id}}">
                    <label style="width:100%;"><input type="radio" name="check-id" value="{{i}}"{{if id==address.id}} checked{{/if}} />
                    <p class="address-value">{{address.name}} &nbsp;&nbsp;&nbsp; {{address.mobile}}</p>
                    <p>{{address.province}}{{address.city}}{{if id!=0}}{{address.district}}{{/if}}{{address.street}}</p></label>
                </div>  
            {{/each}}
            </div>
        </section>
    ...
    <div id="modal-address-new" class="display-none">
        <header class="modal-header">
            <div class="header">
                <div class="header-back"><a href="javascript:modalAddressList()" class="icon icon-cross"></a></div>
                <div class="header-title">新增地址</div>
            </div>
        </header>
        <section class="modal-body">
           <div class="form">
                <form class="validate-form" action="/address/insert" call-back='modalAddressPostSuccess' method="post">
                <input type="hidden" name="id" value="" />
                    <div class="form-item">
                        <input type="text" name="name" placeholder="收货人姓名" data-rule="byte" class="input validate" title="收货人姓名" />
                    </div>
                    ...
                    <div class="form-item item-flex">
                        <input type="text" name="mobile" placeholder="收货人手机号" class="input validate" title="收货人手机号" min-length="5" max-length="20" data-rule="num"/>
                        <input type="hidden" name="nationCode" value="" class="mobile-code" />
                    </div>
                    <div class="form-item">
                        &nbsp;<input type="checkbox" name="isdefault" value="true" id="isdefault" /><label for="isdefault">设为默认地址</label>
                    </div>
...
<script id="address-select-template" type="text/html">
    <div class="form-item item-address" data-id="{{id}}" style="padding-right:2em;">
        <span class="icon icon-location"></span>
        <p>{{name}} &nbsp;&nbsp;&nbsp; {{mobile}}</p>
        <p class="address-value">{{province}}{{city}}{{district}}{{street}}</p>
        <span class="icon icon-arrow-right"></span>
...
<script>
<#if addresses?size gt 0>
var address_data=[<#list addresses as address>{'id':'${address.id}','name':'${address.name}','mobile':'${address.mobile}','province':'${address.province.name}','city':'${address.city.name}','district':'${address.district.name}','street':'${address.street}','isdefault':'${address.isdefault}'}<#if address_index lt (addresses?size-1)>,</#if></#list>]; 
<#else>
var html=template('address-place-template',{});
$('#address-list').append(html);
$('#address-list').utilSetArea(); 
</#if>
var card_data=[<#list cashCards as card>{'id':'${card.id}','code':'${card.code}','available':'${card.available}','amount':'${card.amount}','expireDate':'${card.expireDate?string("yyyy年MM月dd日")}'}<#if card_index lt (cashCards?size-1)>,</#if></#list>]; 
