...
<#if (summary.summaryText?size gt 0||summary.summaryImg?size gt 0||summary.summaryVideo?size gt 0)>
    ...
    <a style="display:inline-block;width:100%;height:3em;background:green;text-align:center;color:#FFF;line-height:3em;" href="//img.d2c.cn${val}">观看视频</a>
    </#list>
    </#if>
<#else>
...
<style>
.appproduct-detail img{width:100%!important;}
embed{width:100%}
</style>

<div class="appproduct-detail">
<#if product.introduction!=''  || product.introduction>
	<h4 class="product-detail-title text-center">商品介绍</h4><div style="padding:12px 4% 15px;color:#666;font-size:0.6em;line-height:20px;">${product.introduction}</div>
</#if>
<#if product.attributes || product.sizeJson || product.tryOnReportJson  || product.categoryBannerPic>

	 <#if product.categoryBannerPic>
		 <a href="${product.categoryBannerUrl}"><img src="${picture_base}/${product.categoryBannerPic}" style="width:100%!important;max-height:12rem"></a>
	 </#if>
