...
<@m.page_header  title='看设计师直播，就来D2C！ 我是设计师 ${result.datas.live.designerName}，这是我在D2C上的直播，快来看看！' js="live/swfobject|https://cdn.ronghub.com/RongIMLib-2.2.5.min.js |https://static.mlinks.cc/scripts/dist/mlink.min.js" service='false'  keywords="设计师现货区,全球好设计,D2C,Designer To Customer,原创设计,个性,原创设计概念店,设计师品牌集成店" description="看设计师直播，就来D2C！ 我是 设计师${result.datas.live.designerName}，，这是我在D2C上的直播，快来看看！"  hastopfix='false'/>
<style>
body{background: rgb(0, 0, 0); width: 100%; height: 100%; margin-top: 0px;}
.heightem{display:none}
.input:focus, .input:active { border: 0 slid #999;box-shadow:none;}
.watchinfo{position:absolute;top:60%;text-align:center;width:80%;left:50%;margin-left:-40%;color:#FFF;font-size:20px;display: -webkit-box; display: box;}
.watchinfo div{line-height:125%;-moz-box-flex: 3;-webkit-box-flex: 3;box-flex: 3;border: 0;text-align: center;}
.end-info{position:absolute;text-align:center;width:100%;}
</style>
<share data-pic="//static.d2c.cn/common/nm/css/img/app_icon.png"></share>
<#assign live = result.datas.live>
<#if live.mark==1>
<#if live.status==1 ||  live.status==2 ||  live.status== 0>
<input type="hidden" name="id" value="${live.id}" />
<div class="video">
	<div  id="player">
  		<video  id="video"  webkit-playsinline="true" x-webkit-airplay="true"  playsinline="true" x5-video-player-type="h5" x5-video-player-fullscreen="true" loop="loop" width="100%" preload="auto" style="background: rgb(0, 0, 0);object-fit: fill;" <#if live.cover>poster="${picture_base}${(live.cover)!}"</#if> >
  		<source type="application/x-mpegurl" src="<#if live.status==1>${live.hlsUrl}<#elseif live.status==2>${live.replayUrl}</#if>">
  		<source type="application/vnd.apple.mpegurl" src="<#if live.status==1>${live.hlsUrl}<#elseif live.status==2>${live.replayUrl}</#if>">
  		</video>
 	</div>
</div>
...
<div class="live-user-info">
 ...
 <div class="live-pause">
 	<div class="pause-wrap">
 		<a href="javascript:;" style="width:4rem;height:4rem;border-radius:50%;"><img src="//static.d2c.cn/common/nm/css/img/icon_live_play.png"  style="width:4rem;height:4rem;border-radius:50%;"  id="players"></a>
 	</div>
 </div>
...
<section class="user_live_room" style="min-width:100%">
	...
	<img src="${picture_base}${(live.cover)!}" style="width:100%;position:absolute;top:0;left:0;display:block;z-index:10;height:100%;filter: blur(10px);display:none" class="bg-all">
 	<div style="width:100%;background-color:#000;opacity:0.7;position:absolute;top:0;left:0;display:block;z-index:20;height:100%;display:none" class="all">
		<div>
			<#if live.headPic>
		      <img src="<#if live.headPic?index_of('http') lt 0>//img.d2c.cn</#if>${live.headPic}" style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em" />
		    <#elseif live.thirdHeadPic>
		       <img src="${live.thirdHeadPic}" class=""  style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em"/>
		    <#else>
		       <img src="//static.d2c.cn/img/home/160627/images/headpic.png"  style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em"></img>
	  	  </#if>
	  	  ...
	  	  <div  class="watchinfo">
	  	  	<div><p style="color:#FFF">${live.watched}</p><p style="font-size:0.6em">观看人数</p></div>
	  	  	<div><p style="color:#FFF">${live.vcount}</p><p style="font-size:0.6em">粉丝人数</p></div>
	  	  </div>
		</div>
	</div>
...
<script>
	...
	if(userAgents.indexOf('wifi')<0) setTimeout(function(){$.flashTip({position:'center',type:'normal',message:'您不在wifi环境'});},1500)
	    $('.live-pause').hide();
	}, false);
	...
	if(iOS){
		 if((isWeChat || isQQ) && 0 != $("#video").length) 
		 	{
	 			try{
	 				setTimeout(function(){$("#video")[0].play();document.addEventListener("WeixinJSBridgeReady", function () { $("#video")[0].play();}, false); },1000)
	 			}catch(err){
	 				console.log(err)
	 			}	
		 	} 
 ...
 /*
	function mShow(hlsData){
	
    var p = navigator.platform;
    if(p.indexOf("Win") >= 0)
        $("#player").css({"width":"100%","height":"485px"});
    else
        $("#player").css({"width":"100%","height":"9em"});
	...
	function videocre(){
		var videoHls="<#if live.status==1>${live.hlsUrl}<#elseif live.status==2>${live.replayUrl}</#if>"
	}
  */
// videocre();	
</script>
<script> 
	new Mlink({
		...
		inapp : false,
		params: {myUrl: '/zegolive/watch/${live.id}'}
	})
</script>
<#elseif live.status==-1>
	<img src="${picture_base}${(live.cover)!}" style="width:100%;position:absolute;top:0;left:0;display:block;z-index:10;height:100%;filter: blur(10px);" class="bg-all">
	<div style="width:100%;background-color:#000;opacity:0.7;position:absolute;top:0;left:0;z-index:20" class="all">
		<div>
			<#if live.headPic>
		      <img src="<#if live.headPic?index_of('http') lt 0>//img.d2c.cn</#if>${live.headPic}" style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em;z-index:12;" />
		    <#elseif live.thirdHeadPic>
		       <img src="${live.thirdHeadPic}" class=""  style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em"/>
		    <#else>
		       <img src="//static.d2c.cn/img/home/160627/images/headpic.png"  style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em"></img>
...
.blur {	filter: url(blur.svg#blur); -webkit-filter: blur(15px);-moz-filter: blur(15px);-ms-filter: blur(15px);    filter: blur(15px);filter: progid:DXImageTransform.Microsoft.Blur(PixelRadius=15, MakeShadow=false);}
.live-t{position:absolute;width:84%;left:50%;margin-left:-42%;margin-top:7.8125rem;text-align:center;}
.live-t .live-s{position:relative;width:4.5rem;height:4.5rem;border-radius:100%;margin:0 auto;}
</style>
<img style="width:102%;position:absolute;top:0;left:0;height:100%;margin-left:-1%;margin-right:-1%;" src="${picture_base}${(live.cover)!}"  class="blur">
<div class="live-t">
		<div  class="live-s">
		<#if live.headPic>
		      <img src="<#if live.headPic?index_of('http') lt 0>//img.d2c.cn</#if>${live.headPic}" style="border-radius:100%;width:100%;" />
		    <#elseif live.thirdHeadPic>
		       <img src="${live.thirdHeadPic}" class=""  style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em"/>
		    <#else>
		       <img src="//static.d2c.cn/img/home/160627/images/headpic.png"  style="width:4em;height:4em;position:absolute;border-radius:100%;left:50%;top:15%;margin-left:-2em"></img>
	  	 ...
	  	 <p style="margin-top:1.25rem;font-size:1.125rem;color:#FD5562;">${live.previewDate?string("MM月dd日  HH:mm")}开始直播</p>
	  	 <p>
</div>
		<a href="javascript:;" class="live-top d2cDownApp"  style="display:block;width:100%;position:absolute;bottom:0;"><img src="//static.d2c.cn/common/nm/css/img/live_share.jpg" style="width:100%"></a>
<#elseif live.status==-3 || live.status==-4 || result.status==-1>
	<header>
 ...
 {{each list as item i}}
  <div class="product-item" >
	 	...
	 	<div class="product-info">
	 		...
	 		<p class="p-price"><span><i class="f-a">&yen;&nbsp;</i>{{item.minPrice}}</span>{{if item.minPrice < item.originalPrice }}<s>&yen;&nbsp;{{item.originalPrice}}</s>{{/if}}</p>
	 	</div>
...
var chatRoomId = "${live.channelId}"; // 聊天室 Id。
var count = 0;// 拉取最近聊天最多 50 条。

//获取用户token
function getstatus(){
	$.get('/zegolive/token.json',function(data){
		getmember(data);
	})
...
//获取用户信息并登录融云
	function getmember(data){
		token=data.result.datas.token;
		nickanme=data.result.datas.nickName;
		userId=data.result.datas.memberId;
		if(data.result.datas.headPic==''){
			headpic="//static.d2c.cn/img/home/160627/images/headpic.png"
		}else{
			headpic=data.result.datas.headPic;
		}
		getRongIMClient()
	}
...
//发送消息 
	...
	function installMsg(data,num){
		//num 1文本 4通知 5礼物 
		var co=data.content,str,h=$('#message').height(),ih=$('#list-info').height(),i=$('.room-chat-item').size();
		...
		if(h<=ih){$('.room-chat-scroller').animate({scrollTop:hs+ih},400);}
		if(i==20){$('.room-chat-item:first').remove();}
		
	}
	//进入聊天室成功回调
	function enterSuccess(){
		$.ajax({
			'url':'/zegolive/in/${live.id}.json',
			'type':'post',
	...
	$('.gobuy').on('click',function(e){
		...
		timer=false;
		$.get("/product/recommend/list?liveId=${live.id}", function(e){
		if(e.result.status==1){
			if(e.result.list.length>0){
				var data=e;
   				var html=template('live-productlist',data.result);
	...
	$(document).on(click_type,'.forbuy',function(){
		...
		var data={message:'BUY',user:{icon:headpic,id:userId,name:nickanme},extra:{productId:id,productName:name}}
