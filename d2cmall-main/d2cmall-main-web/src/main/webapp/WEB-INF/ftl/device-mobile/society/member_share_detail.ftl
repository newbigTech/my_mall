...
<@m.page_header back="" hastopfix='true' title="买家秀详情"  button="false" cart="has-bar" js='utils/swipe.min'  service='false' description="${(memberShare.description)!}"/>
<style>
   ...
   <div class="theme-list">
	<div class="theme-item">
		...
		<div class="item-content">
			<#if memberShare.topicId gt 0><a href="/membershare/topic/${memberShare.id}"><span class="item-title">#${(memberShare.topicName)!}#</span></a></#if><span>${memberShare.description}</span>
		</div>
		</#if>
		<#if list.resourceType=='video'>
			<div class="item-img item-one item-video">
				<video width="100%" height="100%"  src="${picture_base}${memberShare.video}"  style="position:absolute;width:100%;height:100%;display:none;background:#000" type="video/mp4" class="videoarea" controls="controls"></video>
		...
		<div class="item-bar flex">
			<div>
				<span class="btn-like ajax-request" method-type="post" data-url="/membershare/like/insert/${memberShare.id}"  call-back="liked(${memberShare.id})" ><i class="<#if memberShare.liked==1>icon-liked<#else>icon-like</#if> like-${list.id}  mar5"></i><span class="lin">${memberShare.likeMeCount}</span></span>
				<span class="btn-comment"><a href="/membershare/${memberShare.id}"><i class="icon-comment mar5" style="margin-left:25px;"></i>${memberShare.commentCount}</a></span>
			</div>
			<div>
				<span class="btn-watch"><i class="icon-watchnum mar5" style="width:28.3px;"></i>${memberShare.watchCount}</span>
			</div>
		</div>
	</div>
