...
<div class="user-header-bar display-none">
	<div class="search-bar" style="position:absolute;background:#FFF;box-shadow:0 0.1em 0.4em #CCC;display:none;width:100%;top:2.8em;">
...
<div class="tab-holder">
    <div class="tab tab-suspend">
        ...
        <a href="/member/order?orderStatus=WaitingForPay"<#if searcher.orderStatus?exists&&searcher.orderStatus[0]=='WaitingForPay'> class="on"</#if>>待付款<#if WaitingForPay gt 0><em>${WaitingForPay}</em></#if></a>
        <a href="/member/order?orderStatus=WaitingForDelivery"<#if searcher.orderStatus?exists&&searcher.orderStatus[0]=='WaitingForDelivery'> class="on"</#if>>待发货<#if WaitingForDelivery gt 0><em>${WaitingForDelivery}</em></#if></a>
        <a href="/member/order?orderStatus=Delivered"<#if searcher.orderStatus?exists&&searcher.orderStatus[0]=='Delivered'> class="on"</#if>>待收货<#if Delivered gt 0><em>${Delivered}</em></#if></a>
...
<div class="section lazyload">
    ...
    <div class="form text-center">
    	如果发现订单丢失，请 <button type="button" id="get-back-order" <#if m.LOGINMEMBER.source=='WeixinGz'> data-text="本次通过微信登录，只能找回同一微信账号下的订单。" data-url="/member/getBackOrder/WeixinGz"<#elseif m.LOGINMEMBER.source=='QQ'> data-text="本次通过QQ登录，只能找回同一QQ账号下的订单。" data-url="/member/getBackOrder/QQ" </#if> class="button button-red">找回订单</button>
    </div>
    </#if>
        ...
        <div class="form-item item-card item-margin clearfix" data-id="${order.id}">
            ...
            <div class="top">下单时间：${order.createDate?string("yyyy/MM/dd HH:mm:ss")}
             ...
             <a href="/member/order/${order.orderSn}" class="clearfix">
                ...
                <span class="amount"><em class="unit">${orderItem.productPrice}</em><#if orderItem.originalPrice!=orderItem.productPrice><s>${(orderItem.originalPrice)?string("currency")}</s></#if></strong> x ${orderItem.productQuantity}&nbsp;&nbsp;实付：<strong class="price">&yen;${(orderItem.actualAmount / orderItem.productQuantity)}</strong>
               	<#if orderItem.estimateDate?exists><span style="margin-top:0.3em;display:block;">预计发货时间&nbsp;&nbsp;(${orderItem.estimateDate?string("yyyy/MM/dd")})</span></#if>
               	<em class="float-right">【 ${orderItem.appStatusName}&nbsp;】</em></span>
             </a>
             </#list>
            <div class="bar bar-text text-right"><span class="float-left" style="color:#FF3300;">【<#if order.paymentType==3>货到付款<#elseif order.paymentType==1>支付宝支付<#elseif order.paymentType==4||order.paymentType==5>银行转账<#elseif order.paymentType==7>钱包支付<#elseif order.paymentType==6||order.paymentType==8||order.paymentType==9>微信支付<#elseif order.paymentType==11>优惠券支付<#elseif order.paymentType==18>线下转账<#else>在线支付</#if>】</span>共有${order.orderItems?size}款商品 &nbsp;&nbsp;共计：<strong class="price">&yen; ${(order.totalPay)}</strong></div>
            <#if order.orderStatus lt 2>
            <div class="bar text-right">
                <#if order.orderStatus==1>
                <a href="javascript:" data-url="/member/order/cancel/${order.orderSn}" template-id="cancel-reason-template" class="button button-outline ajax-request">取消订单</a>
                ...
                <a href="javascript:" data-url="/member/order/delete/${order.orderSn}" confirm="确定要删除该订单吗?" method-type="post" call-back="removeItem('${order.id}')" class="button button-outline  ajax-request">删除订单</a>
                </#if>
            </div>
            </#if>
            <#if order.orderStatus==2 && (order.paymentType==0||order.paymentType==3)>
            <div class="bar text-right">
                <a href="javascript:" data-url="/member/order/cancel/${order.orderSn}" confirm="确定要取消该订单吗?" template-id="cancel-reason-template" class="button button-outline ajax-request">取消订单</a>
            </div>
            </#if>
