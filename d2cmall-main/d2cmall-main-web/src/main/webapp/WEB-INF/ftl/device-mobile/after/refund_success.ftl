...
<div class="section">
    ...
    <div class="form">
        ...
        <div class="form-item item-flex form-nobor">
            <label>退款原因</label><#if reship?exists && reship!=null><label>${(reship.reshipReasonName)!}</label><#else><label>${(refund.refundReasonName)!}</label></#if>
        </div>
			...
			<li>
				<div class="logis-detail-d">
					<div class="logis-detail-content">
						...
						<p class="float-left grey">经办人：<#if log.creator><span class="creater">${log.creator}</span><#else>D2C售后组</#if></p>
