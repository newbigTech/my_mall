...
<#list result.items as item>
	<div class="item-detail" data-id="${item.key}">
		...
		<input class="productpromotionPrice" type="hidden" value="${(item.productPrice-item.promotionPrice)?string("currency")?substring(1)}" />
		...
		<div class="totalProductAmount">&yen;${(item.totalProductAmount-item.promotionAmount)?string("currency")?substring(1)}</div>
	</div>
</#list>
</div>