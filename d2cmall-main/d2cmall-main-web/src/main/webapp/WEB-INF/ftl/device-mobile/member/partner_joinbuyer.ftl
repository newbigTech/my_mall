...
<style>
	footer,.footer-space,.app-download-bar,.online-chat{display:none;}
	img{display:block;width:100%;}
	.content-container{width:100%;position:relative;background:#000;}
	.gift-header{
		text-align:center;
		padding:20px 0;
		color:#FFF;
...
<div class="content-container">
	...
	<div class="gift-header">
		...
		<span style="display:inline-block;vertical-align:middle;">${nickName}邀您成为买手</span>	
	</div>
	</#if>	
	...
	<div class="open-shop" data-current="<#if m.LOGINMEMBER.partnerId==parentId>invite<#else>join</#if>"><#if m.LOGINMEMBER.partnerId==parentId>我要邀请<#else>我要加入</#if></div>
...
<p><script id="list-template" type="text/html">
	{{each list as value i}}
		<a href="/product/{{value.id}}?parent_id=${parentId}" class="gift-item">
			<div class="gift-img"><img data-none src="//img.d2c.cn/{{value.productImageCover}}!300" alt="{{value.name}}"></div>
			<div class="gift-cont">
				<p class="title">{{value.name}}</p>
				<p class="price">&yen;{{value.salePrice}}<span style="font-size:14px;font-weight:normal;text-decoration:line-through;margin-left:10px;color:#999;">&yen;{{value.originalPrice}}</span></p>
			</div>	
		</a>
	{{/each}}
</script></p>
