...
<div class="section">
    ...
    <div class="form">
        <form name="" action="/member/reship/logistic" class="validate-form" redirect-url="/member/order/${reship.orderSn}" method="post">
            ...
            <div class="form-item item-flex">
                <label>支付宝账号</label>
                <input type="text" name="backAccountSn" value="${reship.refund.backAccountSn}" class="input validate" placeholder="退款仅支持退到支付宝账号" title="支付宝账号" />
            </div>
            <div class="form-item item-flex">
                <label>支付宝名称</label>
                <input type="text" name="backAccountName" value="${reship.refund.backAccountName}" class="input" placeholder="财务退款需核对支付宝账号的姓名" />
            </div>
            <#else>
