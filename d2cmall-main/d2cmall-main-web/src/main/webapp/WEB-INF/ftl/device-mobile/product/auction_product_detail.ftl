...
<div class="auction-detail-container">
	<div class="auction-detail-header auction-flex auction-base" data-id="${productDet.id}">
		<div class="header-price">
			<p style="font-weight:bold;color:#FFBA4B;">&yen;<span style="font-size:24px;">${productDet.currentPrice}</span></>
			<div style="color:#C39B5C;line-height:1rem;">
				<img src="//static.d2c.cn/img/topic/180510/auction/images/pic_dangqianjia@3x.png" style="width:3.57rem;height:1rem;vertical-align:middle;" />
		...
		<div class="header-count">
			...
			<p>距离结束还有</p>
			<div class="count-down" data-type="split-time"  data-endtime="${productDet.endDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}" data-defined="second">
			...
			<p>距离开始还有</p>
			<div class="count-down" data-type="split-time"  data-endtime="${productDet.beginDate?string("yyyy/MM/dd HH:mm:ss")}" data-serverTime="${.now?string("yyyy/MM/dd HH:mm:ss")}" data-defined="second">
				<span class="day down-time">00</span>天
				<span class="hour down-time">00</span> :
				<span class="minute down-time">00</span> :
				<span class="second down-time" style="color:#F21A1A;">00</span>
	...
	<div class="auction-detail-process">
	     ...
	     <div class="auction-flex" style="font-size:11px;color:rgba(0,0,0,.5);">
	     	<span><img src="//static.d2c.cn/img/topic/180510/auction/images/01@3x.png" width="100%" class="detail-process-icon" alt=""/>支付保证金</span>
	     	<span><img src="//static.d2c.cn/img/topic/180510/auction/images/02@3x.png" width="100%" class="detail-process-icon" alt=""/>参与竞拍</span>
	     	<span><img src="//static.d2c.cn/img/topic/180510/auction/images/03@3x.png" width="100%" class="detail-process-icon" alt=""/>竞拍得标</span>
	     	<span><img src="//static.d2c.cn/img/topic/180510/auction/images/04@3x.png" width="100%" class="detail-process-icon" alt=""/>支付尾款</span>
	     	<span><img src="//static.d2c.cn/img/topic/180510/auction/images/05@3x.png" width="100%" class="detail-process-icon" alt=""/>竞拍成功</span>
	     </div>
	</div>
	<div class="product-detail-tab clearfix">
       <a href="javascript:" class="on" data-url="/product/detail/${productDet.product.id}?type=ajax" style="width:100%;text-align:left;padding-left:.9375rem;font-weight:bold;font-size:.9em;">商品详情</a>
    </div>
	...
	<div class="auction-fix">
		<#if !m.FROMAPP>
			<a href="javascript:ysf.open();" class="auction-assistant"><i class="icon icon-assistant" style="width:2.1rem;height:1.9rem;margin-top:.2rem;background-size:80%;"></i><br/>客服</a>
		<#else>
			<a href="/customer/service" class="auction-assistant"><i class="icon icon-assistant" style="width:2.1rem;height:1.9rem;margin-top:.2rem;"></i><br/>客服</a>
		</#if>
			...
			<#if productDet.doing>
				<#if myMargin?exists && myMargin.status==1>
					<div class="auction-fix-wrap">	
				 		<div class="wrap-item">
					 	 	<span id="decrease" style="border-right:1px solid #EEE;padding:0 .5rem;font-weight:800;">－</span>
					 	 	<span class="num-input" style="color:#B2B2B2;font-size:14px;" data-price="${productDet.stepPrice}">最低加${productDet.stepPrice}元</span>
					 	 	<span id="increase" style="border-left:1px solid #EEE;padding:0 .5rem;font-weight:800;">＋</span>
					 	 	<input type="hidden" class="product-count" id="num" name="num" readonly value="${productDet.currentPrice + productDet.stepPrice}" data-price="${productDet.currentPrice + productDet.stepPrice}"/>
				 	 	</div>
				 	</div> 
				    <button type="button" data-type="auction" data-sn="${margin.marginSn}" class="auction-pay auction-button" style="width:33%;">立即出价</button>
				<#else>
					<button type="button" data-type="margin" class="auction-pay auction-button">支付保证金<br/><span style="color:rgba(255,255,255,.5);font-size:10px;font-weight:normal;padding-top:2px;">若出局保证金将将全额退回</span></button>
				</#if>
			<#else>
				<#if myMargin?exists>
					<#if myMargin.status == 2 || myMargin.status == 6 ||  myMargin.status == 8 || myMargin.status == 10> 
						<#if myMargin.status == 2>
							<form name="form-auction" id="form-auction" action="/auction/margin/confirm/${myMargin.marginSn}" method="POST"></form>
							<div style="font-size:13px;color:#FFCACA;background:rgba(242,26,26,.5);position:absolute;bottom:3rem;left:0;width:100%;line-height:1.875rem;">恭喜您成功中标！请尽快支付尾款</div>
							<button type="button" data-type="payauction" data-sn="${myMargin.marginSn}" class="auction-pay auction-button">支付尾款<br/><span style="font-size:12px;color:rgba(0,0,0,.5);">超时未支付将扣除保证金</span></button>
						<#else>
							<button type="button" data-type="auctionout" class="auction-pay" disabled="disabled">竞拍成功</button>
						</#if>
					<#elseif myMargin.status == -2 || myMargin.status == -8 || myMargin.status = -6>
						<button type="button" data-type="auctionout" class="auction-pay" disabled="disabled" style="color:rgba(255,255,255,.5)">拍卖已结束</button>
					<#else>
						<div class="auction-pay" style="line-height:3rem;">拍卖结果即将揭晓</div>
					</#if>
				<#else>
					<button type="button" data-type="auctionout" class="auction-pay auction-button" disabled="disabled" style="color:rgba(255,255,255,.5)">${productDet.statusName}</button>	
				</#if>
			</#if>
		<#else>
			<button type="button" class="auction-pay" disabled="disabled" style="color:rgba(255,255,255,.5)">已下架</button>
		</#if>
	</div>
</div>
<share data-title="${productDet.product.name}" data-desc="1元起拍 等你来出价！" data-pic="//static.d2c.cn/img/home/160627/images/headpic.png"></share>
<script>
//拍卖规则
