...
<div class="top-downloadl" >
	...
	<#if result.datas.coupon.wapCode?exists && result.datas.coupon.wapCode!="">${result.datas.coupon.wapCode}<#else><img src="//static.d2cmall.com/img/topic/160826/testest.png" width="100%"></#if> 
</div>
      ...
      <div>
            ...
            <div class="coupon-left" style="height:100%">
             	...
             	<#if result.datas.coupon.type=="DISCOUNT">
                   <p style="font-size:20px;padding-top:15px">折扣 ${result.datas.coupon.amount/10}</strong></p> 
                    <#else>
                    <p style="font-size:20px;padding-top:15px">&yen;<strong>${result.datas.coupon.amount}</strong></p>
                    </#if>
                <p style="font-size:10px;">满${result.datas.coupon.needAmount}元使用</p>
            </div>
            <div class="coupon-right">
                <p style="font-size:15px">${result.datas.coupon.name}</p>
                <p style="font-size:10px">有效期：${(result.datas.coupon.enableDate?string("yy/MM/dd"))!} - ${(result.datas.coupon.expireDate?string("yy/MM/dd"))!}</p>
            </div>
	    ...
	    <form id="coupon-form" class="validate-form" action="${result.datas.receiveUrl}" method="post" call-back="receviesuccess" success-tip="false" data-go="${result.datas.coupon.redirectUrl}">
	       ...
	       <div class="coupon-button" style="margin-top:10px;">
	            <button class="button button-red" type="submit" style="width: 100%;line-height:105%;font-size: 15px;height:40px;line-height:40px;border-radius:3px">立即领取</button>
	        </div>
        </#if>
       </form>
    </div>
