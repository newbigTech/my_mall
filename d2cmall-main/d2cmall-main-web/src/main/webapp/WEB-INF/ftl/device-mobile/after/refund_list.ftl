    ...
    <#if refund.reshipId==null>
        <div class="form-item item-card item-margin clearfix">
            ...
            <a href="/member/refund/${(refund.id)!}" class="clearfix">
                <span class="img"><img src="${static_base}/m/img/blank100x157.png" data-image="<#if refund.productImg?exists>${picture_base}/${(refund.sp1?eval.img)!}!120</#if>" /></span>
            ...
            <div class="bar text-right">
 				...
 				<a href="javascript:" data-url="/member/refund/cancel/${(refund.id)!}.json" method-type="post" confirm="确定要取消退款申请中吗？" class="ajax-request button button-outline"  style="margin-top:.3em">取消申请</a>
 				</#if>			
 				<a href="/member/refund/${refund.id}" class="button button-red" style="margin-top:.3em">查看详情</a>
            </div>
        </div>    
         </#if>
    </#list>
