...
<div class="theme-list">
	<div class="theme-tab">
		<a href="/membershare/topic/${topic.id}"  class="<#if !RequestParameters.orderType>current</#if>" style="margin-right:55px">热门</a>
		<a href="/membershare/topic/${topic.id}?orderType=new"   class="<#if RequestParameters.orderType>current</#if>">最新</a>
	</div>
	...
	<div class="theme-item">
		...
		<div class="item-content">
			<a href="/membershare/topic/${topic.id}"><span class="item-title">#${(topic.title)!}#</span></a><span>${list.description}</span>
		</div>
		<#if list.resourceType=='video'>
			<div class="item-img item-one item-video">
				<video width="100%" height="100%"  src="${picture_base}${list.video}"  style="position:absolute;width:100%;height:100%;display:none;background:#000" type="video/mp4" class="videoarea" controls="controls"></video>
		...
		<#else>
			...
			<#assign pics=list.pics/>
				...
				<#list pics as pic>
					<#if pic_index lte 2><a href="${picture_base}${pic}"><img src="${picture_base}${pic}!/both/345x345"></a></#if>
				</#list>
			  <#if pics?size gt 3> 
		...
		<div class="item-bar flex">
			<div>
				<span class="btn-like ajax-request" method-type="post" data-url="/membershare/like/insert/${list.id}" call-back="liked(${list.id})" ><i class="<#if list.liked==1>icon-liked<#else>icon-like</#if> like-${list.id}  mar5"></i><span class="lin">${list.likeMeCount}</span></span>
				<span class="btn-comment"><a href="/membershare/${list.id}"><i class="icon-comment mar5" style="margin-left:25px;"></i>${list.commentCount}</a></span>
			</div>
			<div>
				<span class="btn-watch"><i class="icon-watchnum mar5" style="width:28.3px;"></i>${list.watchCount}</span>
			</div>
		</div>
	</div>
	</#list>
	<#if pager.pageCount gt 1>
	<div class="load-more scroll-load-more" data-url="/membershare/topic/${topic.id}" data-target="load-more-product" template-id="list-template" data-page="${pager.pageNumber}" data-total="${pager.pageCount}">点击加载更多</div>  
	</#if>
</div>
