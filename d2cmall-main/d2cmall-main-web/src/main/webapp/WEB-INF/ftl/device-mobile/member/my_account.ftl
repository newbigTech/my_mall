...
<div class="section">
    ...
    <div class="form">
        <form name="form-info" id="form-info" class="validate-form" redirect-url="/member/account/info" action="/member/account/changePassword.json" method="post">
            ...
            <div class="form-item item-flex">
                ...
                <input type="hidden" name="mobile" class="validate validate-account" title="手机号码" data-rule="mobile" value="${(account.mobile)!}" />
            </div>
            <#else>
            ...
            <div class="form-item item-flex">
                <label class="choose-country-code" style="font-size:16px;">+86</label>
                <input type="text" name="mobile" class="input validate validate-account" title="手机号码" data-rule="mobile" placeholder="请输入常用的手机号码" value="${(account.mobile)!}" />
            </div>
            </#if>
            <div class="form-item item-flex">
                <label>校 验 码</label>
                <input type="text" name="code" id="code" value="" class="input validate" title="校验码" placeholder="请输入校验码" />
                <button type="button" data-source="${(account.account)!}" data-type="MEMBERMOBILE" class="button button-white validate-send validate-button">获取校验码</button>
            </div>
            <div class="form-item item-flex">
                <label>支付密码</label>
                <input type="password" name="newPassword1" id="newPassword1" class="input" min-length="6" placeholder="请输入6位以上支付密码" value="" />
            </div>
            
            <div class="form-item item-flex">
                <label>确认密码</label>
                <input type="password" name="newPassword2" id="newPassword2" class="input" compare-with="newPassword1" placeholder="请确认一次密码" value="" />
            </div>
    ...
    <div class="user-wallet">
        ...
        <p class="grey">现金 &yen; ${(account.cashAmount?string("currency")?substring(1))!} &nbsp;&nbsp;红包 &yen; ${(account.giftAmount?string("currency")?substring(1))!}</p>
        <p class="grey">可用金额 &yen; ${(account.availableTotalAmount?string("currency")?substring(1))!} &nbsp;&nbsp; 冻结金额 &yen;${(account.freezeAmount?string("currency")?substring(1))!}</p>       
    </div>
    <div class="form">
