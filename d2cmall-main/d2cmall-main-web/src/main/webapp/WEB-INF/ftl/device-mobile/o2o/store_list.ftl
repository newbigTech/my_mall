...
<div class="section lazyload">
	...
	<#if store.xy?exists>
    	...
    	<div class="item item-vertical">
    	    <p class="img"><img src="${static_base}/m/img/blank100x45.png" data-image="${picture_base}${(store.pic)!}" /></p>
    		<p class="desc">名称：${(store.name)!}<a class="float-right link-map" data-name="${(store.name)!}" data-address="${(store.address)!}/${(store.tel)!}" data-lant="${map_arr[1]}" data-long="${map_arr[0]}" href="<#if browser=='wechat'>javascript:<#else>//mo.amap.com/?q=${map}&name=${(store.name)!} ${(store.address)!}&dev=0</#if>">查看地图</a> </p>
