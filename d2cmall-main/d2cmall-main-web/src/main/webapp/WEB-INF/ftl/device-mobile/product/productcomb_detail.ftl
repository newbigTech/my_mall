...
<div class="section">
    <div class="product-base" style="padding-bottom:0.6em;">
        ...
        <div class="product-label">
            <label>套餐价</label><span class="data-price" id="promo-price"><#if productComb.price lte 0>????.??<#else>&yen; ${productComb.price?string('0.00')}</#if></span>
            <#if productComb.originalCost gt productComb.price>&nbsp;&nbsp;&nbsp;<label style="width:auto;">原价</label> <s class="grey">&yen; <strong>${(productComb.originalCost)}</strong></s></#if>
        </div>
	...
	<form name="product-package-buy" id="package-buy" action="/order/comb/buynow" method="post">
			...
			<div class="product-select">
				...
				<div class="product-sku img clearfix" style="position:relative;">
					...
					<#list product.salesproperty?eval.sp1 as p>
					   <a href="javascript:" id="c-${product.id}-${p.id}" class="color-section" data-type="color" data-id="${p.id}" rel="${p.value}" data-image="<#if p.img!=''>${picture_base}/${p.img}</#if>">${p.value}</a>
                    </#list>
					</span>
				</div>
				<div class="product-sku txt clearfix">
					...
					<#list product.salesproperty?eval.sp2 as p>
                        <a href="javascript:" id="s-${product.id}-${p.id}" class="size-section" data-type="size" data-id="${p.id}" rel="${p.value}">${p.value}</a>
                    </#list>
					</span>
				</div>
		...
		<div class="num-crease text-center" style="padding:0.6em 0;">
			<div style="line-height:150%;">购买数量</div><div class="num-crease-wrap"><span class="num-decrease" id="decrease">－</span><span class="num-input">1</span><span class="num-increase" id="increase">＋</span><input type="hidden" class="product-count" id="num" name="num" readonly value="1"></div>
		</div>
		<div class="suspend-bar">
            <div class="desc">应付：<span class="price">&yen; <strong id="now-pay-price" class="price" rel="${productComb.price}">${productComb.price?string("currency")?substring(1)}</strong> 元</span></div>
            <div class="buttons"><#if productComb.mark==1><button type="submit" id="submit-button" class="button button-red">立刻购买</button><#else><div class="desc">已下架</div></#if></div>
        </div>
...
$('.product-sku a').on('touchstart',function(){
	var obj=$(this),package_item=$(this).parent().parent().parent(),product_id=package_item.find('input[name=productId]').val();
