...
<div class="section">
    <div class="form">
        <form name="pay-form" id="pay-form" action="/payment" method="post" >
            <input type="hidden" name="id" value="${result.datas.auctionMargin.id}" />
            <input type="hidden" name="sn" value="${(result.datas.auctionMargin.marginSn)}" />
            <input type="hidden" name="orderType" value="margin" />
            <div class="form-item item-flex item-margin">
                <label>应付金额</label><label class="red">&yen; ${(result.datas.auctionMargin.billTotalFee)} 元</label>
            </div>
            <#if browser=='wechat'>
