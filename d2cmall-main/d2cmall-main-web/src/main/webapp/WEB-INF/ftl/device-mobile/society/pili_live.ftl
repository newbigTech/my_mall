...
<div class="video">
	<div  id="player">
  		<video  id="video"  webkit-playsinline="true" x-webkit-airplay="true"  playsinline="true" x5-video-player-type="h5" x5-video-player-fullscreen="true" loop="loop" width="100%" preload="auto" style="background: rgb(0, 0, 0);object-fit: fill;">
  		<source type="application/x-mpegurl" src="<#if live.status==4>${live.hlsUrl}<#elseif live.status==8>${live.replayUrl}</#if>">
  		<source type="application/vnd.apple.mpegurl" src="<#if live.status==4>${live.hlsUrl}<#elseif live.status==8>${live.replayUrl}</#if>">
  		</video>
 	</div>
</div>
...
<div class="live-user-info">
 ...
 <div class="bg-icon">
  <#if live.cover><div style="position: absolute;width: 100%;height: 100%;top: 0;background-image: url(${picture_base}${(live.cover)!});background-position: center center;background-size: cover;"></div></#if>
