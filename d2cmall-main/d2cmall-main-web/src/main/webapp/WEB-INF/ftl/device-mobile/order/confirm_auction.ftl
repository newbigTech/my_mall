...
<div class="auction-order">
	<form id="confirm-form" class="validate-form" action="/auction/margin/update/${auctionMargin.id}" method="post" call-back="auctionCreateSuccess">
		...
		<#if addresses?size gt 0>
	        ...
	        <div class="auction-order-item" data-id="${address.id}" id="auction-order-list">
	            <p style="color:rgba(0,0,0,.85);font-size:18px;font-weight:bold;">${(address.name)!}<span style="margin-left:.625rem;">${address.mobile}</span></p>
		...
		<div class="auction-order-item auction-flex" style="margin-top:.625rem;">
			<div><img src="${picture_base}/${(product.product.productImageCover)!}!80" alt="" style="width:70px;height:105px;"/></div>
			<div style="margin-left:.625rem;">
				...
				<div style="font-size:12px;color:rgba(0,0,0,.85);">出价：<span style="color:#F21A1A;font-weight:bold;">&yen;<span style="font-size:18px;">${product.currentPrice?string("currency")?substring(1)}</span></span></div>
			</div>
		</div>
		...
		<div class="auction-fix">
			<div class="desc" data-sn="${auctionMargin.marginSn}" style="float:left;width:67%;text-align:left;line-height:3rem;padding-left:.625rem;box-sizing:border-box;color:#F21A1A;font-weight:bold;font-size:12px;">&yen;<span style="font-size:24px;">${(auctionMargin.billTotalFee?string("currency")?replace('￥',''))}</span></div>
        ...
        <header class="modal-header">
            <div class="header">
                ...
                <div class="header-btn">
                    <button type="button" onclick="modalAddressNew()" class="button button-s button-blue">新增</button>
                </div>
            </div>
        </header>
        <section class="modal-body">
            ...
            {{each list as address i}}
                <div class="form-item item-address select-address" data-id="{{address.id}}">
                    ...
                    <p>{{address.province}}{{address.city}}{{address.district}}{{address.street}}</p></label>
                </div>  
            {{/each}}
            </div>
        </section>
        ...
        <section class="modal-body">
            <div class="form">
                ...
                <input type="hidden" name="id" value="" />
                    ...
                    <div class="form-item item-flex">
                        <label class="choose-country-code" id="choose-country" style="font-size:16px;">+86</label>
                        <input type="text" name="mobile" placeholder="收货人手机号" class="input validate" title="收货人手机号" min-length="4" maxlength="20" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
...
<script id="address-select-template" type="text/html">
    <div class="auction-order-item" data-id="{{id}}" id="auction-order-list">
        <p style="color:rgba(0,0,0,.85);font-size:18px;font-weight:bold;">{{name}}<span style="margin-left:.625rem;">{{mobile}}</span></p>
        <div style="margin-top:.625rem;font-size:12px;color:rgba(0,0,0,.5);">
            <span class="icon-location"></span>
            <span>{{province}}{{city}}{{district}}{{street}}</span>
        </div>
        <span class="icon-right"></span>
