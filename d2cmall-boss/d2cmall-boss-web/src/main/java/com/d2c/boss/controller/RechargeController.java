...
import com.d2c.boss.member.service.RechargeService;
...
@Controller
@RequestMapping("/api/crm/recharge")
public class RechargeController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<Recharge> rechargeList = rechargeService.findRechargesByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", rechargeList.getTotalCount());// 总条数
			result.put("index", rechargeList.getPageNumber());// 当前页码
			result.put("pageSize", rechargeList.getPageSize());// 每页记录条数
			result.put("pageCount", rechargeList.getPageCount());// 总页数
			for (Recharge b : rechargeList.getList()) {
			...
			apiLog.setNum(rechargeList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		Recharge recharge = (Recharge) JsonHelper.instance().toObject(data, Recharge.class);
		recharge = rechargeService.insert(recharge);
		return result;
	}
}
