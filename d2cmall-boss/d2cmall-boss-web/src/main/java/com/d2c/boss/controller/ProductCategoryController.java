...
import com.d2c.boss.product.service.ProductCategoryService;
...
@Controller
@RequestMapping("/api/crm/productcategory")
public class ProductCategoryController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<ProductCategory> productCategoryList = productCategoryService.findProductCategorysByQuery(page,
					query);
			JSONArray datas = new JSONArray();
			result.put("total", productCategoryList.getList().size());// 总条数
			result.put("index", productCategoryList.getPageNumber());// 当前页码
			result.put("pageSize", productCategoryList.getPageSize());// 每页记录条数
			result.put("pageCount", productCategoryList.getPageCount());// 总页数
			for (ProductCategory b : productCategoryList.getList()) {
			...
			apiLog.setNum(productCategoryList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		ProductCategory productCategory = (ProductCategory) JsonHelper.instance().toObject(data, ProductCategory.class);
