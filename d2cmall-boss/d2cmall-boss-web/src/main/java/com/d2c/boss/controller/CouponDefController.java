...
import com.d2c.boss.member.service.CouponDefService;
...
@Controller
@RequestMapping("/api/crm/coupondef")
public class CouponDefController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<CouponDef> couponDefList = couponDefService.findCouponDefsByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", couponDefList.getList().size());// 总条数
			result.put("index", couponDefList.getPageNumber());// 当前页码
			result.put("pageSize", couponDefList.getPageSize());// 每页记录条数
			result.put("pageCount", couponDefList.getPageCount());// 总页数
			for (CouponDef b : couponDefList.getList()) {
			...
			apiLog.setNum(couponDefList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		CouponDef couponDef = (CouponDef) JsonHelper.instance().toObject(data, CouponDef.class);
