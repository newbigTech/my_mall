...
import com.d2c.boss.member.service.CommentService;
...
@Controller
@RequestMapping("/api/crm/comment")
public class CommentController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<Comment> commentContList = commentService.findCommentsByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", commentContList.getTotalCount());// 总条数
			result.put("index", commentContList.getPageNumber());// 当前页码
			result.put("pageSize", commentContList.getPageSize());// 每页记录条数
			result.put("pageCount", commentContList.getPageCount());// 总页数
			for (Comment b : commentContList.getList()) {
			...
			apiLog.setNum(commentContList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		Comment comment = (Comment) JsonHelper.instance().toObject(data, Comment.class);
