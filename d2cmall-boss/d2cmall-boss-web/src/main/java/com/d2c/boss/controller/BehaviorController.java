...
import com.d2c.boss.member.service.BehaviorService;
...
public class BehaviorController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<Behavior> behaviorList = behaviorService.findBehaviorsByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", behaviorList.getTotalCount());// 总条数
			result.put("index", behaviorList.getPageNumber());// 当前页码
			result.put("pageSize", behaviorList.getPageSize());// 每页记录条数
			result.put("pageCount", behaviorList.getPageCount());// 总页数
			for (Behavior b : behaviorList.getList()) {
			...
			apiLog.setNum(behaviorList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		Behavior designer = (Behavior) JsonHelper.instance().toObject(data, Behavior.class);
		designer = behaviorService.insert(designer);
		return result;
	}
}
