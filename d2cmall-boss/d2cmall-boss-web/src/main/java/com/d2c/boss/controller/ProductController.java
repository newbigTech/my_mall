...
public class ProductController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<ProductDto> productList = productService.findProductDtosByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", productList.getList().size());// 总条数
			result.put("index", productList.getPageNumber());// 当前页码
			result.put("pageSize", productList.getPageSize());// 每页记录条数
			result.put("pageCount", productList.getPageCount());// 总页数
			for (ProductDto b : productList.getList()) {
			...
			apiLog.setNum(productList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		Product product = (Product) JsonHelper.instance().toObject(data, Product.class);
