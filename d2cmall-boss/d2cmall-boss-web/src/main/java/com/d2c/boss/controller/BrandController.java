...
public class BrandController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<Brand> brandList = brandService.findBrandsByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", brandList.getList().size());// 总条数
			result.put("index", brandList.getPageNumber());// 当前页码
			result.put("pageSize", brandList.getPageSize());// 每页记录条数
			result.put("pageCount", brandList.getPageCount());// 总页数
			for (Brand b : brandList.getList()) {
			...
			apiLog.setNum(brandList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		Brand brand = (Brand) JsonHelper.instance().toObject(data, Brand.class);
