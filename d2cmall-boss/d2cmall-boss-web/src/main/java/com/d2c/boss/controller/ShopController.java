...
import com.d2c.boss.product.service.ShopService;
...
public class ShopController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<Shop> shopList = shopService.findShopsByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", shopList.getList().size());// 总条数
			result.put("index", shopList.getPageNumber());// 当前页码
			result.put("pageSize", shopList.getPageSize());// 每页记录条数
			result.put("pageCount", shopList.getPageCount());// 总页数
			for (Shop b : shopList.getList()) {
			...
			apiLog.setNum(shopList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		Shop shop = (Shop) JsonHelper.instance().toObject(data, Shop.class);
		shop = shopService.insert(shop);
		return result;
	}
}
