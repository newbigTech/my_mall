...
import com.d2c.boss.member.service.MemberCollectionService;
...
@Controller
@RequestMapping("/api/crm/membercollection")
public class MemberCollectionController extends BaseController {
	...
	public JSONObject dealDatas(Map<String, Object> resultMap) {
		...
		try {
			PageResult<MemberCollection> memberCollectionList = memberCollectionService
					.findMemberCollectionsByQuery(page, query);
			JSONArray datas = new JSONArray();
			result.put("total", memberCollectionList.getTotalCount());// 总条数
			result.put("index", memberCollectionList.getPageNumber());// 当前页码
			result.put("pageSize", memberCollectionList.getPageSize());// 每页记录条数
			result.put("pageCount", memberCollectionList.getPageCount());// 总页数
			for (MemberCollection b : memberCollectionList.getList()) {
			...
			apiLog.setNum(memberCollectionList.getTotalCount());
	...
	protected Response doInsert(JSONObject data) {
		...
		MemberCollection memberCollection = (MemberCollection) JsonHelper.instance().toObject(data,
				MemberCollection.class);
		memberCollection = memberCollectionService.insert(memberCollection);
		return result;
	}
}
