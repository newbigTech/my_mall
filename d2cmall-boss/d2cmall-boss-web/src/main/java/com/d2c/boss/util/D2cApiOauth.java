...
public class D2cApiOauth {
	...
	private D2cApiOauth() {
		...
		// 上海七际网络科技有限公司，合作伙伴（明星借衣服）
		roles.put("secret_key", "d2cd2c6d7da05cc8874640ba6aac060192d2cd2c");
		roles.put("all", "none");
		roles.put("product", "1");
		roles.put("productsku", "1");
		partners.put("0010", roles);
		roles = new HashMap<String, String>();
	...
	public JSONArray getJsonArray(HttpServletRequest request, String role) {
		...
		if (!("1".equals(roleSc) || partner.get("all").equals("all"))) {
			...
			String md5Str2 = DigestUtils.md5Hex(jsonArray.toString() + secretKey);
