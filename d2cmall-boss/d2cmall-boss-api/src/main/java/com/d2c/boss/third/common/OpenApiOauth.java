package com.d2c.boss.third.common;
...
import com.d2c.boss.sys.util.Base64Coder;

public class OpenApiOauth {

	public static final String UIN = "2742514965";
	private static final String APP_OAUTH_ID = "700180246";
	private static final String APP_OAUTH_KEY = "X5AKnof8VWZJhIkS";
	private static final String ACCESS_TOKEN = "2367bb5a0ede32178fc6197c28a13c31";
	private static final String HOST_NAME = "api.weigou.qq.com";
	...
	private static OpenApiOauth oauth;
	private String apiPath;

	private OpenApiOauth() {
	}

	public static OpenApiOauth getInstance() {
		if (oauth == null) {
			oauth = new OpenApiOauth();
			oauth.uin = UIN;
			oauth.appOAuthID = APP_OAUTH_ID;
			oauth.appOAuthkey = APP_OAUTH_KEY;
			oauth.accessToken = ACCESS_TOKEN;
			oauth.hostName = HOST_NAME;
		}
		return oauth;
	}
	 ...
	 * 
	 * @param apiName
	 *            OpenApi CGI, 例如 /deal/sellerSearchDealList.xhtml
	 * @param protocol
	 *            HTTP请求协议 "http" / "https"
	 * @return 返回服务器响应内容
	 */
	...
	private String makeSource(String method, String url_path) throws OpenApiException, UnsupportedEncodingException {
		...
		for (int i = 0; i < keys.length; i++) {
			buffer2.append(keys[i]).append("=").append(getValue(params.get(keys[i])));// new
		...
		// oidb按1024最大长度截取
		// buffer = buffer.length() > 1024 ? buffer.delete(1024,
		// buffer.length())
		// : buffer;
