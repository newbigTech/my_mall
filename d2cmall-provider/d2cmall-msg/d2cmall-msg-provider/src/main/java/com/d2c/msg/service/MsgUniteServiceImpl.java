...
public class MsgUniteServiceImpl implements MsgUniteService {
	...
	private void sendEmail(EmailBean emailBean) {
		...
		if (LoginUtil.checkEmail(emailBean.getEmail())) {
			emailLogService.sendEmail(emailBean.getEmail(), emailBean.getSubject(), emailBean.getTemplateId(),
					emailBean.getEmailC(), emailBean.getContent(), emailBean.getBuzId(), emailBean.getEmailLogType());
		}
	}
