package com.d2c.msg.third.sms.montnets.common;
...
public class ValidateParamTool {
	...
	public static boolean validateSubPort(String strSubPort) {
		try {
			if (strSubPort != null && !"".equals(strSubPort)
					&& ("*".equals(strSubPort) || SmsTool.isUnSignDigit(strSubPort)) && strSubPort.length() < 7) {
				return true;
			} else {
				return false;
			}
	...
	public static boolean validateSpNumber(String strSpNumber) {
		try {
			if (strSpNumber != null && !"".equals(strSpNumber) && ("*".equals(strSpNumber)
					|| (SmsTool.isUnSignDigit(strSpNumber) && strSpNumber.length() <= 20))) {
				return true;
			} else {
				return false;
			}
	...
	public static boolean validateMobile(String strMobile) {
		try {
			if (strMobile != null && !"".equals(strMobile) && strMobile.length() == 11
					&& SmsTool.isUnSignDigit(strMobile)) {
				return true;
			} else {
				return false;
			}
			...
			if (strMobiles != null && !"".equals(strMobiles)) {
				...
				if (arrMobiles.length > 0 && arrMobiles.length <= 100) {
					for (int i = 0; i < arrMobiles.length; i++) {
						if ("".equals(arrMobiles[i]) || arrMobiles[i].length() != 11
								|| !SmsTool.isUnSignDigit(arrMobiles[i])) {
							return false;
						}
					}
				} else {
