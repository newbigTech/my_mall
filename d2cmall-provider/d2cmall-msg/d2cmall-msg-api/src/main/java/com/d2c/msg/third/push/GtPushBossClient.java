package com.d2c.msg.third.push;
...
import com.d2c.msg.third.push.common.GtPushBossUtil;
...
public class GtPushBossClient {

	static String host = "http://sdk.open.api.igexin.com/apiex.htm";
	private final static Logger logger = LoggerFactory.getLogger(GtPushBossClient.class);
	...
	public static void pushMessageToSingle(String clientId, String msg, APNPayload apn) {
		...
		target.setAppId(GtPushBossUtil.appId);
		// target.setClientId(clientId);
		target.setAlias(clientId);
		IPushResult ret = null;
		try {
			ret = GtPushBossUtil.getInstance().pushMessageToSingle(message, target);
		} catch (RequestException e) {
			e.printStackTrace();
			ret = GtPushBossUtil.getInstance().pushMessageToSingle(message, target, e.getRequestId());
		}
		if (ret != null) {
	...
	public static void pushTransmissionMessageToApp(String msg, int transmissionType, long offlineExpireTime,
		...
		appIdList.add(GtPushBossUtil.appId);
		...
		IPushResult ret = GtPushBossUtil.getInstance().pushMessageToApp(message, "Tran_toApp");
	...
	public static void pushTransmissionMessageToList(List<String> clientIds, String msg, int transmissionType,
		...
		String appId = GtPushBossUtil.appId;
		// 配置返回每个用户返回用户状态，可选
		...
		String taskId = GtPushBossUtil.getInstance().getContentId(message);
		IPushResult ret = GtPushBossUtil.getInstance().pushMessageToList(taskId, targets);
	...
	private static TransmissionTemplate getTransmissionTemplate(String msg, int transmissionType) {
		...
		template.setAppId(GtPushBossUtil.appId);
		template.setAppkey(GtPushBossUtil.appKey);
