	...
	public void printRptPack(List<RPT_PACK> rptPackList) {
		if (rptPackList != null && rptPackList.size() > 0) {
			...
			for (int i = 0; i < rptPackList.size(); i++) {
				...
				System.out.println(rptPack.getStrMoTime() + "," + rptPack.getStrPtMsgId() + ","
						+ rptPack.getStrSpNumber() + "," + rptPack.getStrMobile() + "," + rptPack.getStrUserMsgId()
						+ "," + rptPack.getStrReserve() + "," + rptPack.getnStatus() + "," + rptPack.getStrErCode());
			}
		} else {
