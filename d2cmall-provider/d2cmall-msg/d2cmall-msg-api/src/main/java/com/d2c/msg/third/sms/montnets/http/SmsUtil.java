package com.d2c.msg.third.sms.montnets.http;
...
import com.d2c.msg.third.sms.montnets.common.StaticValue;
...
public class SmsUtil {
	...
	/**
	 * 获取上行/状态报告等 返回值： null 无信息 接收信息内容列表(最高维数为500)格式说明：
	 * 信息类型(上行标志0),日期,时间,上行源号码,上行目标通道号,*,*,上行信息内容 或
	 * 信息类型(状态报告标志1),日期,时间,信息编号,通道号,手机号,MongateSendSubmit时填写的MsgId,*,状态值,详细错误原因
	 * 例如： 1,2008-01-23 15:43:34,15986756631,10657302056780408,*,*,上行信息内容1
	 * 2,2008-01-23 15:43:34,0518153837115735,10657302056780408,13265661403,
	 * 456123457895210124, ,0,DELIVRD 注：格式中的*号是备用字段
	 * 第一标志位的0表示上行和状态报告，1表示上行，2表示状态报告
	 */
	...
	/**
	 * 查询余额接口 返回值：正数或0：帐户可发送条数 -1:登陆失败 其他错误见附录
	 */
	...
	/**
	 * 短信息发送接口 返回值：错误描述对应说明 发送成功：信息编号，如：-8485643440204283743或1485643440204283743
	 * 错误返回：-1 参数为空。信息、电话号码等有空指针，登陆失败 -2 电话号码个数超过100 -10 申请缓存空间失败 -11
	 * 电话号码中有非数字字符 -12 有异常电话号码 -13 电话号码个数与实际个数不相等 -14 实际号码个数超过100 -101 发送消息等待超时
	 * -102 发送或接收消息失败 -103 接收消息超时 -200 其他错误 -999 web服务器内部错误
	 */
	 ...
	 * 注意：由于单个短信结构体中各元素间，以及短信包中各个短信结构体间采用的分隔符分隔，所以要求短信结构体中各元素的值中不能包含分隔符（,和/），
	 * 否则将导致解析错误。 短信包中最大包含200个短信结构体，超过200个时200个以后的将丢失。 返回值： 错误描述对应说明
	 * 发送成功：信息编号，如：-8485643440204283743或1485643440204283743 错误返回： -1
	 * 参数为空。信息、电话号码等有空指针，登陆失败 -11 电话号码中有非数字字符 -12 有异常电话号码 -101 发送消息等待超时 -102
	 * 发送或接收消息失败 -103 接收消息超时 -200 其他错误 -999 web服务器内部错误
	 */
