...
public interface OrderMapper extends SuperMapper<Order> {
	...
	int updateOrderReciverInfoAndMemo(Order order);

	int updateOrderShippingAddress(@Param("orderId") Long orderId, @Param("contact") String contact,
			@Param("province") String province, @Param("city") String city, @Param("zipCode") String zipCode,
			@Param("address") String address, @Param("receiver") String receiver);

	int updateOrderStatusByDeliverySn(@Param("deliverySn") String deliverySn,
	 ...
	 * 双十二支付金额>=1000的前1000单
	 */
	List<Order> findByTop1000(@Param("startDate") Date startDate, @Param("endDate") Date endDate,
