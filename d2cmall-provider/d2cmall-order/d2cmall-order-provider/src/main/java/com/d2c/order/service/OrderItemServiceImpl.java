...
public class OrderItemServiceImpl extends ListServiceImpl<OrderItem> implements OrderItemService {
	...
	public int doReOpen(OrderItem orderItem) {
		...
		if (success > 0) {
			// TODO
			// productSkuService.doFreezeStock(orderItem.getProductId(),
	...
	public int doCloseItem(Long orderItemId, String closer, String closeInfo) {
		...
		if (success > 0) {
			redisHandler.delete("orderitem_count_" + orderItem.getBuyerMemberId());
			// 扣除评价积分
			// MemberIntegration oldIntegration = memberIntegrationService
			// .findByTransctionAndType(orderItem.getCommentId(),
			// PointRuleTypeEnum.COMMENT.name());
			// if (oldIntegration != null) {
			// MemberIntegration integration = new
			// MemberIntegration(oldIntegration);
	...
	public int doMerge(Long memberSourceId, Long memberTargetId) {
		return orderItemMapper.doMerge(memberSourceId, memberTargetId);
	}
