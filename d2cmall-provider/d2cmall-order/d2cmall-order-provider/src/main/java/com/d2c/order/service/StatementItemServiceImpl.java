...
import com.d2c.msg.service.StatementLogService;
...
public class StatementItemServiceImpl extends ListServiceImpl<StatementItem> implements StatementItemService {
	...
	@Override
	@Transactional(rollbackFor = Exception.class, readOnly = false, propagation = Propagation.REQUIRED)
	public int doSend(Long statementId, int settleYear, int settleMonth, Long designerId, String operator) {
		return statementItemMapper.doSend(statementId, settleYear, settleMonth, designerId, operator);
	...
	@Override
	@Transactional(rollbackFor = Exception.class, readOnly = false, propagation = Propagation.REQUIRED)
	public int updateSettleDate(Integer settleYear, Integer settleMonth, Long id, String operator) {
		StatementItem statementItem = this.findById(id);
		if (statementItem.getStatementId() != null) {
			throw new BusinessException("已生成对账单，不能更改此对账单明细的结算时间！");
		}
		int result = statementItemMapper.updateSettleDate(settleYear, settleMonth, id, operator);
		return result;
	}
	...
	@Override
	public List<StatementItem> findEmptySettle(int settleYear, int settleMonth, Long designerId) {
		return statementItemMapper.findEmptySettle(settleYear, settleMonth, designerId);
	}
