...
public class OrderQueryServiceImpl implements OrderQueryService {
	...
	@Override
	public PageResult<Long> findExpireOrder(int closeTime, Order.OrderType[] types, PageModel page) {
		PageResult<Long> pager = new PageResult<>(page);
		int totalCount = orderMapper.countExpireOrder(closeTime, types);
		if (totalCount > 0) {
			List<Long> list = orderMapper.findExpireOrder(closeTime, types, page);
			pager.setTotalCount(totalCount);
			pager.setList(list);
		}
		return pager;
	...
	@Override
	public int countExpireOrder(int closeTime, Order.OrderType[] types) {
		return orderMapper.countExpireOrder(closeTime, types);
	}
	...
	@Override
	public List<Order> findByTop1000(Date startDate, Date endDate, BigDecimal amount) {
		return orderMapper.findByTop1000(startDate, endDate, amount);
	}
