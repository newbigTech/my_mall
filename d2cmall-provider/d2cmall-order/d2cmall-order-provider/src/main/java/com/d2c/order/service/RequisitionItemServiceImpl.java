...
import com.d2c.msg.model.BurgeonErrorLog.DocSourceType;
import com.d2c.msg.model.BurgeonErrorLog.DocType;
import com.d2c.msg.model.RequisitionLog;
import com.d2c.msg.model.SmsLog.SmsLogType;
import com.d2c.msg.service.BurgeonErrorLogService;
import com.d2c.msg.service.MsgUniteService;
import com.d2c.msg.service.RequisitionLogService;
...
public class RequisitionItemServiceImpl extends ListServiceImpl<RequisitionItem> implements RequisitionItemService {
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public RequisitionItem doPurchaseInsert(RequisitionItem requisitionItem, String operator) {
		...
		if (rqItem != null) {
			...
			} else if (requisitionItem.getType() == ItemType.DEPOTALL.getCode()) {
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public RequisitionItem doReshipInsert(RequisitionItem requisitionItem, String operator, String deliveryCorp,
			String deliverySn, String remark) {
		requisitionItem.setCreator(operator);
		requisitionItem.setOrderDate(new Date());
		requisitionItem.setDeliveryCorp(deliveryCorp);
		requisitionItem.setDeliverySn(deliverySn);
		requisitionItem.setRemark(remark);
		RequisitionItem rqItem = this.doPurchaseInsert(requisitionItem, operator);
		return rqItem;
	}
		...
		if (old != null) {
			if (old.getStatus() < 0) {
				...
				if (brand.getDirect() == 1) {
					...
					this.doCancelClose(old.getId(), 1, order.getReciver(), order.getContact(),
							order.getProvince() + order.getCity() + order.getAddress(), null, null, operator,
							"重新指定设计师直发", old.getRequisitionId(), 1);
				} else {
					this.doCancelClose(old.getId(), 1, lbp.getLinker(), lbp.getTel(), lbp.getAddress(), lbp.getId(),
							lbp.getName(), operator, "重新指定设计师采购", old.getRequisitionId(), 0);
				}
				old.setType(1);
				requisitionItem = old;
			} else {
			...
			Brand brand = brandService.findById(orderItem.getDesignerId());
			requisitionItem = new RequisitionItem(lbp, order, orderItem, product, productSku, brand.getDirect(),
					brand.getOperation());
			requisitionItem.setExpiredDate(orderItem.getEstimateDate());
			requisitionItem.setEstimateDate(requisitionItem.getExpiredDate());
			requisitionItem = this.doPurchaseInsert(requisitionItem, operator);
		}
		...
		if (old != null) {
			if (old.getStatus() < 0) {
				if (order.getPaymentType() == 3) {
					...
					this.doCancelClose(old.getId(), 2, lbp.getLinker(), lbp.getTel(), lbp.getAddress(), store.getId(),
							store.getName(), operator, "重新指定门店（" + store.getName() + "）发货", old.getRequisitionId(), 0);
					old.setType(2);
				} else {
					Store store = storeService.findById(storeId);
					this.doCancelClose(old.getId(), 3, order.getReciver(), order.getContact(),
			...
			if (ProductTradeType.COMMON.name().equals(product.getProductTradeType())
				...
				Store store = storeService.findById(storeId);
				Brand brand = brandService.findById(orderItem.getDesignerId());
				requisitionItem = new RequisitionItem(order, orderItem, product, productSku, store, null,
						brand.getOperation(), ItemType.STOREALLC);
				requisitionItem = this.doPurchaseInsert(requisitionItem, operator);
		...
		if (old != null) {
			if (old.getStatus() < 0) {
				...
				if (brand.getDirect() == 1) {
					Order order = orderService.findById(orderItem.getOrderId());
					this.doCancelClose(old.getId(), 1, order.getReciver(), order.getContact(),
							order.getProvince() + order.getCity() + order.getAddress(), null, null, "sys", "重新指定设计师直发",
							old.getRequisitionId(), 1);
				} else {
					this.doCancelClose(old.getId(), 1, lbp.getLinker(), lbp.getTel(), lbp.getAddress(), lbp.getId(),
							lbp.getName(), "sys", "重新打开调拨明细", old.getRequisitionId(), 0);
				}
				old.setType(1);
				requisitionItem = old;
			} else {
			...
			requisitionItem.setExpiredDate(orderItem.getEstimateDate());
			requisitionItem.setEstimateDate(requisitionItem.getExpiredDate());
			requisitionItem = this.doPurchaseInsert(requisitionItem, "sys");
		}
		orderItemService.updateRequisition(orderItem.getId(), 1);
		orderItemService.updateEstimateDate(orderItem.getId(), requisitionItem.getExpiredDate(), "sys");
		return requisitionItem;
		...
		if (old != null) {
			if (old.getStatus() < 0) {
				if (order.getPaymentType() == 3) {
					Store store = storeService.findById(storeId);
					this.doCancelClose(old.getId(), 2, lbp.getLinker(), lbp.getTel(), lbp.getAddress(), store.getId(),
							store.getName(), "sys", "重新指定门店（" + store.getName() + "）发货", old.getRequisitionId(), 0);
					old.setType(2);
				} else {
					Store store = storeService.findById(storeId);
					this.doCancelClose(old.getId(), 3, order.getReciver(), order.getContact(),
	...
	public int doSubmit(Long id, String operator) {
		...
		if (success > 0) {
			if (requisitionItem.getType() == ItemType.DEPOTALL.getCode()) {
	...
	public int doDelivery(Long id, Integer quantity, String deliverySn, String deliveryCorp, String operator)
		...
		if (success > 0) {
			...
			} else if (requisitionItem.getType() == ItemType.ORDERPCH.getCode()) {
	...
	public int doReceive(Long id, Integer quantity, String operator, String info) throws Exception {
		...
		if (!requisitionItem.getDeliveryQuantity().equals(quantity + requisitionItem.getDefectiveQuantity())) {
			throw new BusinessException("款号:" + requisitionItem.getProductSn() + "，颜色：" + requisitionItem.getSp1Value()
					+ "，尺码：" + requisitionItem.getSp2Value() + "，发货数量必须等于实收数量加上次品数量！");
		}
		if (requisitionItem.getDefectiveQuantity() > 0) {
			this.doRequisition(requisitionItem.getDesignerId(), requisitionItem.getProductId(),
					requisitionItem.getBarCode(), requisitionItem.getDefectiveQuantity(), requisitionItem.getOrderSn(),
					requisitionItem.getStoreId(), requisitionItem.getRequisitionSn());
		}
		...
		if (success > 0) {
			...
			if (requisitionItem.getType() == ItemType.ORDERPCH.getCode()
					|| requisitionItem.getType() == ItemType.DEPOTALL.getCode()) {
	...
	private void doRequisition(Long brandId, Long productId, String barCode, Integer quantity, String orderSn,
			Long storeId, String relationSn) {
		Store store = storeService.findById(storeId);
		Brand brand = brandService.findById(brandId);
		Product product = productService.findById(productId);
		ProductSku productSku = productSkuService.findByBarCode(barCode);
		RequisitionItem requisitionItem = new RequisitionItem(brand, product, productSku, store,
				ItemType.QUALITYRTN.getCode(), ItemStaus.DELIVERED);
		requisitionItem.setRelationSn(relationSn);
		requisitionItem.setStatus(2);
		requisitionItem.setQuantity(quantity);
		requisitionItem.setOperation(brand.getOperation());
		requisitionItem.setOrderSn(orderSn + "-R");
		this.doReshipInsert(requisitionItem, "系统", null, null, null);
	}
	...
	private void doRequisition(RequisitionItem rqItem, String modifyMan) {
		...
		if ((1 == rqItem.getStatus() || 8 == rqItem.getStatus())
			...
			requisitionItem = new RequisitionItem(brand, product, sku, lbp, 5, ItemStaus.WAITDELIVERED);
			requisitionItem.setQuantity(rqItem.getQuantity());
			requisitionItem.setOrderSn(rqItem.getRequisitionSn());
			requisitionItem.setOperation(brand.getOperation());
			...
			requisitionItem = new RequisitionItem(brand, product, sku, lbp, 6, ItemStaus.WAITDELIVERED);
			requisitionItem.setQuantity(rqItem.getQuantity());
			requisitionItem.setOrderSn(rqItem.getRequisitionSn());
			requisitionItem.setOperation(brand.getOperation());
			requisitionItem.setRelationSn(order.getOrderSn());
			...
			requisitionItem = new RequisitionItem(order, orderItem, product, sku, lbp, jgcbt, brand.getOperation(),
					ItemType.DEPOTALL);
			requisitionItem.setQuantity(rqItem.getQuantity());
			requisitionItem.setOrderSn(rqItem.getRequisitionSn());
			requisitionItem.setRelationSn(order.getOrderSn());
		}
		this.doDeportAllInsert(requisitionItem, modifyMan);
	}
	...
	public int updateLogisticAddress(Long id, String contact, String tel, String address, String operator) {
		RequisitionItem rqItem = this.findOneById(id);
		int success = requisitionItemMapper.updateLogisticAddress(id, contact, tel, address);
		if (success > 0) {
			JSONObject info = new JSONObject();
			info.put("操作", "修改收货地址");
	 ...
	 * @throws Exception
	 */
	private void sendBurgeonOnTransfer(RequisitionItem item) throws Exception {
		...
		if (!"003".equals(store.getCode())
		...
		} else if (item.getType() == ItemType.DEPOTALL.getCode()) {// 仓库调拨-内部流转
			if (BusTypeEnum.DPLUS.name().equals(store.getBusType())) {// D+店申请的调拨从D2C022发出，D2C022的库存由管易发货到该门店
				if (item.getStatus() == 8) {
			...
			} else {// 普通门店（仓库）的仓库调拨（目前只有退换货用到）
				if ("D2C101".equals(store.getCode())) {
					...
					if (item.getStatus() == 2) {// 退换货单仓库确认后生成，从零售仓到销退仓078（实际上并没有从零售仓发货，但是因为零售退货到零售仓，所以从零售仓出，做到销退仓。随后需发往加工仓）
						...
						data.put("dest", "078");
						data.put("remark", "订单编号:" + oi.getOrderSn() + ",sku:" + item.getBarCode()
								+ "退换货,仓库确认收货后，系统自动做零售退货单后做调拨单");
					}
					if (item.getStatus() == 8) {// 加工仓确认后，销退仓078做到加工仓
						data.put("orig", "078");
						data.put("dest", "D2C101");
						data.put("remark",
								"订单编号:" + oi.getOrderSn() + ",sku:" + item.getBarCode() + "退换货，加工仓确认收货后，系统自动做调拨单");
					}
				}
			}
		} else {
		...
		try {
			if (requisitionItem.getType() == ItemType.STOREPCH.getCode()
				...
				if (!"003".equals(store.getCode())) {
					fallCount = 1;
					type = DocType.Retail;
					this.sendBurgeonOnTransfer(requisitionItem);
				}
			}
			if (requisitionItem.getType() == ItemType.STOREALLC.getCode()) {
				fallCount = 2;
			...
			if (requisitionItem.getType() == ItemType.DEPOTALL.getCode()) {// TODO
	...
	public int doReProcess(Long id) {
		...
		if (DocSourceType.ORDERITEM.name().equals(burgeonErrorLog.getSourceType())) {
			return this.doBurgeonForOrderItem(burgeonErrorLog.getSourceId(),
					DocType.valueOf(burgeonErrorLog.getType()));
		}
		Integer fallCount = 2;
		DocType type = null;
		try {
			...
			if (requisitionItem.getType() == ItemType.ORDERPCH.getCode()) {
				...
				if (burgeonErrorLog.getFallCount() == 1 && DocType.Retail.name().equals(burgeonErrorLog.getType())) {// 补做零售
					fallCount = 1;
					type = DocType.Retail;
					this.sendBurgeonOnRetail(requisitionItem);
				}
	...
	@Override
	public RequisitionItem doDeportAllInsert(RequisitionItem requisitionItem, String operator) {
		RequisitionItem item = this.doPurchaseInsert(requisitionItem, operator);
		requisitionItem.setReceiveQuantity(requisitionItem.getQuantity());
	...
	private void sendRetailForOrderItem(OrderItem orderItem) throws Exception {
		...
		Date nowDate = new Date();
		JSONObject data = new JSONObject();
		data.put("refno", orderItem.getOrderSn());
		data.put("billdate", fmt.format(nowDate));
