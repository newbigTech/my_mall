...
import com.d2c.msg.model.RefundLog.RefundLogType;
import com.d2c.msg.service.RefundLogService;
...
import com.d2c.order.third.payment.alipay.core.pcwap.AlipayUtil;
...
import com.d2c.order.third.payment.wxpay.refund.RefundConfig1;
...
public class RefundServiceImpl extends ListServiceImpl<Refund> implements RefundService {
	...
	@Autowired
	private RefundConfig1 refundConfig1;
	...
	@Override
	@Transactional(rollbackFor = Exception.class, readOnly = false, propagation = Propagation.REQUIRED)
	public int doThirdRefund(Long refundId, Integer backAccountType, BigDecimal payMoney, String payer, String ip) {
		Refund refund = findById(refundId);
		if (refund == null) {
		...
		Setting setting = settingService.findByCode(Setting.REFUNDCODE);
		if (setting != null) {
			BigDecimal limitPrice = new BigDecimal(0);
			if (setting.getStatus() != 1) {
				throw new BusinessException("无密退款功能已关闭，不支持无密退款！");
			} else if (setting.getStatus() == 1 && StringUtils.isNotEmpty(setting.getValue())) {
				limitPrice = new BigDecimal(setting.getValue());
			}
			if (payMoney.compareTo(limitPrice) > 0) {
				throw new BusinessException("未设置或超过无密退款的退款金额上限！");
			}
		} else {
			throw new BusinessException("无密退款功能已关闭，不支持无密退款！");
		}
		int result = 0;
		if (refund.getOrderPayType() != null) {
			// 财务退款时间
			Date payDate = new Date();
			// 把状态4变成5，并设置autoRefund 为无密退款
			result = refundMapper.doThirdRefund(refund.getId(), backAccountType, payDate, payMoney, payer);
			if (result > 0) {
				String refundResult = "";
				if (refund.getOrderPayType() == PaymentTypeEnum.ALIPAY.getCode()) {
					// 支付宝
					try {
						refundResult = AlipayRefundClient.doRefund(alipayConfig, payDate,
								DateUtil.day2str(new Date()) + refund.getRefundSn(), "1",
								refund.getBackAccountName() + "^" + refund.getTotalAmount() + "^" + "退款退货");
					} catch (Exception e) {
						logger.error(e.getMessage(), e);
					}
				} else if (refund.getOrderPayType() == PaymentTypeEnum.WXAPPPAY.getCode()) {
					// 微信支付
					try {
						WXPay client = new WXPay(refundConfig1, refundConfig1.getRefund_notify_url(), false, false);
						WeixinRefundHelper helper = new WeixinRefundHelper();
						Payment payment = paymentService.findById(refund.getOrderPaymentId());
						Map<String, String> response = client.refund(helper.createParamMap(refund.getOrderSn(),
								refund.getRefundSn(), payment.getTotalAmount(), refund.getTotalAmount()));
				...
				if ("SUCCESS".equalsIgnoreCase(refundResult)) {
					String info = "财务已退款，预计2-5个工作日到账，退款时间：" + DateUtil.day2str(payDate);
					this.insertRefundLog(refund, payer, info, RefundLogType.modify);
					return 1;
				} else {
					throw new BusinessException("退款不成功：" + AlipayUtil.convertError(refundResult));
				}
			}
		} else {
