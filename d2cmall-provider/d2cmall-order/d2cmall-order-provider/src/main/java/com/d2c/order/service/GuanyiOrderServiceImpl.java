...
public class GuanyiOrderServiceImpl extends ListServiceImpl<GuanyiOrder> implements GuanyiOrderService {
	...
	private void sendBurgeonOnTransfer(final GuanyiOrderDto guanyiOrderDto) throws Exception {// TODO
		...
		if (!"D2C001".equals(guanyiOrderDto.getShopCode())) {
			...
			data.put("remark", "管易订单编号:" + guanyiOrderDto.getCode() + "," + guanyiOrderDto.getVipName() + "物流:"
					+ guanyiOrderDto.getExpressName() + "," + guanyiOrderDto.getExpressNo() + ",系统自动做调拨单");
		} else {
	...
	private void sendBurgeonOnRetail(final GuanyiOrderDto guanyiOrderDto) throws Exception {
		...
		List<OrderItem> list = orderItemService.findByOrderSn(guanyiOrderDto.getPlatformCode());
		OrderItem item = null;
		if (list != null && list.size() > 0 && (item = list.get(0)).getDplusCode() != null) {
			data.put("store", item.getDplusCode());
	...
	// * @throws Exception
	// */
	// private void processExpress(GuanyiOrderDto guanyiOrderDto) throws
	// Exception { // TODO按明细发货，记录管易传过来的平台定好，忽略飞机盒。
	// String[] orderSns = guanyiOrderDto.getPlatformCode().split(";");
	// List<String> orderSku = new ArrayList<>();
	// for (String orderSn : orderSns) {
	// for (GuanyiOrderItem guanyiOrderItem : guanyiOrderDto.getItems()) {
	// orderSku.add(orderSn + guanyiOrderItem.getSkuCode());
	// }
	// }
	// List<OrderItem> allItemList = new ArrayList<>();
	...
	// }
	// List<OrderItem> list = orderItemService.findByOrderId(order.getId());
	...
	// }
	// if (!orderSku.contains(orderItem.getOrderSn() +
	// orderItem.getDeliveryBarCode())) {
	// throw new Exception("实发条码不一致");
	// }
	// if (orderItem.getReshipId() != null || orderItem.getRefundId() != null
	// || orderItem.getExchangeId() != null) {
	// if (orderItemService.findInAfter(orderItem.getId()) == 1) {
	// throw new Exception("订单明细处于售后中");
	// }
	// }
	// skus.add(orderItem.getDeliveryBarCode() + "_" +
	// orderItem.getProductQuantity());
	// }
	// for (GuanyiOrderItem goi : guanyiOrderDto.getItems()) {
	// if (!skus.contains(goi.getSkuCode() + "_" + goi.getQty())) {
	// throw new Exception("实发条码或商品数量不一致");
	// }
	// skus.remove(goi.getSkuCode() + "_" + goi.getQty());
	// }
	// for (Long orderId : orderIds) {
	// orderService.doDeliverOrder(orderId, guanyiOrderDto.getExpressName(),
	// guanyiOrderDto.getExpressNo(),
	// "sys管易发货", "");
	// }
	// }
