...
import com.d2c.msg.service.RecommendLogService;
...
public class AccountTxServiceImpl implements AccountTxService {
	...
	public AccountItem doConsume(OrderInfo orderInfo) {
		...
		// 有期限的赠送金额
		if (account.getLimitGiftAmount().compareTo(zero) > 0) {
			BigDecimal limitGiftAmount = giftAmount;
			if (account.getLimitGiftAmount().compareTo(giftAmount) < 0) {
				limitGiftAmount = account.getLimitGiftAmount();
			}
			orderInfo.setLimited(1);
			orderInfo.setLimitGiftAmount(limitGiftAmount);
		}
		...
		// 有期限的赠送金额
		if (accountItem.getLimited() > 0) {
			accountService.doUpateLimitAmount(account.getId(), accountItem.getFactLimitGiftAmount(),
					accountItem.getLimitDate());
		}
		return accountItem;
	}
	...
	public AccountItem doRefund(OrderInfo orderInfo) {
		...
		if (payItem.getLimited() > 0) {
			BigDecimal lessGiftAmount = new BigDecimal(0);
			if (payItem.getGiftAmount().subtract(payItem.getLimitGiftAmount()).compareTo(zero) > 0) {
				lessGiftAmount = this.rateCurrency(payItem.getGiftAmount().subtract(payItem.getLimitGiftAmount()),
						payItem.getGiftAmount(), giftAmount);
			}
			orderInfo.setGiftAmount(lessGiftAmount.negate());
			orderInfo.setLimitGiftAmount(giftAmount.subtract(lessGiftAmount).negate());
		}
		AccountItem accountItem = new AccountItem(orderInfo);
