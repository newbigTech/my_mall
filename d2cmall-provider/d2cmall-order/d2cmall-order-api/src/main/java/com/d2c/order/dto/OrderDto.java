...
public class OrderDto extends Order implements IPaymentInterface {
	 ...
	 * @param calitem
	 */
	public OrderDto initPromotion(PromotionCalculateResult calResult) {
		if (calResult == null) {
			return this;
		}
		...
		// 普通订单运费满299包邮, 跨境订单运费满88包邮, 经销商订单运费另算
		if (calResult.getType().equals(OrderType.distribution.name())) {
			this.setType(calResult.getType());
			this.setShippingRates(calResult.getShipFee());
		} else {
			BigDecimal needAmount = (this.getCrossBorder() == 1 ? new BigDecimal(88) : new BigDecimal(299));
			this.setShippingRates(PromotionCalculateResult.shippingRates(needAmount, calResult.getTotalAmount()));
		}
		return this;
	}
