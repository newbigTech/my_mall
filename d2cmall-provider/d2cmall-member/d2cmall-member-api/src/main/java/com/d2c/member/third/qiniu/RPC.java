...
final class RPC {
	...
	public String callWithJson(String urlStr, String json) throws Exception {
		...
		Request request = new Request.Builder().url(url).post(rBody).header("User-Agent", Config.APIUserAgent)
				.addHeader("Authorization", "Qiniu " + macToken).addHeader("Content-Type", contentType).build();
	...
	public String callWithGet(String urlStr) throws Exception {
		...
		Request request = new Request.Builder().url(url).get().header("User-Agent", Config.APIUserAgent)
				.addHeader("Authorization", "Qiniu " + macToken).build();
	...
	public String callWithDelete(String urlStr) throws Exception {
		...
		Request request = new Request.Builder().url(url).delete().header("User-Agent", Config.APIUserAgent)
				.addHeader("Authorization", "Qiniu " + macToken).build();
