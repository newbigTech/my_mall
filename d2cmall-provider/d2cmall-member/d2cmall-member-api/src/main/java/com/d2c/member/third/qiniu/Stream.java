...
public final class Stream {
	...
	Stream(StreamInfo info, RPC cli) throws UnsupportedEncodingException {
		...
		this.baseUrl = String.format("%s%s/v2/hubs/%s/streams/%s", Config.APIHTTPScheme, Config.APIHost, info.getHub(),
				ekey);
		this.cli = cli;
		this.gson = new Gson();
	}
	...
	public static class SaveOptions {
		...
		/**
		 * URL address. After dora asynchronous operation is done, will notify
		 * this address
		 */
		public String notify;
		/**
		 * ts's expiration days -1 means no change of ts's expiration 0 means
		 * storing forever any other positive number can change the ts's
		 * expiration days
		 */
