...
import com.d2c.msg.service.AccountLogService;
...
public class AccountServiceImpl extends ListServiceImpl<Account> implements AccountService {
	...
	public int doUpdateAmount(Long accountId, BigDecimal cashAmount, BigDecimal giftAmount) {
		if (giftAmount == null) {
			giftAmount = new BigDecimal("0");
		}
		Account account = accountMapper.selectByPrimaryKey(accountId);
		int success = accountMapper.doInOutAmount(accountId, cashAmount, giftAmount);
		if (success > 0) {
			String info = "";
			String title = "";
	...
	@TxTransaction
	@Transactional(readOnly = false, rollbackFor = Exception.class)
	public int doUpateLimitAmount(Long accountId, BigDecimal limitGiftAmount, Date limitDate) {
		int result = accountMapper.doUpateLimitAmount(accountId, limitGiftAmount, limitDate);
		return result;
	}
