...
public class MemberShareServiceImpl extends ListServiceImpl<MemberShare> implements MemberShareService {
	...
	public MemberShare insert(MemberShare memberShare) {
		...
		if (memberShare.getType() == 1 && memberShare.getStatus() == 1) {
			IntegrationRule rule = integrationRuleService.findVaildByType(PointRuleTypeEnum.MEMBERSHARE.name());
			if (rule != null && Integer.parseInt(rule.getRatio()) > 0) {
				MemberIntegration integration = new MemberIntegration(memberShare.getMemberId(), null,
						PointRuleTypeEnum.MEMBERSHARE.name(),
						PointRuleTypeEnum.MEMBERSHARE.calculatePoint(rule.getRatio(), null), memberShare.getId(), null,
						memberShare.getCreateDate(), rule.getName(), 1, StatusEnum.SUCCESS.getCode());
		...
		if (res > 0) {
			...
			if (memberShare != null && memberShare.getType() == 1) {
				...
				if (rule != null && Integer.parseInt(rule.getRatio()) > 0) {
					MemberIntegration integration = new MemberIntegration(memberShare.getMemberId(), null,
							PointRuleTypeEnum.MEMBERSHARE.name(),
							PointRuleTypeEnum.MEMBERSHARE.calculatePoint(rule.getRatio(), null), memberShare.getId(),
							null, memberShare.getCreateDate(), rule.getName(), 1, StatusEnum.SUCCESS.getCode());
					memberIntegrationService.doSendIntegration(integration);
				}
			}
		}
