...
public class PullingHandler extends AsyncProcessHandler {
	...
	public class Params {
		 ...
		 * 请求参数
		 * <p>
		 * bucket_name string 是 文件所在空间名称 notify_url string 是 回调通知地址 tasks string
		 * 是 处理任务信息，详见下 app_name string 是 任务所使用的云处理程序，文件拉取为 spiderman
		 */
		 ...
		 * 回调通知参数
		 * <p>
		 * task_id string 任务对应的 TaskId bucket_name string 文件所在的空间名 status_code
		 * integer 处理结果状态码，200 表示成功处理 path string 文件保存路径 error string
		 * 处理错误信息描述，空字符串表示没有错误
		 */
		 ...
		 * x-gmkerl-thumb string 图片处理参数，见 上传作图 random bool 是否追加随机数，默认 true
		 * overwrite bool 是否覆盖，默认 false save_as string 文件存放路径，必填项， 如
		 * /site/index.html
		 */
