...
public class BargainPromotionServiceImpl extends ListServiceImpl<BargainPromotion> implements BargainPromotionService {
	...
	@Override
	@Transactional(readOnly = false, rollbackFor = Exception.class, propagation = Propagation.REQUIRED)
	public int doRemind(Long memberId, SearcherBargainPromotion bargainPromotion) {
		...
		if (seconds > 0) {
			this.bargainRemindMQ(memberId, bargainPromotion.getId(), seconds / 1000);
		} else {
	...
	private void bargainRemindMQ(Long memberId, Long promotionId, Long seconds) {
