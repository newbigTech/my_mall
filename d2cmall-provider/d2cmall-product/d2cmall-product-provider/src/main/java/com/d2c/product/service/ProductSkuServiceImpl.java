...
import com.d2c.msg.model.ProductLog.ProductLogType;
import com.d2c.msg.model.SkuOperateLog;
import com.d2c.msg.service.ProductLogService;
import com.d2c.msg.service.SkuOperateLogService;
...
public class ProductSkuServiceImpl extends ListServiceImpl<ProductSku> implements ProductSkuService {
	...
	@TxTransaction
	@Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRED, readOnly = false)
	public int doCancelFreezeAndDeduceStock(Long productId, Long skuId, int quantity) {
		ProductSku sku = productSkuMapper.selectByPrimaryKey(skuId);
		int pop = (sku.getStore() >= quantity ? 0 : 1);
		int result = productSkuMapper.cancelFreezeAndDeduceStock(skuId, quantity, pop);
	...
	@TxTransaction
	@Transactional(rollbackFor = Exception.class, propagation = Propagation.REQUIRED, readOnly = false)
	public int doCancelFreezeAndDeduceFlashStock(Long productId, Long skuId, int quantity) {
		ProductSku sku = productSkuMapper.selectByPrimaryKey(skuId);
		int pop = (sku.getStore() >= quantity ? 0 : 1);
		int result = productSkuMapper.cancelFreezeAndDeduceFlashStock(skuId, quantity, pop);
		if (result > 0 && productId != null) {
			redisTemplate.delete("product_flash_store_" + productId);
		}
		return pop;
	...
	public int updateAPrice(Long skuId, BigDecimal aPrice, String operator) {
		...
		if (success > 0) {
			Product product = productService.findById(oldSku.getProductId());
			if (product.getaPrice().compareTo(aPrice) > 0) {
				productService.updateAPrice(oldSku.getProductId(), aPrice);
			}
