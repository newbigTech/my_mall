...
import com.d2c.order.model.RequisitionItem.PurchaseRole;
import com.d2c.order.model.RequisitionItem.PurchaseType;
...
public class RequisitionItemCtrl extends BaseCtrl<RequisitionItemSearcher> {
		...
		return this.processImportExcel(request, new EachRow() {
			...
			public boolean process(Map<String, Object> map, Integer row, StringBuilder errorMsg) {
				...
				RequisitionItem requisitionItem = new RequisitionItem(store, brand, product, productSku,
						ItemType.GOODSPCH.getCode(), orderSn);
				requisitionItem.setQuantity(Integer.parseInt(quantity));
				...
				try {
					requisitionItemService.doPurchaseInsert(requisitionItem, admin.getUsername());
				} catch (Exception e) {
	...
	public Response importType56(HttpServletRequest request, Integer type) {
		...
		SuccessResponse result = this.processImportExcel(request, new EachRow() {
			...
			public boolean process(Map<String, Object> map, Integer row, StringBuilder errorMsg) {
				...
				Brand brand = brandService.findById(product.getDesignerId());
				RequisitionItem requisitionItem = new RequisitionItem(brand, product, productSku, lbp, type,
						ItemStaus.DELIVERED);
				requisitionItem.setQuantity(Integer.parseInt(quantity));
				requisitionItem.setOrderSn(orderSn);
				requisitionItem.setOperation(brand.getOperation());
				...
				try {
					...
					String deliverySn = String.valueOf(map.get("物流单号"));
					String remark = map.get("备注") == null ? "" : String.valueOf(map.get("备注"));
					requisitionItem = requisitionItemService.doReshipInsert(requisitionItem, admin.getUsername(),
							deliveryCorp, deliverySn, remark);
	 ...
	 */
	@RequestMapping(value = "/sys/import", method = RequestMethod.POST)
	public Response importType1() throws BusinessException, ParseException, NotLoginException {
		...
		int totalCount = orderItemService.countPopOrderitems(deadline);
		PageResult<OrderItem> pager = new PageResult<>(page);
		pager.setTotalCount(totalCount);
		do {
			page.setPageNumber(pagerNumber);
			pager = orderItemService.findPopOrderitems(deadline, page);
			for (OrderItem orderItem : pager.getList()) {
				if (orderItem.getRefundId() != null || orderItem.getReshipId() != null
						|| orderItem.getExchangeId() != null) {
					continue;
				}
				requisitionItemService.doSysDesignerDelivery(orderItem);
			}
	...
	public Response doApply(String barCode, Integer quantity, String orderSn, String storeMemo, String purchaseReason,
			String storeCode) throws NotLoginException, BusinessException {
		...
		RequisitionItem requisitionItem = new RequisitionItem(store, brand, product, productSku,
				ItemType.STOREPCH.getCode(), orderSn);
	...
	public Response importApply(HttpServletRequest request) {
		AdminDto admin = this.getLoginedAdmin();
		return this.processImportExcel(request, new EachRow() {
	...
	public Response doAsk(String barCode, Integer quantity, String storeMemo) {
		AdminDto admin = this.getLoginedAdmin();
		SuccessResponse result = new SuccessResponse();
		ProductSku productSku = productSkuService.findByBarCode(barCode);
		if (productSku == null) {
